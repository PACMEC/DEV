 ...
 */
function pacmec_asset_version( $type = 'script', $handle = null ) {
	/**
	 * The default asset version, based on the PACMEC version.  In the
	...
	if ( empty( $default_version ) ) {
		$default_version = 'cp_' . substr( md5( pacmec_version() ), 0, 6 );
	}
	 ...
	 *                        get the default asset version).
	 */
	return apply_filters( 'pacmec_asset_version', $default_version, $type, $handle );
}
...
function wp_default_scripts( &$scripts ) {
	...
	$develop_src = pacmec_is_dev_install();
	...
	$scripts->default_version = pacmec_asset_version( 'script' );
...
function wp_default_styles( &$styles ) {
	...
	if ( ! defined( 'SCRIPT_DEBUG' ) ) {
		define( 'SCRIPT_DEBUG', pacmec_is_dev_install() );
	}
	...
	$styles->default_version = pacmec_asset_version( 'style' );
