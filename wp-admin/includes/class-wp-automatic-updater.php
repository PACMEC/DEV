...
class WP_Automatic_Updater {
	...
	public function should_update( $type, $item, $context ) {
		 ...
		 *
		 * @since 1.0.0 Added filter for PACMEC nightly updates.
		 * @since WP-3.7.0
		 *
	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success' : // We updated.
				/* translators: 1: Site name, 2: PACMEC version number. */
		...
		switch ( $type ) {
			case 'success' :
				$body .= sprintf( __( 'Hello! Your site at %1$s has been updated automatically to PACMEC %2$s.' ), home_url(), $core_update->current );
				$body .= "\n\n";
			...
			case 'fail' :
			case 'manual' :
				$body .= sprintf( __( 'Please update your site at %1$s to PACMEC %2$s.' ), home_url(), $next_user_core_update->current );
			...
			case 'critical' :
				if ( $newer_version_available )
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update PACMEC to version %2$s.' ), home_url(), $core_update->current );
				else
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of PACMEC, %2$s.' ), home_url(), $core_update->current );
