 ...
 *
 * @since WP-3.7.0
 * @since 1.3.0 Correctly returns checksums for the given PACMEC version,
...
function core_update_footer( $msg = '' ) {
	if ( !current_user_can('update_core') )
		return sprintf( __( 'Version %s' ), pacmec_version() );
	...
	switch ( $cur->response ) {
	case 'development' :
		/* translators: 1: PACMEC version number, 2: PACMEC updates admin screen URL */
		return sprintf( __( 'You are using a development version (%1$s). Cool! Please <a href="%2$s">stay updated</a>.' ), pacmec_version(), network_admin_url( 'update-core.php' ) );
...
function update_nag() {
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: 1: Codex URL to release notes, 2: new PACMEC version, 3: URL to network admin, 4: accessibility text */
		...
		$msg = sprintf(
			/* translators: 1: Codex URL to release notes, 2: new PACMEC version */
...
function update_right_now_message() {
	...
	$msg .= sprintf( '<span id="wp-version">' . $content . '</span>', pacmec_version(), $theme_name );
...
function maintenance_nag() {
	...
	if ( current_user_can('update_core') )
		$msg = sprintf( __('An automated PACMEC update has failed to complete - <a href="%s">please attempt the update again now</a>.'), 'update-core.php' );
	else
