 ...
 */
 ...
 /**
  * Prints information about the PACMEC development version, if any.
  *
  * @since 1.0.0
  */
function pacmec_dev_version_info() {
	if ( ! pacmec_is_dev_install() ) {
		return;
	}
...
final class WP_Privacy_Policy_Content {
	...
	public static function get_default_content( $descr = false ) {
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default PACMEC does not collect any personal data about visitors, and only collects the data shown on the User Profile screen from registered users. However some of your plugins may collect personal data. You should add the relevant information below.' ) . '</p>';

		$content .=
			'<h3>' . __( 'Comments' ) . '</h3>';
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'In this subsection you should note what information is captured through comments. We have noted the data which PACMEC collects by default.' ) . '</p>';
		$content .=
		...
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'By default, PACMEC does not include a contact form. If you use a contact form plugin, use this subsection to note what personal data is captured when someone submits a contact form, and how long you keep it. For example, you may note that you keep contact form submissions for a certain period for customer service purposes, but you do not use the information submitted through them for marketing purposes.' ) . '</p>';

		$content .=
			'<h3>' . __( 'Cookies' ) . '</h3>';
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'In this subsection you should list the cookies your web site uses, including those set by your plugins, social media, and analytics. We have provided the cookies which PACMEC installs by default.' ) . '</p>';
		$content .=
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default PACMEC does not collect any analytics data. However, many web hosting accounts collect some anonymous analytics data. You may also have installed a PACMEC plugin that provides analytics services. In that case, add information from that plugin here.' ) . '</p>';
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default PACMEC does not share any personal data with anyone.' ) . '</p>';
	...
	public static function add_suggested_content() {
		...
		wp_add_privacy_policy_content( __( 'PACMEC' ), $content );
	}
}
