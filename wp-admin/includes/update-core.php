...
$_old_files = array(
	// Added in WP-5.0.0, not included in PACMEC
	...
	// Added in WP-5.2.3 or older, not included in PACMEC
...
function update_core($from, $to) {
	...
	$old_wp_version = $GLOBALS['wp_version']; // The version of PACMEC or WordPress we're updating from
	...
	if ( !$mysql_compat && !$php_compat )
		return new WP_Error( 'php_mysql_not_compatible', sprintf( __('The update cannot be installed because PACMEC %1$s requires PHP version %2$s or higher and MySQL version %3$s or higher. You are running PHP version %4$s and MySQL version %5$s.'), $wp_version, $required_php_version, $required_mysql_version, $php_version, $mysql_version ) );
	elseif ( !$php_compat )
		return new WP_Error( 'php_not_compatible', sprintf( __('The update cannot be installed because PACMEC %1$s requires PHP version %2$s or higher. You are running version %3$s.'), $wp_version, $required_php_version, $php_version ) );
	elseif ( !$mysql_compat )
		return new WP_Error( 'mysql_not_compatible', sprintf( __('The update cannot be installed because PACMEC %1$s requires MySQL version %2$s or higher. You are running version %3$s.'), $wp_version, $required_mysql_version, $mysql_version ) );
	...
	// This gives us the ability to install new plugins & themes bundled with future versions of PACMEC whilst avoiding the re-install upon upgrade issue.
	 ...
	 * @since 1.0.0
	 *
	 * @param string $cp_version The current PACMEC version.
	 */
	do_action( 'pacmec_core_updated_successfully', $cp_version );
	 ...
	 *
	 * @see 'pacmec_core_updated_successfully'
	 *
	 * @since WP-3.3.0
	 *
...
/**
 * Gets the checksums for the given version of PACMEC.
 *
...
/**
 * Verifies and returns the root directory entry of a PACMEC update.
 *
 * For WordPress, this was always '/wordpress/'.  For PACMEC, since
 ...
 * name starts with 'PACMEC-'.
 *
 * We also need to allow the root directory to be called 'wordpress', since
 * this is used when migrating from WordPress to PACMEC.  If the
 ...
 * @since 1.0.0
 *
 * @param string $working_dir The directory where a PACMEC update package
 *                            has been extracted.
 *
...
function cp_get_update_directory_root( $working_dir ) {
	...
	if (
		count( $entries ) === 1 &&
		(
			substr( $entries[0]['name'], 0, 13 ) === 'PACMEC-' ||
...
function _redirect_to_about_wordpress( $new_version ) {
	...
	show_message( '<span class="hide-if-no-js">' . sprintf( __( 'Welcome to PACMEC %1$s. You will be redirected to the About PACMEC screen. If not, click <a href="%2$s">here</a>.' ), $new_version, 'about.php?updated' ) . '</span>' );
	show_message( '<span class="hide-if-js">' . sprintf( __( 'Welcome to PACMEC %1$s. <a href="%2$s">Learn more</a>.' ), $new_version, 'about.php?updated' ) . '</span>' );
	echo '</div>';
	?>
