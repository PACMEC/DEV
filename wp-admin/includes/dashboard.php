...
function wp_dashboard_setup() {
	...
	// PACMEC News
	wp_add_dashboard_widget( 'dashboard_primary', __( 'PACMEC News' ), 'wp_dashboard_events_news' );

	// PACMEC Petitions
	wp_add_dashboard_widget( 'dashboard_petitions', __( 'PACMEC Petitions' ), 'cp_dashboard_petitions' );
...
/**
 * Renders the PACMEC News dashboard widget.
 *
 * @since WP-4.8.0
...
function wp_dashboard_events_news() {
	?>
	<div class="pacmec-news-inline-notice">
		<?php printf(
...
function wp_dashboard_primary() {
	$feeds = array(
		'news' => array(
			...
			/**
			 * Filters the primary link URL for the 'PACMEC News' dashboard widget.
			 *
			 * @since WP-2.5.0
			 *
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		if ( $response['insecure'] ) {
			...
			$msg = sprintf( __( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best PACMEC experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			/* translators: %s: browser name and link */
			$msg = sprintf( __( "It looks like you're using an old version of %s. For the best PACMEC experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
...
function wp_welcome_panel() {
	$display_version = pacmec_version();
?>
<div class="welcome-panel-content">
	<h2><?php _e( 'Welcome to PACMEC!' ); ?></h2>
	<p class="welcome-panel-tagline"><?php printf( __( 'Thank you for trying PACMEC&nbsp;%s!' ), $display_version ); ?></p>
	<p>
		<?php _e(
...
<?php
	if ( get_locale() === 'en_US' ) {
		printf(
			/* translators: link to "About PACMEC" dashboard page */
			__( 'To see how you can help, visit the <a href="%s">About PACMEC</a> page.' ),
		...
		printf(
			/* translators: link to "About PACMEC" dashboard page */
			__( 'For other ways you can help, visit the <a href="%s">About PACMEC</a> page.' ),
			esc_url( self_admin_url( 'about.php' ) )
		);
	}
?>
...
function cp_dashboard_petitions_output( $widget_id, $feeds ) {
	 ...
	 *  'link'        - string - Link to the PACMEC petitions website.
