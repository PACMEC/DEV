...
class Core_Upgrader extends WP_Upgrader {
	...
	public function upgrade( $current, $args = array() ) {
		...
		// PACMEC only supports the "full" upgrade package.
	...
	/**
	 * Determines if the current PACMEC Core version should update to an
	 ...
	 * @since 1.0.0
	 *
	 * @param string $ver_current      The current version of PACMEC.
	 * @param string $ver_offered      The proposed version of PACMEC.
	...
	public static function _auto_update_enabled_for_versions(
		...
		if ( $current['major'] > $offered['major'] ) {
			 ...
			 * PACMEC may contain breaking changes.
			 * @see https://semver.org/
			 *
			 * @since WP-3.7.0
			 * @since 1.0.0 Version numbering scheme changed from WordPress
			 * to PACMEC (semver). New parameters $current and $offered.
			 *
	 ...
	 *
	 * @since WP-3.7.0
	 * @since 1.3.0 Correctly uses the checksums for the current PACMEC
	...
	public function check_files() {
		...
		if ( version_compare( $cp_version, '1.3.0-rc1', '<' ) ) {
			// This version of PACMEC has a `get_core_checksums()`
