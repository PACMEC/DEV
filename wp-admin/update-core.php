...
function list_core_update( $update ) {
	...
	if ( 'development' == $update->response ) {
		$message = __('You are using a development version of PACMEC. You can update to the latest nightly build automatically:');
	} else {
		if ( $current ) {
	...
	elseif ( 'en_US' == $update->locale && get_locale() != 'en_US' && ( ! $update->packages->partial && $wp_version == $update->partial_version ) ) {
	    echo '<p class="hint">'.sprintf( __('You are about to install PACMEC %s <strong>in English (US).</strong> There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.'), $update->response != 'development' ? $update->current : '' ).'</p>';
	}
	echo '</form>';
...
function core_upgrade_preamble() {
	...
	if ( ! isset( $updates[0]->response ) || 'latest' === $updates[0]->response ) {
		if ( ! isset( $updates[0]->response ) ) {
			if ( pacmec_is_dev_install() ) {
				echo '<h2>';
				_e( 'You are running a development version of PACMEC.' );
				echo "</h2>\n";
				echo '<p>';
				_e( 'Development versions of PACMEC do not receive automatic updates.' );
				echo "</p>\n";
			} else {
				echo '<h2>';
				_e( 'Unable to determine whether a PACMEC update is available.' );
				echo "</h2>\n";
				echo '<p>';
				_e( 'You may be running a customized build of PACMEC, or your server may be having internet connectivity problems.' );
				echo "</p>\n";
			}
		} else { // 'latest'
			echo '<h2>';
			_e( 'You have the latest version of PACMEC.' );
			echo "</h2>\n";
		}
	...
	if ( $updates && ( count( $updates ) > 1 || $updates[0]->response != 'latest' ) ) {
		...
		echo '<p>' . sprintf(
			__( '<a href="%s">Learn more about PACMEC %s</a>.' ),
...
<?php
	...
	if ( is_wp_error( $result ) ) {
		...
		switch ( $result->get_error_code() ) {
			case 'up_to_date':
				// PACMEC is already up to date, no need to show a different message
				break;
	...
	show_message( '<span class="hide-if-no-js">' . sprintf( __( 'Welcome to PACMEC %1$s. You will be redirected to the About PACMEC screen. If not, click <a href="%2$s">here</a>.' ), $result, esc_url( self_admin_url( 'about.php?updated' ) ) ) . '</span>' );
	show_message( '<span class="hide-if-js">' . sprintf( __( 'Welcome to PACMEC %1$s. <a href="%2$s">Learn more</a>.' ), $result, esc_url( self_admin_url( 'about.php?updated' ) ) ) . '</span>' );
	?>
	</div>
	<script type="text/javascript">
