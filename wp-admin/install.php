...
// Is PACMEC already installed? If so, offer login link, bail.
if ( is_blog_installed() ) {
	display_header( 'cp-installation-error' );
	echo '<h1>' . __( 'PACMEC Already Installed' ) . '</h1>';
	echo '<p>' . __( 'It seems that PACMEC is already installed. If you are trying to reinstall PACMEC, you must first delete the database tables associated with the previous installation.' ) . '</p>';
...
$cp_download_url = 'https://github.com/PACMEC/PACMEC-release';
...
if ( ! $mysql_compat && ! $php_compat ) {
	...
	$compat = sprintf(
		__( '<a href="' . $cp_download_url . '">PACMEC %1$s</a> requires PHP version %2$s or higher and MySQL version %3$s or higher. You are running PHP version %4$s and MySQL version %5$s.' ),
		$cp_version,
	...
	$compat = sprintf(
		__( '<a href="' . $cp_download_url . '">PACMEC %1$s</a> requires PHP version %2$s or higher. You are running version %3$s.' ),
		$cp_version,
		$required_php_version,
		$php_version
	);
	...
	$compat = sprintf(
		__( '<a href="' . $cp_download_url . '">PACMEC %1$s</a> requires MySQL version %2$s or higher. You are running version %3$s.' ),
		$cp_version,
...
switch($step) {
	...
	case 2:
		...
		if ( $error === false ) {
			...
			echo '<p>' . __( 'Your PACMEC site is ready to go!' ) . '</p>' . "\n";
...
/**
 * Display PACMEC-branded installation header.
 *
 * @since WP-2.5.0
 *
...
function display_header( $body_classes = array() ) {
	...
	echo '<title>' . __( 'PACMEC &rsaquo; Installation' ) . '</title>' . "\n";
...
function display_setup_form( $error = null ) {
	...
	echo '</table>' . "\n";
	echo '<p class="step"><input name="submit" id="submit" type="submit" value="' . htmlspecialchars( __( 'Install PACMEC' ), ENT_QUOTES ) . '" class="button button-primary button-hero cp-button" /></p>' . "\n";
