...
if ( ! $config_file ) {
	...
	echo '<p>' . sprintf(
		...
		__( 'A %s file was not found. Please upload it to the root of your PACMEC installation, or one level higher, and then try again. Need a <a href="%s" target="_blank" rel="noopener">fresh copy</a>?' ),
		'<code>wp-config-sample.php</code>',
		esc_url( 'https://raw.githubusercontent.com/PACMEC/PACMEC-release/master/wp-config-sample.php' )
	) . '</p>';
...
if ( file_exists( ABSPATH . 'wp-config.php' ) ) {
	...
	echo '<p>' . sprintf(
		...
		__( 'A %1$s file was found in your PACMEC installation. If you are trying to reinstall PACMEC, you must first delete that file.</p><p> If you created and uploaded your own config file, you can <a href="%2$s">continue installing</a>.' ),
...
if ( @file_exists( ABSPATH . '../wp-config.php' ) && ! @file_exists( ABSPATH . '../wp-settings.php' ) ) {
	...
	echo '<p>' . sprintf(
			...
			__( 'A %1$s file was found one level above your PACMEC installation. If you are trying to reinstall PACMEC, you must first delete that file.</p><p> If you created and uploaded your own config file, you can <a href="%2$s">continue installing</a>.' ),
			'<code>wp-config.php</code>',
			'install.php'
		) . '</p>';
	setup_config_display_footer();
...
switch ( $step ) {
	...
	case 1:
		...
		echo '		<td><input name="dbname" id="dbname" type="text" size="25" value="pacmec" /></td>';
		echo '	</tr>';
		echo '	<tr>';
...
function setup_config_display_header( $body_classes = array() ) {
	...
	echo '<title>' . __( 'PACMEC &rsaquo; Setup Configuration File' ) . '</title>' . "\n";
