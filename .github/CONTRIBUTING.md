# Contributing to PACMEC

PACMEC is a volunteer-driven open source project that thrives when we have multiple contributors working towards a common goal.
...
- [Set up a local development environment](#setting-up-a-local-development-environment) and try out PACMEC on your own computer.
- Review and test [existing PRs](https://github.com/PACMEC/PACMEC/pulls), especially looking at how well they fit the criteria described above. Let us know how you tested the changes and what you found. We need to be thorough and careful with any changes we make, and the more eyes we can get on our PRs the better. Screenshots and text instructions documenting the status and testing of all PRs are very useful, as well as videos and gifs if applicable. These help us know the status and completion of each PR so that we can be thorough and careful with any changes we make, and the more eyes we can get on our PRs the better.
- Take a look at issues with the [`help wanted`](https://github.com/PACMEC/PACMEC/labels/help%20wanted) or [`good first issue`](https://github.com/PACMEC/PACMEC/labels/good%20first%20issue) labels.
- Submit PRs based on our [planned milestones](https://github.com/PACMEC/PACMEC/milestones), or exploratory PRs with your own suggested changes. Please remember these will be subject to review to make sure they are in line with the project's direction.
...
2. Fork PACMEC to your GitHub account using the GitHub website.
3. Clone your PACMEC fork to your computer using the following command:

   ```
   git clone https://github.com/YOUR_GITHUB_USERNAME/PACMEC
   ```
...
4. Change to the PACMEC repository: `cd PACMEC`
5. Add the main PACMEC repository so that you can pull changes from it: `git remote add upstream https://github.com/PACMEC/PACMEC`
6. Run `git remote -v` and confirm that you have your own fork set as `origin` and the main PACMEC repository set as `upstream`. The rest of this document assumes you have things set up this way.
...
- Set up `grunt` to run the pre-commit checks, make your own builds of PACMEC, and perform other miscellaneous build and development tasks:
  - Set up [`nvm`](https://github.com/nvm-sh/nvm) or a similar program to manage Node versions.
  - Run `nvm install` or use your version manager to switch to the current version of Node used by PACMEC. Run this step periodically.
  - Run `npm install` to install/update the PACMEC `npm` dependencies. Run this step periodically.
  - When changing Node versions, you may need to run `npm install -g grunt-cli` to make the `grunt` command work.
  - Run `grunt build` to create your own build of PACMEC (in the `build` directory) or run `grunt precommit:verify` to test whether any other files may need to be updated for your PR. There are many other `grunt` commands available in the `Gruntfile.js` file.
...
- You can always refresh your PR branch against the latest PACMEC code using the following sequence of commands:
...
2. Set up a remote link to the user who submitted the PR. For example, if GitHub user `bahiirwa` submitted the PR:
   ```
   git remote add bahiirwa https://github.com/bahiirwa/PACMEC
   git fetch bahiirwa
   ```
...
_This section only applies to **core committers** - people who have access to merge changes to PACMEC._

See [MAINTAINING.md](MAINTAINING.md) for some guidelines for maintaining PACMEC.

## Automated tests

Any change that introduces new code or changes behavior should have automated tests. These tests mostly use [PHPUnit](https://phpunit.de/) to verify the behavior of the many thousands of lines of PHP code in PACMEC.
...
PACMEC version `1.0.0` is a fork of [WordPress `4.9.x`](https://github.com/PACMEC/PACMEC/tree/LAST_WP_COMMIT). Since then, a number of changes have been made to WordPress for performance, bugfixes or new features.

PACMEC `1.x.x` is committed to keeping backward compatibility with the WordPress 4.9 branch. However, we're also open to merging bugfixes and enhancements from later versions of WordPress, **as long as the review criteria listed above are met**.
...
- In the body text of the PR, explain what the specific change is, how it works and why PACMEC in particular should adopt it.
...
- If there are merge conflicts during the backport, check if they have been resolved correctly by comparing the final changes from the full PR against the corresponding WP changeset(s). The code changes to be applied to PACMEC should be basically the same as the code changes that were applied to WordPress.
