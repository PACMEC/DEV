...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\StreamCipher;
 ...
 * @access  public
 */
class Salsa20 extends \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\StreamCipher
{
    /**
     * Part 1 of the state
     *
    ...
    private function crypt($text, $mode)
    {
        ...
        if (!\strlen($buffer['ciphertext'])) {
            ...
            $ciphertext = $text ^ \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($buffer['ciphertext'], \strlen($text));
        ...
        // & 0x3F
        if ($overflow) {
            $text2 = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::pop($text, $overflow);
            if ($this->engine == self::ENGINE_OPENSSL) {
                ...
                $temp = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::pop($encrypted, 64);
            } else {
                $blocks = \str_split($text, 64);
                if (\strlen($text)) {
                    foreach ($blocks as &$block) {
