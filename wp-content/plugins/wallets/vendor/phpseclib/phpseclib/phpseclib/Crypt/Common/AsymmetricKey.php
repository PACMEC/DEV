...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\ECDSA;
/**
...
abstract class AsymmetricKey
{
    ...
    protected function __construct()
    {
        ...
        $this->hash = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash('sha256');
        $this->hmac = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash('sha256');
    }
    /**
    ...
    {
        if (!isset(self::$zero)) {
            self::$zero = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(0);
            self::$one = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(1);
        }
    ...
    public static function load($key, $password = \false)
    {
        ...
        if ($components === \false) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\NoKeyLoadedException('Unable to read key');
        }
        ...
        $new->format = $format;
        $new->comment = $comment;
        return $new instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey ? $new->withPassword($password) : $new;
    }
    /**
     * Loads a private key
     *
    ...
    public static function loadPrivateKey($key, $password = '')
    {
        ...
        if (!$key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\NoKeyLoadedException('The key that was loaded was not a private key');
        }
        return $key;
    }
    /**
    ...
    {
        $key = self::load($key);
        if (!$key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PublicKey) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\NoKeyLoadedException('The key that was loaded was not a public key');
        }
        return $key;
    }
    /**
    ...
    {
        $key = self::load($key);
        if (!$key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey && !$key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PublicKey) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\NoKeyLoadedException('The key that was loaded was not a parameter');
        }
        return $key;
    }
    /**
    ...
    protected static function validatePlugin($format, $type, $method = NULL)
    {
        ...
        if (!isset(self::$plugins[static::ALGORITHM][$format][$type])) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedFormatException("{$type} is not a supported format");
        }
        $type = self::$plugins[static::ALGORITHM][$format][$type];
        if (isset($method) && !\method_exists($type, $method)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedFormatException("{$type} does not implement {$method}");
        }
        return $type;
    }
    /**
    ...
    public function getLoadedFormat()
    {
        if (empty($this->format)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\NoKeyLoadedException('This key was created with createKey - it was not loaded with load. Therefore there is no "loaded format"');
        }
    ...
    public function withHash($hash)
    {
        ...
        $new->hmac = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash($hash);
        return $new;
    }
    /**
    ...
    {
        $v = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($in, 256);
        $vlen = \strlen($in) << 3;
