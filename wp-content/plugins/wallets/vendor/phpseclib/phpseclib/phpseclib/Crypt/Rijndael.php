...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\BlockCipher;
 ...
 * @access  public
 */
class Rijndael extends \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\BlockCipher
{
    /**
    ...
    public function __construct($mode)
    {
        ...
        if ($this->mode == self::MODE_STREAM) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\BadModeException('Block ciphers cannot be ran in stream mode');
        }
    }
    /**
     * Sets the key length.
    ...
    protected function setup()
    {
        ...
        if (\is_string($this->iv) && \strlen($this->iv) != $this->block_size) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\InconsistentSetupException('The IV length (' . \strlen($this->iv) . ') does not match the block size (' . $this->block_size . ')');
        }
    }
    /**
    ...
    public function encrypt($plaintext)
    {
        ...
        switch ($this->engine) {
            case self::ENGINE_LIBSODIUM:
                ...
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->newtag, \strlen($plaintext));
            case self::ENGINE_OPENSSL_GCM:
    ...
    public function decrypt($ciphertext)
    {
        ...
        switch ($this->engine) {
            case self::ENGINE_LIBSODIUM:
                ...
                if ($plaintext === \false) {
                    $this->oldtag = \false;
                    throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\BadDecryptionException('Error decrypting ciphertext with libsodium');
                }
                return $plaintext;
            case self::ENGINE_OPENSSL_GCM:
                if ($this->oldtag === \false) {
                ...
                if ($plaintext === \false) {
                    $this->oldtag = \false;
                    throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\BadDecryptionException('Error decrypting ciphertext with OpenSSL');
                }
                return $plaintext;
        }
