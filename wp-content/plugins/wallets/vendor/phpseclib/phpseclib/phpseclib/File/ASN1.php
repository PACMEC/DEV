...
abstract class ASN1
{
    ...
    {
        if ($encoded instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element) {
    ...
    private static function decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                ...
                $current['content'] = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(\substr($content, $content_pos), -256);
                break;
            case self::TYPE_REAL:
    ...
    public static function asn1map($decoded, $mapping, $special = [])
    {
        ...
        switch (\true) {
            case $mapping['type'] == self::TYPE_ANY:
                ...
                if (isset($decoded['constant']) || !\array_key_exists($intype, self::ANY_MAP) || \ord(self::$encoded[$decoded['start']]) & 0x20) {
                    return new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element(\substr(self::$encoded, $decoded['start'], $decoded['length']));
                }
        ...
        switch ($decoded['type']) {
            ...
            case self::TYPE_ENUMERATED:
                ...
                if (isset($mapping['implicit'])) {
                    $temp = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($decoded['content'], -256);
                }
                if (isset($mapping['mapping'])) {
    ...
    {
        $length = \ord(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($string));
        if ($length & 0x80) {
            // definite length, long form
            $length &= 0x7f;
            $temp = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($string, $length);
    ...
    {
        if ($source instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element) {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                if (!isset($mapping['mapping'])) {
                    if (\is_numeric($source)) {
                        $source = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($source);
                    }
                    ...
                    $value = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($value);
            ...
            case self::TYPE_ANY:
                ...
                switch (\true) {
                    ...
                    case \is_int($source):
                    case $source instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger:
    ...
    public static function decodeOID($content)
    {
        static $eighty;
        if (!$eighty) {
            $eighty = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(80);
        }
        $oid = [];
        $pos = 0;
        $len = \strlen($content);
        ...
        $n = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger();
        while ($pos < $len) {
            $temp = \ord($content[$pos++]);
            $n = $n->bitwise_leftShift(7);
            $n = $n->bitwise_or(new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($temp & 0x7f));
            if (~$temp & 0x80) {
                $oid[] = $n;
                $n = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger();
            }
    ...
    public static function encodeOID($source)
    {
        ...
        if (!$mask) {
            $mask = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(0x7f);
            $zero = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger();
            $forty = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(40);
        }
        ...
        $first = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($part1);
        $first = $first->multiply($forty);
        $first = $first->add(new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($part2));
        ...
        foreach ($parts as $part) {
            if (!$part) {
                ...
                $temp = '';
                $part = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($part);
