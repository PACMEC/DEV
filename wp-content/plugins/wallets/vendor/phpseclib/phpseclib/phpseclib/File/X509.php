 ...
 */
namespace Ethereumico\EthereumWallet\Dependencies\phpseclib3\File;

use DateTimeImmutable;
use DateTimeInterface;
use DateTimeZone;
use Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Base64;
use Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Hex;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PublicKey;
...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\PublicKeyLoader;
/**
 * Pure-PHP X.509 Parser
 *
 * @package X509
...
class X509
{
    ...
    public function __construct()
    {
        ...
        if (!self::$oidsLoaded) {
            ...
            \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::loadOIDs([
    ...
    public function loadX509($cert, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcert = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::extractBER($cert);
        ...
        $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($cert);
        if (!empty($decoded)) {
            $x509 = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Certificate::MAP);
        }
        ...
        $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($key, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\SubjectPublicKeyInfo::MAP);
    ...
    public function saveX509($cert, $format = self::FORMAT_PEM)
    {
        ...
        switch (\true) {
            ...
            case \is_object($cert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey']):
                break;
            default:
                $cert['tbsCertificate']['subjectPublicKeyInfo'] = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element(\base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $cert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey'])));
        }
        ...
        $filters = [];
        $type_utf8_string = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \phpseclib3\File\ASN1::TYPE_IA5_STRING.
             ...
             characters.
           */
        $filters['policyQualifiers']['qualifier'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_IA5_STRING];
        \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $cert = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($cert, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Certificate::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $cert;
            // case self::FORMAT_PEM:
            default:
                return "-----BEGIN CERTIFICATE-----\r\n" . \chunk_split(\Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Base64::encode($cert), 64) . '-----END CERTIFICATE-----';
        }
    }
    /**
        ...
        if ($extensions) {
            for ($i = 0; $i < \count($extensions); $i++) {
                ...
                if (!\is_bool($map)) {
                    ...
                    $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($value);
                    $mapped = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], $map, ['iPAddress' => $decoder]);
                    ...
                    if ($id == 'id-ce-certificatePolicies') {
                        for ($j = 0; $j < \count($value); $j++) {
                            ...
                            for ($k = 0; $k < \count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($subvalue);
                                    $mapped = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
    ...
    private function mapOutExtensions(&$root, $path)
    {
        ...
        if (\is_array($extensions)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                if ($extensions[$i] instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element) {
                    continue;
                }
                $id = $extensions[$i]['extnId'];
                $value =& $extensions[$i]['extnValue'];
                        ...
                        for ($j = 0; $j < \count($value); $j++) {
                            ...
                            for ($k = 0; $k < \count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    ...
                                    $subvalue = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($subvalue, $map));
                                }
                            }
                        }
                        break;
                    case 'id-ce-authorityKeyIdentifier':
                        ...
                        if (isset($value['authorityCertSerialNumber'])) {
                            if ($value['authorityCertSerialNumber']->toBytes() == '') {
                                $temp = \chr(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::CLASS_CONTEXT_SPECIFIC << 6 | 2) . "\1\0";
                                $value['authorityCertSerialNumber'] = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element($temp);
                            }
                        }
                }
                /* [extnValue] contains the DER encoding of an ASN.1 value
                ...
                if (\is_bool($map)) {
                    ...
                    $value = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($value, $map, ['iPAddress' => [static::class, 'encodeIP']]);
                }
            }
        }
    }
            ...
            for ($i = 0; $i < \count($attributes); $i++) {
                ...
                if (\is_array($attributes[$i]['value'])) {
                    ...
                    for ($j = 0; $j < \count($values); $j++) {
                        $value = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($values[$j], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\AttributeValue::MAP);
                        $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($value);
                        if (!\is_bool($map)) {
                            $mapped = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                            if ($mapped !== \false) {
            ...
            for ($i = 0; $i < $size; $i++) {
                ...
                if ($map === \false) {
                    ...
                    for ($j = 0; $j < \count($values); $j++) {
                        ...
                        if (!\is_bool($map)) {
                            $temp = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($values[$j], $map);
                            $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($temp);
                            $values[$j] = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\AttributeValue::MAP);
                        }
                    }
                }
            }
        ...
        if (\is_array($dns)) {
            for ($i = 0; $i < \count($dns); $i++) {
                for ($j = 0; $j < \count($dns[$i]); $j++) {
                    $type = $dns[$i][$j]['type'];
                    $value =& $dns[$i][$j]['value'];
                    if (\is_object($value) && $value instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element) {
                        $map = $this->getMapping($type);
                        if (!\is_bool($map)) {
                            $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($value);
                            $value = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
            }
        ...
        if (\is_array($dns)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                for ($j = 0; $j < \count($dns[$i]); $j++) {
                    ...
                    $map = $this->getMapping($type);
                    if (!\is_bool($map)) {
                        $value = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($value, $map));
                    }
                }
            }
        }
    ...
    private function getMapping($extnId)
    {
        ...
        switch ($extnId) {
            case 'id-ce-keyUsage':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\KeyUsage::MAP;
            case 'id-ce-basicConstraints':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\BasicConstraints::MAP;
            case 'id-ce-subjectKeyIdentifier':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\KeyIdentifier::MAP;
            case 'id-ce-cRLDistributionPoints':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CRLDistributionPoints::MAP;
            case 'id-ce-authorityKeyIdentifier':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\AuthorityKeyIdentifier::MAP;
            case 'id-ce-certificatePolicies':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CertificatePolicies::MAP;
            case 'id-ce-extKeyUsage':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\ExtKeyUsageSyntax::MAP;
            case 'id-pe-authorityInfoAccess':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\AuthorityInfoAccessSyntax::MAP;
            case 'id-ce-subjectAltName':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\SubjectAltName::MAP;
            case 'id-ce-subjectDirectoryAttributes':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\SubjectDirectoryAttributes::MAP;
            case 'id-ce-privateKeyUsagePeriod':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\PrivateKeyUsagePeriod::MAP;
            case 'id-ce-issuerAltName':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\IssuerAltName::MAP;
            case 'id-ce-policyMappings':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\PolicyMappings::MAP;
            case 'id-ce-nameConstraints':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\NameConstraints::MAP;
            case 'netscape-cert-type':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\netscape_cert_type::MAP;
            ...
            case 'id-qt-unotice':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\UserNotice::MAP;
            ...
            case 'pkcs-9-at-unstructuredName':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\PKCS9String::MAP;
            case 'pkcs-9-at-challengePassword':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\DirectoryString::MAP;
            case 'pkcs-9-at-extensionRequest':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Extensions::MAP;
            // CRL extensions.
            case 'id-ce-cRLNumber':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CRLNumber::MAP;
            case 'id-ce-deltaCRLIndicator':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CRLNumber::MAP;
            case 'id-ce-issuingDistributionPoint':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\IssuingDistributionPoint::MAP;
            case 'id-ce-freshestCRL':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CRLDistributionPoints::MAP;
            case 'id-ce-cRLReasons':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CRLReason::MAP;
            case 'id-ce-invalidityDate':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\InvalidityDate::MAP;
            case 'id-ce-certificateIssuer':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CertificateIssuer::MAP;
            case 'id-ce-holdInstructionCode':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\HoldInstructionCode::MAP;
            case 'id-at-postalAddress':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\PostalAddress::MAP;
        }
        return \false;
    }
    /**
    ...
    private function validateSignatureHelper($publicKeyAlgorithm, $publicKey, $signatureAlgorithm, $signature, $signatureSubject)
    {
        switch ($publicKeyAlgorithm) {
            case 'id-RSASSA-PSS':
                $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::loadFormat('PSS', $publicKey);
                break;
            case 'rsaEncryption':
                $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::loadFormat('PKCS8', $publicKey);
                switch ($signatureAlgorithm) {
                    ...
                    case 'sha512WithRSAEncryption':
                        $key = $key->withHash(\preg_replace('#WithRSAEncryption$#', '', $signatureAlgorithm))->withPadding(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
                        break;
                    default:
                        throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('Signature algorithm unsupported');
                }
                break;
            case 'id-Ed25519':
            case 'id-Ed448':
                $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC::loadFormat('PKCS8', $publicKey);
                break;
            case 'id-ecPublicKey':
                $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC::loadFormat('PKCS8', $publicKey);
                switch ($signatureAlgorithm) {
                ...
                break;
            case 'id-dsa':
                $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\DSA::loadFormat('PKCS8', $publicKey);
                switch ($signatureAlgorithm) {
                ...
                break;
            default:
                throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('Public key algorithm unsupported');
        }
    ...
    public function getDNProp($propName, $dn = null, $withType = \false)
    {
        ...
        $filters = [];
        $filters['value'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::setFilters($filters);
        ...
        for ($i = 0; $i < \count($dn); $i++) {
            if ($dn[$i][0]['type'] == $propName) {
                ...
                if (!$withType) {
                    if (\is_array($v)) {
                        foreach ($v as $type => $s) {
                            $type = \array_search($type, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::ANY_MAP);
                            if ($type !== \false && \array_key_exists($type, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                $s = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::convert($s, $type);
                                if ($s !== \false) {
                                    $v = $s;
                                    break;
                                }
                    ...
                    } elseif (\is_object($v) && $v instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element) {
                        $map = $this->getMapping($propName);
                        if (!\is_bool($map)) {
                            $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($v);
                            $v = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
                $result[] = $v;
    ...
    public function getDN($format = self::DN_ARRAY, $dn = null)
    {
        ...
        switch ((int) $format) {
            ...
            case self::DN_ASN1:
                $filters = [];
                $filters['rdnSequence']['value'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
                \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::setFilters($filters);
                $this->mapOutDNs($dn, 'rdnSequence');
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($dn, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Name::MAP);
            case self::DN_CANON:
                ...
                foreach ($dn['rdnSequence'] as $rdn) {
                    foreach ($rdn as $i => $attr) {
                        ...
                        if (\is_array($attr['value'])) {
                            foreach ($attr['value'] as $type => $v) {
                                $type = \array_search($type, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::ANY_MAP, \true);
                                if ($type !== \false && \array_key_exists($type, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                    $v = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::convert($v, $type);
                                    if ($v !== \false) {
                                        $v = \preg_replace('/\\s+/', ' ', $v);
                    ...
                    $result .= \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($rdn, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\RelativeDistinguishedName::MAP);
                }
                return $result;
            case self::DN_HASH:
                $dn = $this->getDN(self::DN_CANON, $dn);
                $hash = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash('sha1');
                $hash = $hash->hash($dn);
                \extract(\unpack('Vhash', $hash));
                return \strtolower(\Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Hex::encode(\pack('N', $hash)));
        }
        ...
        foreach ($dn['rdnSequence'] as $field) {
            ...
            } elseif (\is_object($value) && $value instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element) {
                $callback = function ($x) {
    ...
    public function getChain()
    {
        ...
        foreach ($chain as $key => $value) {
            $chain[$key] = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\X509();
     ...
     * @return bool
     */
    public function setPublicKey(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PublicKey $key)
    {
        $this->publicKey = $key;
    }
    /**
     ...
     * @access public
     */
    public function setPrivateKey(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey $key)
    {
        $this->privateKey = $key;
    }
    /**
    ...
    public function getPublicKey()
    {
        ...
        switch ($keyinfo['algorithm']['algorithm']) {
            case 'id-RSASSA-PSS':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::loadFormat('PSS', $key);
            case 'rsaEncryption':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::loadFormat('PKCS8', $key)->withPadding(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
            case 'id-ecPublicKey':
            case 'id-Ed25519':
            case 'id-Ed448':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC::loadFormat('PKCS8', $key);
            case 'id-dsa':
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\DSA::loadFormat('PKCS8', $key);
        }
        return \false;
    }
    /**
    ...
    public function loadCSR($csr, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcsr = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::extractBER($csr);
        ...
        $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($csr);
        if (empty($decoded)) {
            $this->currentCert = \false;
            return \false;
        }
        $csr = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CertificationRequest::MAP);
    ...
    public function saveCSR($csr, $format = self::FORMAT_PEM)
    {
        ...
        switch (\true) {
            ...
            case \is_object($csr['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey']):
                break;
            default:
                $csr['certificationRequestInfo']['subjectPKInfo'] = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element(\base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $csr['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey'])));
        }
        $filters = [];
        $filters['certificationRequestInfo']['subject']['rdnSequence']['value'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $csr = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($csr, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CertificationRequest::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $csr;
            // case self::FORMAT_PEM:
            default:
                return "-----BEGIN CERTIFICATE REQUEST-----\r\n" . \chunk_split(\Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Base64::encode($csr), 64) . '-----END CERTIFICATE REQUEST-----';
        }
    }
    /**
     * Load a SPKAC CSR
    ...
    public function loadSPKAC($spkac)
    {
        ...
        $temp = \preg_match('#^[a-zA-Z\\d/+]*={0,2}$#', $temp) ? \Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Base64::decode($temp) : \false;
        if ($temp != \false) {
            $spkac = $temp;
        }
        $orig = $spkac;
        ...
        $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($spkac);
        if (empty($decoded)) {
            $this->currentCert = \false;
            return \false;
        }
        $spkac = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\SignedPublicKeyAndChallenge::MAP);
    ...
    public function saveSPKAC($spkac, $format = self::FORMAT_PEM)
    {
        ...
        switch (\true) {
            ...
            case \is_object($spkac['publicKeyAndChallenge']['spki']['subjectPublicKey']):
                break;
            default:
                $spkac['publicKeyAndChallenge']['spki'] = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element(\base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $spkac['publicKeyAndChallenge']['spki']['subjectPublicKey'])));
        }
        $spkac = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($spkac, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\SignedPublicKeyAndChallenge::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $spkac;
            // case self::FORMAT_PEM:
            default:
                ...
                return 'SPKAC=' . \Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Base64::encode($spkac);
        }
    }
    /**
    ...
    public function loadCRL($crl, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcrl = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::extractBER($crl);
        ...
        $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($crl);
        if (empty($decoded)) {
            $this->currentCert = \false;
            return \false;
        }
        $crl = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CertificateList::MAP);
    ...
    public function saveCRL($crl, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $filters['tbsCertList']['issuer']['rdnSequence']['value'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['tbsCertList']['signature']['parameters'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['signatureAlgorithm']['parameters'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        if (empty($crl['tbsCertList']['signature']['parameters'])) {
            $filters['tbsCertList']['signature']['parameters'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_NULL];
        }
        if (empty($crl['signatureAlgorithm']['parameters'])) {
            $filters['signatureAlgorithm']['parameters'] = ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_NULL];
        }
        \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $crl = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($crl, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\CertificateList::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $crl;
            // case self::FORMAT_PEM:
            default:
                return "-----BEGIN X509 CRL-----\r\n" . \chunk_split(\Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Base64::encode($crl), 64) . '-----END X509 CRL-----';
        }
    }
    /**
    ...
    {
        if ($date instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element) {
            return $date;
        }
    ...
    public function sign($issuer, $subject)
    {
        ...
        if ($signatureAlgorithm != 'id-RSASSA-PSS') {
            ...
            $r = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\Formats\Keys\PSS::load($issuer->privateKey->withPassword()->toString('PSS'));
            $signatureAlgorithm = ['algorithm' => 'id-RSASSA-PSS', 'parameters' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\Formats\Keys\PSS::savePSSParams($r)];
        }
        if (isset($subject->currentCert) && \is_array($subject->currentCert) && isset($subject->currentCert['tbsCertificate'])) {
            ...
            /* "The serial number MUST be a positive integer"
                        ...
                        */
            $serialNumber = !empty($this->serialNumber) ? $this->serialNumber : new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Random::string(20) & "" . \str_repeat("ÿ", 19), 256);
    ...
    public function signCRL($issuer, $crl)
    {
        ...
        if (!empty($this->serialNumber)) {
            ...
            $crlNumber = $crlNumber !== \false ? $crlNumber->add(new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(1)) : null;
        }
        $this->removeExtension('id-ce-authorityKeyIdentifier');
     ...
     * @return string
     */
    private static function identifySignatureAlgorithm(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey $key)
    {
        if ($key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA) {
            if ($key->getPadding() & \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PSS) {
            ...
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms for RSA are: md2, md5, sha1, sha224, sha256, sha384, sha512');
        }
        if ($key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\DSA) {
            switch ($key->getHash()) {
                case 'sha1':
                case 'sha224':
                case 'sha256':
            ...
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms for DSA are: sha1, sha224, sha256');
        }
        if ($key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC) {
            switch ($key->getCurve()) {
                case 'Ed25519':
                case 'Ed448':
            ...
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms for EC are: sha1, sha224, sha256, sha384, sha512');
        }
        throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported public key classes are: RSA, DSA, EC');
    }
    /**
    ...
    public function setEndDate($date)
    {
        ...
        */
        if (\is_string($date) && \strtolower($date) === 'lifetime') {
            ...
            $temp = \chr(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_GENERALIZED_TIME) . \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeLength(\strlen($temp)) . $temp;
            $this->endDate = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element($temp);
        } else {
            if (!\is_object($date) || !$date instanceof \DateTimeInterface) {
                $date = new \DateTimeImmutable($date, new \DateTimeZone(@\date_default_timezone_get()));
            }
    ...
    {
        $this->serialNumber = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($serial, $base);
    }
    /**
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (\true) {
            ...
            case !\is_object($key):
                return \false;
            case $key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element:
                // Assume the element is a bitstring-packed key.
                $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($key->element);
                if (empty($decoded)) {
                    return \false;
                }
                $raw = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], ['type' => \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::TYPE_BIT_STRING]);
                if (empty($raw)) {
                    return \false;
                }
                // If the key is private, compute identifier from its corresponding public key.
                $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\PublicKeyLoader::load($raw);
                if ($key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey) {
                    // If private.
                ...
                // Is a public key.
                break;
            case $key instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\X509:
        ...
        $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::extractBER($key);
    ...
    {
        $format = $this->publicKey instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA && $this->publicKey->getPadding() & \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PSS ? 'PSS' : 'PKCS8';
        $publicKey = \base64_decode(\preg_replace('#-.+-|[\\r\\n]#', '', $this->publicKey->toString($format)));
        $decoded = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($publicKey);
        $mapped = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\SubjectPublicKeyInfo::MAP);
    ...
    {
        $serial = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($serial);
