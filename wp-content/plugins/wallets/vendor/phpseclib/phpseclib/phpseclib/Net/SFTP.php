...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\FileNotFoundException;
 ...
 * @access  public
 */
class SFTP extends \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Net\SSH2
{
    /**
    ...
    public function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        if (!\defined('Ethereumico\\EthereumWallet\\Dependencies\\NET_SFTP_QUEUE_SIZE')) {
            \define('Ethereumico\\EthereumWallet\\Dependencies\\NET_SFTP_QUEUE_SIZE', 32);
        }
        if (!\defined('Ethereumico\\EthereumWallet\\Dependencies\\NET_SFTP_UPLOAD_QUEUE_SIZE')) {
            \define('Ethereumico\\EthereumWallet\\Dependencies\\NET_SFTP_UPLOAD_QUEUE_SIZE', 1024);
        }
    }
    /**
    ...
    {
        if (!($this->bitmap & \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Net\SSH2::MASK_LOGIN)) {
            return \false;
        }
        if ($this->pwd === \false) {
    ...
    private function partial_init_sftp_connection()
    {
        ...
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CsN3', NET_SSH2_MSG_CHANNEL_OPEN, 'session', self::CHANNEL, $this->window_size, 0x4000);
        ...
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsbs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL], 'subsystem', \true, 'sftp');
        $this->send_binary_packet($packet);
        ...
        if ($response === \false) {
            ...
            // is redundant
            $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsCs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL], 'exec', 1, $command);
            $this->send_binary_packet($packet);
            $this->channel_status[self::CHANNEL] = NET_SSH2_MSG_CHANNEL_REQUEST;
            $response = $this->get_channel_packet(self::CHANNEL, \true);
            if ($response === \false) {
        ...
        list($this->defaultVersion) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        while (!empty($response)) {
            list($key, $value) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $response);
        ...
        if (isset($this->extensions['versions']) && (!$this->preferredVersion || $this->preferredVersion != $this->version)) {
            ...
            foreach ($supported as $ver) {
                if (\in_array($ver, $versions)) {
                    ...
                    $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', 'version-select', "{$ver}");
                    ...
                    list($status) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
    ...
    public function realpath($path)
    {
        ...
        if ($this->pwd === \true) {
            ...
            $this->send_sftp_packet(NET_SFTP_REALPATH, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $path));
            $response = $this->get_sftp_packet();
            switch ($this->packet_type) {
                case NET_SFTP_NAME:
                    ...
                    list(, $filename) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Ns', $response);
                    return $filename;
    ...
    public function chdir($dir)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_OPENDIR, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $dir));
    ...
    private function readlist($dir, $raw = \true)
    {
        ...
        while (\true) {
            ...
            $this->send_sftp_packet(NET_SFTP_READDIR, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $handle));
            $response = $this->get_sftp_packet();
            switch ($this->packet_type) {
                case NET_SFTP_NAME:
                    list($count) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
                    for ($i = 0; $i < $count; $i++) {
                        list($shortname) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                        // SFTPv4 "removed the long filename from the names structure-- it can now be
                        //         built from information available in the attrs structure."
                        if ($this->version < 4) {
                            list($longname) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                        }
    ...
    private function stat_helper($filename, $type)
    {
        ...
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $filename);
    ...
    {
        $attr = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('NQ', NET_SFTP_ATTR_SIZE, $new_size);
    ...
    public function touch($filename, $time = null, $atime = null)
    {
        ...
        $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_ACCESSTIME, $atime, $time) : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('NQ2', NET_SFTP_ATTR_ACCESSTIME | NET_SFTP_ATTR_MODIFYTIME, $atime, $time);
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $filename);
    ...
    public function chown($filename, $uid, $recursive = \false)
    {
        ...
        */
        $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_UIDGID, $uid, -1) : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Nss', NET_SFTP_ATTR_OWNERGROUP, $uid, '');
    ...
    {
        $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_UIDGID, $gid, -1) : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Nss', NET_SFTP_ATTR_OWNERGROUP, '', $gid);
        return $this->setstat($filename, $attr, $recursive);
    }
    /**
    ...
    private function setstat_recursive($path, $attr, &$i)
    {
        ...
        foreach ($entries as $filename => $props) {
            ...
            if ($props['type'] == NET_SFTP_TYPE_DIRECTORY) {
                ...
                $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $temp);
        ...
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $path);
        $packet .= $this->version >= 4 ? \pack('Ca*', NET_SFTP_TYPE_UNKNOWN, $attr) : $atr;
        $this->send_sftp_packet(NET_SFTP_SETSTAT, $packet);
        $i++;
        if ($i >= NET_SFTP_QUEUE_SIZE) {
            if (!$this->read_put_responses($i)) {
    ...
    public function readlink($link)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_READLINK, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $link));
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
            case NET_SFTP_NAME:
                break;
        ...
        list($filename) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
        return $filename;
    }
    /**
     * Create a symlink
    ...
    public function symlink($target, $link)
    {
        ...
        if ($this->version == 6) {
            $type = NET_SFTP_LINK;
            $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ssC', $link, $target, 1);
        } else {
            ...
            /* quoting http://bxr.su/OpenBSD/usr.bin/ssh/PROTOCOL#347 :
                           ...
                           SSH_FXP_SYMLINK as follows:
                               ...
                               string      targetpath
                               string      linkpath */
            $packet = \substr($this->server_identifier, 0, 15) == 'SSH-2.0-OpenSSH' ? \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $target, $link) : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $link, $target);
        }
        $this->send_sftp_packet($type, $packet);
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
    ...
    private function mkdir_helper($dir, $mode)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_MKDIR, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $dir) . "\0\0\0\0");
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
            throw new \UnexpectedValueException('Expected NET_SFTP_STATUS. ' . 'Got packet type: ' . $this->packet_type);
        }
    ...
    public function rmdir($dir)
    {
        ...
        $this->send_sftp_packet(NET_SFTP_RMDIR, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $dir));
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
            throw new \UnexpectedValueException('Expected NET_SFTP_STATUS. ' . 'Got packet type: ' . $this->packet_type);
        }
    ...
    public function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null)
    {
        ...
        $this->remove_from_stat_cache($remote_file);
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $remote_file);
        ...
        switch (\true) {
            ...
            case $mode & self::SOURCE_LOCAL_FILE:
                if (!\is_file($data)) {
                    throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\FileNotFoundException("{$data} is not a valid file");
                }
                $fp = @\fopen($data, 'rb');
                if (!$fp) {
                    return \false;
        ...
        if ($mode & \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Net\SFTP::SOURCE_LOCAL_FILE) {
            if (isset($fp) && \is_resource($fp)) {
                \fclose($fp);
            }
            if ($this->preserveTime) {
                $stat = \stat($data);
                $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_ACCESSTIME, $stat['atime'], $stat['time']) : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('NQ2', NET_SFTP_ATTR_ACCESSTIME | NET_SFTP_ATTR_MODIFYTIME, $stat['atime'], $stat['time']);
    ...
    public function get($remote_file, $local_file = \false, $offset = 0, $length = -1, $progressCallback = null)
    {
        ...
        while (\true) {
            ...
            while ($i < NET_SFTP_QUEUE_SIZE && ($length < 0 || $read < $length)) {
                ...
                $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sN3', $handle, $tempoffset / 4294967296, $tempoffset, $packet_size);
                try {
    ...
    private function delete_recursive($path, &$i)
    {
        ...
        foreach ($entries as $filename => $props) {
            ...
            if ($props['type'] == NET_SFTP_TYPE_DIRECTORY) {
                ...
                $this->send_sftp_packet(NET_SFTP_REMOVE, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $temp));
        ...
        $this->send_sftp_packet(NET_SFTP_RMDIR, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $path));
        $this->remove_from_stat_cache($path);
        $i++;
        if ($i >= NET_SFTP_QUEUE_SIZE) {
            if (!$this->read_put_responses($i)) {
    ...
    public function is_readable($path)
    {
        ...
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sNN', $this->realpath($path), NET_SFTP_OPEN_READ, 0);
        $this->send_sftp_packet(NET_SFTP_OPEN, $packet);
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
            case NET_SFTP_HANDLE:
    ...
    public function is_writable($path)
    {
        ...
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sNN', $this->realpath($path), NET_SFTP_OPEN_WRITE, 0);
        $this->send_sftp_packet(NET_SFTP_OPEN, $packet);
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
            case NET_SFTP_HANDLE:
    ...
    public function rename($oldname, $newname)
    {
        ...
        // http://tools.ietf.org/html/draft-ietf-secsh-filexfer-13#section-8.3
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $oldname, $newname);
        if ($this->version >= 5) {
    ...
    {
        $attr = [];
        list($attr[$key]) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Q', $response);
        if ($flags & NET_SFTP_ATTR_SUBSECOND_TIMES) {
            list($attr[key . '-nseconds']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        }
        return $attr;
    }
    /**
    ...
    {
        if ($this->version >= 4) {
            list($flags, $attr['type']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NC', $response);
        } else {
            list($flags) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        }
        foreach ($this->attributes as $key => $value) {
            ...
            switch ($flags & $key) {
                case NET_SFTP_ATTR_SIZE:
                    ...
                    list($attr['size']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Q', $response);
                    break;
                case NET_SFTP_ATTR_UIDGID:
                    // 0x00000002 (SFTPv3 only)
                    list($attr['uid'], $attr['gid']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $response);
                    break;
                case NET_SFTP_ATTR_PERMISSIONS:
                    // 0x00000004
                    list($attr['mode']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
                ...
                case NET_SFTP_ATTR_ACCESSTIME:
                    ...
                    list($attr['atime'], $attr['mtime']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $response);
                    break;
                case NET_SFTP_ATTR_CREATETIME:
                    // 0x00000010 (SFTPv4+)
                ...
                case NET_SFTP_ATTR_ACL:
                    ...
                    list($count) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
                    for ($i = 0; $i < $count; $i++) {
                        list($type, $flag, $mask, $who) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N3s', $result);
                    }
                    break;
                case NET_SFTP_ATTR_OWNERGROUP:
                    // 0x00000080
                    list($attr['owner'], $attr['$group']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $response);
                    break;
                case NET_SFTP_ATTR_SUBSECOND_TIMES:
                    // 0x00000100
                    break;
                case NET_SFTP_ATTR_BITS:
                    ...
                    list($attrib_bits, $attrib_bits_valid) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N2', $response);
                ...
                case NET_SFTP_ATTR_ALLOCATION_SIZE:
                    ...
                    list($attr['allocation-size']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpack('Q', $response);
                    break;
                case NET_SFTP_ATTR_TEXT_HINT:
                    // 0x00000800
                    ...
                    list($text_hint) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('C', $response);
                    // the above should be $attr['text-hint']
                    break;
                case NET_SFTP_ATTR_MIME_TYPE:
                    // 0x00001000
                    // see https://datatracker.ietf.org/doc/html/draft-ietf-secsh-filexfer-13#section-7.11
                    list($attr['mime-type']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                    break;
                case NET_SFTP_ATTR_LINK_COUNT:
                    // 0x00002000
                    // see https://datatracker.ietf.org/doc/html/draft-ietf-secsh-filexfer-13#section-7.12
                    list($attr['link-count']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSS2('N', $response);
                    break;
                case NET_SFTP_ATTR_UNTRANSLATED_NAME:
                    // 0x00004000
                    // see https://datatracker.ietf.org/doc/html/draft-ietf-secsh-filexfer-13#section-7.13
                    list($attr['untranslated-name']) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                    break;
                case NET_SFTP_ATTR_CTIME:
                    // 0x00008000
    ...
    private function send_sftp_packet($type, $data, $request_id = 1)
    {
        ...
        $stop = \microtime(\true);
        if (\defined('Ethereumico\\EthereumWallet\\Dependencies\\NET_SFTP_LOGGING')) {
    ...
    private function get_sftp_packet($request_id = null)
    {
        ...
        \extract(\unpack('Nlength', \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, 4)));
        /** @var integer $length */
        $tempLength = $length;
        ...
        $stop = \microtime(\true);
        $this->packet_type = \ord(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer));
        if ($this->use_request_id) {
            \extract(\unpack('Npacket_id', \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, 4)));
        ...
        $packet = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, $length);
        if (\defined('Ethereumico\\EthereumWallet\\Dependencies\\NET_SFTP_LOGGING')) {
