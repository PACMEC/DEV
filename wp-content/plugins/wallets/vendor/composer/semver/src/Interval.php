...
class Interval
{
    ...
    /** @var Constraint */
    private $end;
    public function __construct(\Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint $start, \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint $end)
    {
        $this->start = $start;
        $this->end = $end;
    }
    ...
    public static function fromZero()
    {
        static $zero;
        if (null === $zero) {
            $zero = new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('>=', '0.0.0.0-dev');
        }
        return $zero;
    }
    /**
    ...
    public static function untilPositiveInfinity()
    {
        ...
        if (null === $positiveInfinity) {
            $positiveInfinity = new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('<', \PHP_INT_MAX . '.0.0.0');
        }
