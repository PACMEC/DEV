    ...
    public function parseConstraints($constraints)
    {
        ...
        foreach ($orConstraints as $constraints) {
            ...
            if (1 === \count($constraintObjects)) {
                ...
                $constraint = new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\MultiConstraint($constraintObjects);
            }
            $orGroups[] = $constraint;
        }
        $constraint = \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\MultiConstraint::create($orGroups, \false);
    ...
    private function parseConstraint($constraint)
    {
        ...
        if (\preg_match('{^(v)?[xX*](\\.[xX*])*$}i', $constraint, $match)) {
            if (!empty($match[1]) || !empty($match[2])) {
                return array(new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('>=', '0.0.0.0-dev'));
            }
            return array(new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\MatchAllConstraint());
        }
        ...
        if (\preg_match('{^~>?' . $versionRegex . '$}i', $constraint, $matches)) {
            ...
            $lowerBound = new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('>=', $lowVersion);
            // For upper bound, we increment the position of one more significance,
            // but highPosition = 0 would be illegal
            $highPosition = \max(1, $position - 1);
            $highVersion = $this->manipulateVersionString($matches, $highPosition, 1) . '-dev';
            $upperBound = new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('<', $highVersion);
        ...
        if (\preg_match('{^v?(\\d++)(?:\\.(\\d++))?(?:\\.(\\d++))?(?:\\.[xX*])++$}', $constraint, $matches)) {
            ...
            if ($lowVersion === '0.0.0.0-dev') {
                return array(new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('<', $highVersion));
            }
            return array(new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('>=', $lowVersion), new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('<', $highVersion));
        }
        // Hyphen Range
        //
        ...
        if (\preg_match('{^(?P<from>' . $versionRegex . ') +- +(?P<to>' . $versionRegex . ')($)}i', $constraint, $matches)) {
            ...
            $lowerBound = new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint('>=', $lowVersion . $lowStabilitySuffix);
            $empty = function ($x) {
        ...
        if (\preg_match('{^(<>|!=|>=?|<=?|==?)?\\s*(.*)}', $constraint, $matches)) {
            try {
                ...
                return array(new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\Constraint($matches[1] ?: '=', $version));
            } catch (\Exception $e) {
            }
        }
