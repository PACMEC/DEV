 ...
 */
class MultiConstraint implements \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\ConstraintInterface
{
    /**
    ...
    public function matches(\Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\ConstraintInterface $provider)
    {
        ...
        if ($provider instanceof \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\MultiConstraint && $provider->isDisjunctive()) {
        ...
        if (!$conjunctive) {
            ...
            for ($i = 1, $l = \count($constraints); $i < $l; $i++) {
                ...
                if ($left instanceof self && $left->conjunctive && $right instanceof self && $right->conjunctive && \count($left->constraints) === 2 && \count($right->constraints) === 2 && ($left0 = (string) $left->constraints[0]) && $left0[0] === '>' && $left0[1] === '=' && ($left1 = (string) $left->constraints[1]) && $left1[0] === '<' && ($right0 = (string) $right->constraints[0]) && $right0[0] === '>' && $right0[1] === '=' && ($right1 = (string) $right->constraints[1]) && $right1[0] === '<' && \substr($left1, 2) === \substr($right0, 3)) {
                    $optimized = \true;
                    $left = new \Ethereumico\EthereumWallet\Dependencies\Composer\Semver\Constraint\MultiConstraint(array($left->constraints[0], $right->constraints[1]), \true);
                } else {
