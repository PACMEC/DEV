    ...
    public function __construct($txData = [])
    {
        ...
        if (\is_array($txData)) {
            if (isset($txData['maxPriorityFeePerGas']) || isset($txData['maxFeePerGas'])) {
                $this->txImpl = new \Ethereumico\EthereumWallet\Dependencies\Web3p\EthereumTx\EIP1559Transaction($txData);
            } else {
                if (isset($txData['accessList'])) {
                    $this->txImpl = new \Ethereumico\EthereumWallet\Dependencies\Web3p\EthereumTx\EIP2930Transaction($txData);
                } else {
                    $this->txImpl = new \Ethereumico\EthereumWallet\Dependencies\Web3p\EthereumTx\TransactionLegacy($txData);
                }
            }
