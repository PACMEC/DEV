        ...
        foreach ($value['topics'] as $topic) {
            if (\is_array($topic)) {
                foreach ($topic as $subTopic) {
                    if (\Ethereumico\EthereumWallet\Dependencies\Web3\Validators\HexValidator::validate($subTopic) === \false) {
                        return \false;
                    }
                }
                continue;
            }
            if (\Ethereumico\EthereumWallet\Dependencies\Web3\Validators\HexValidator::validate($topic) === \false) {
                if (!\is_null($topic)) {
                    return \false;
                }
            }
