 ...
 * @license MIT
 */
namespace Ethereumico\EthereumWallet\Dependencies\Web3\Contracts;
...
class SolidityType
{
    ...
    {
        if (!\is_string($name)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\InvalidArgumentException('nestedTypes name must string.');
        }
        $matches = [];
    ...
    {
        if (!\is_string($name)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\InvalidArgumentException('nestedName name must string.');
        }
    ...
    public function encode($value, $name)
    {
        if ($this->isDynamicArray($name)) {
            ...
            $result = [];
            $result[] = \Ethereumico\EthereumWallet\Dependencies\Web3\Formatters\IntegerFormatter::format($length);
    ...
    {
        if ($this->isDynamicArray($name)) {
            $arrayOffset = (int) \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::toBn('0x' . \mb_substr($value, (int) ($offset * 2), 64))->toString();
            $length = (int) \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::toBn('0x' . \mb_substr($value, (int) ($arrayOffset * 2), 64))->toString();
            ...
            $dynamicOffset = (int) \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::toBn('0x' . \mb_substr($value, (int) ($offset * 2), 64))->toString();
            $length = (int) \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::toBn('0x' . \mb_substr($value, (int) ($dynamicOffset * 2), 64))->toString();
