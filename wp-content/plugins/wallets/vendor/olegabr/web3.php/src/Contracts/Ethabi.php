...
class Ethabi
{
    ...
    public function encodeFunctionSignature($functionName)
    {
        if (!\is_string($functionName)) {
            $functionName = \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::jsonMethodToString($functionName);
        }
        return \mb_substr(\Ethereumico\EthereumWallet\Dependencies\Web3\Utils::sha3($functionName), 0, 10);
    }
    /**
    ...
    public function encodeEventSignature($functionName)
    {
        ...
        return \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::sha3($functionName);
    }
    /**
     * encodeParameter
     * 
    ...
    public function encodeParameters($types, $params)
    {
        ...
        if ($types instanceof \stdClass && isset($types->inputs)) {
            $types = \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::jsonToArray($types, 2);
        }
    ...
    public function decodeParameters($types, $param)
    {
        ...
        $result = [];
        $param = \mb_strtolower(\Ethereumico\EthereumWallet\Dependencies\Web3\Utils::stripZero($param));
        for ($i = 0; $i < $typesLength; $i++) {
        ...
        if ($solidityType->isDynamicArray($type)) {
            ...
            if ($solidityType->isDynamicArray($nestedName)) {
                ...
                for ($i = 0; $i < \count($encoded); $i++) {
                    ...
                    $result .= \Ethereumico\EthereumWallet\Dependencies\Web3\Formatters\IntegerFormatter::format($offset + $i * $nestedStaticPartLength + $previousLength * 32);
                }
            }
            for ($i = 0; $i < \count($encoded); $i++) {
    ...
    protected function encodeMultiWithOffset($types, $solidityTypes, $encodes, $dynamicOffset)
    {
        ...
        foreach ($solidityTypes as $key => $type) {
            if ($type->isDynamicType($types[$key]) || $type->isDynamicArray($types[$key])) {
                $result .= \Ethereumico\EthereumWallet\Dependencies\Web3\Formatters\IntegerFormatter::format($dynamicOffset);
