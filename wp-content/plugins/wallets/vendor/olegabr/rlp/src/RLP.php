 ...
 * @license MIT
 */
namespace Ethereumico\EthereumWallet\Dependencies\Web3p\RLP;

use InvalidArgumentException;
use RuntimeException;
use Ethereumico\EthereumWallet\Dependencies\Web3p\RLP\Types\Str;
use Ethereumico\EthereumWallet\Dependencies\Web3p\RLP\Types\Numeric;
/**
...
class RLP
{
    ...
    protected function decodeData($input)
    {
        ...
        if ($firstByteDec <= 0x7f) {
            return ['data' => $firstByte, 'remainder' => \mb_substr($input, 2)];
            ...
            return ['data' => $data, 'remainder' => \mb_substr($input, $length * 2)];
            ...
            return ['data' => $data, 'remainder' => \mb_substr($input, ($length + $llength) * 2)];
            ...
            return ['data' => $decoded, 'remainder' => \mb_substr($input, $length * 2)];
        } else {
    ...
    protected function encodeInput($input)
    {
        if (\is_string($input)) {
            if (\strpos($input, '0x') === 0) {
                return \Ethereumico\EthereumWallet\Dependencies\Web3p\RLP\Types\Str::encode($input, 'hex');
            }
            return \Ethereumico\EthereumWallet\Dependencies\Web3p\RLP\Types\Str::encode($input);
        } elseif (\is_numeric($input)) {
            return \Ethereumico\EthereumWallet\Dependencies\Web3p\RLP\Types\Numeric::encode($input);
        } elseif ($input === null) {
            return '';
        }
        throw new \InvalidArgumentException('The input type didn\'t support.');
