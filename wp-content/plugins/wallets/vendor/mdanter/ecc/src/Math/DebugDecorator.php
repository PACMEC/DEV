 ...
 */
class DebugDecorator implements \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\GmpMathInterface
{
    /**
     ...
     */
    public function __construct(\Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\GmpMathInterface $adapter, callable $callback = null)
    {
    ...
    private function call($func, $args)
    {
        $strArgs = \array_map(function ($arg) {
            if ($arg instanceof \GMP) {
                return \var_export($this->adapter->toString($arg), \true);
            } else {
        ...
        if ($res instanceof \GMP) {
            $this->write(' => ' . \var_export($this->adapter->toString($res), \true) . \PHP_EOL);
        } else {
            $this->write(' => ' . \var_export($res, \true) . \PHP_EOL);
        }
        return $res;
    }
    /**
     ...
     */
    public function getEcMath(\Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Primitives\GeneratorPoint $generatorPoint, $input)
    {
        $func = __METHOD__;
        $args = \func_get_args();
        return \call_user_func(array($this, 'call'), $func, $args);
