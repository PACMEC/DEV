...
namespace Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32;

use Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception;
 ...
 */
function convertBits(array $data, $inLen, $fromBits, $toBits, $pad = \true)
{
    $acc = 0;
    $bits = 0;
    $ret = [];
    ...
    for ($i = 0; $i < $inLen; $i++) {
        ...
        if ($value < 0 || $value >> $fromBits) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Invalid value for convert bits');
        }
    ...
    if ($pad) {
        ...
        if ($bits >= $fromBits || $acc << $toBits - $bits & $maxv) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Invalid data');
        }
    }
    return $ret;
}
...
function decodeRaw($sBech)
{
    $length = \strlen($sBech);
    if ($length < 8) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception("Bech32 string is too short");
    }
    ...
    for ($i = 0; $i < $length; $i++) {
        $x = $chars[$i];
        if ($x < 33 || $x > 126) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Out of range character in bech32 string');
        }
        if ($x >= 0x61 && $x <= 0x7a) {
            $haveLower = \true;
        }
    ...
    if ($haveUpper && $haveLower) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Data contains mixture of higher/lower case characters');
    }
    if ($positionOne === -1) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception("Missing separator character");
    }
    if ($positionOne < 1) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception("Empty HRP");
    }
    if ($positionOne + 7 > $length) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Too short checksum');
    }
    ...
    if (!verifyChecksum($hrp, $data)) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Invalid bech32 checksum');
    }
...
function decode($sBech)
{
    $length = \strlen($sBech);
    if ($length > 90) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Bech32 string cannot exceed 90 characters in length');
    }
...
function validateWitnessProgram($version, $program)
{
    if ($version < 0 || $version > 16) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception("Invalid witness version");
    }
    $sizeProgram = \strlen($program);
    if ($version === 0) {
        if ($sizeProgram !== 20 && $sizeProgram !== 32) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception("Invalid size for V0 witness program");
        }
    }
    if ($sizeProgram < 2 || $sizeProgram > 40) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception("Witness program size was out of valid range");
    }
}
/**
...
function encodeSegwit($hrp, $version, $program)
{
    ...
    $programBits = convertBits($programChars, \count($programChars), 8, 5, \true);
...
function decodeSegwit($hrp, $bech32)
{
    ...
    if ($hrpGot !== $hrp) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception('Invalid prefix for address');
    }
    $dataLen = \count($data);
    if ($dataLen === 0 || $dataLen > 65) {
        throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bech32\Exception\Bech32Exception("Invalid length for segwit address");
    }
