...
class Checksig
{
    ...
    public function __construct($info)
    {
        ...
        switch ($infoClass) {
            case \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptInfo\PayToPubkey::class:
                /** @var PayToPubkey $info */
                $this->scriptType = $info->getType();
                $this->requiredSigs = $info->getRequiredSigCount();
                $this->keyCount = 1;
                break;
            case \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptInfo\PayToPubkeyHash::class:
                /** @var PayToPubkeyHash $info */
                $this->scriptType = \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptType::P2PKH;
                $this->requiredSigs = $info->getRequiredSigCount();
                $this->keyCount = 1;
                break;
            case \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptInfo\Multisig::class:
                /** @var Multisig $info */
                $this->scriptType = \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptType::MULTISIG;
                $this->requiredSigs = $info->getRequiredSigCount();
    ...
    {
        if ($this->info instanceof \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptInfo\Multisig) {
            return $this->info->getKeyBuffers();
        } else {
            if ($this->info instanceof \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptInfo\PayToPubkey) {
     ...
     * @return $this
     */
    public function setSignature(int $idx, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Signature\TransactionSignatureInterface $signature)
    {
     ...
     * @return $this
     */
    public function setKey(int $idx, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Crypto\EcAdapter\Key\PublicKeyInterface $key = null)
    {
        if ($idx < 0 || $idx > $this->keyCount) {
     ...
     */
    public function serialize(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Serializer\Signature\TransactionSignatureSerializer $txSigSerializer, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Crypto\EcAdapter\Serializer\Key\PublicKeySerializerInterface $pubKeySerializer) : array
    {
        $outputType = $this->getType();
        $result = [];
        if (\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptType::P2PK === $outputType) {
            if (!$this->required) {
                $result[0] = new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer();
            } else {
            ...
            if (\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptType::P2PKH === $outputType) {
                ...
                if (\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptType::MULTISIG === $outputType) {
                    if (!$this->required) {
                        $result = \array_fill(0, 1 + $this->getRequiredSigs(), new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer());
                    } else {
                        $result[] = new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer();
