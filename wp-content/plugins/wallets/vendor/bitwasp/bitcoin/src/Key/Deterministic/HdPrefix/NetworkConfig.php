...
namespace Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Key\Deterministic\HdPrefix;
...
class NetworkConfig
{
     ...
     */
    public function __construct(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Network\NetworkInterface $network, array $prefixConfigList)
    {
        foreach ($prefixConfigList as $config) {
            if (!$config instanceof \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Key\Deterministic\HdPrefix\ScriptPrefix) {
     ...
     */
    private function setupConfig(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Key\Deterministic\HdPrefix\ScriptPrefix $config)
    {
     ...
     * @param ScriptPrefix $config
     */
    private function checkForOverwriting(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Key\Deterministic\HdPrefix\ScriptPrefix $config)
    {
     ...
     * @param string $prefix
     */
    private function rejectConflictPrefix(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Key\Deterministic\HdPrefix\ScriptPrefix $config, $prefix)
    {
        $conflict = $this->scriptPrefixMap[$prefix];
        throw new \RuntimeException(\sprintf("A BIP32 prefix for %s conflicts with the %s BIP32 prefix of %s", $config->getScriptDataFactory()->getScriptType(), $prefix === $config->getPublicPrefix() ? "public" : "private", $conflict->getScriptDataFactory()->getScriptType()));
    }
    /**
     * @param ScriptPrefix $config
     */
    private function rejectConflictScriptType(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Key\Deterministic\HdPrefix\ScriptPrefix $config)
    {
        throw new \RuntimeException(\sprintf("The script type %s has a conflict", $config->getScriptDataFactory()->getScriptType()));
    }
    /**
