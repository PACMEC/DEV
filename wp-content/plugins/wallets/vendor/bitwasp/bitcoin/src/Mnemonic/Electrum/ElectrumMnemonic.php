...
class ElectrumMnemonic implements \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Mnemonic\MnemonicInterface
{
    /**
     ...
     */
    public function __construct(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Crypto\EcAdapter\Adapter\EcAdapterInterface $ecAdapter, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Mnemonic\Electrum\ElectrumWordListInterface $wordList)
    {
    ...
    public function mnemonicToEntropy(string $mnemonic) : \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface
    {
        ...
        for ($i = 0; $i < $thirdWordCount; $i++) {
            ...
            $x = $math->add($index1, $math->add($math->mul($n, $math->mod($math->sub($index2, $index1), $n)), $math->mul($n, $math->mul($n, $math->mod($math->sub($index3, $index2), $n)))));
            $out .= \str_pad(\gmp_strval($x, 16), 8, '0', \STR_PAD_LEFT);
        }
        return \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer::hex($out);
    }
}
