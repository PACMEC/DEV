...
class Number extends \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Serializable
{
     ...
     * @param Math $math
     */
    public function __construct($number, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math\Math $math)
    {
        $this->number = $number;
        $this->math = $math;
    }
     ...
     * @return self
     */
    public static function int($number, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math\Math $math = null)
    {
        return new self($number, $math ?: \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Bitcoin::getMath());
    }
    /**
     * @param \GMP $number
     * @param Math|null $math
     * @return self
     */
    public static function gmp(\GMP $number, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math\Math $math = null)
    {
        return new self(\gmp_strval($number, 10), $math ?: \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Bitcoin::getMath());
    }
    /**
     ...
     * @return self
     */
    public static function buffer(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface $vch, $fRequireMinimal, $maxNumSize = self::MAX_NUM_SIZE, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math\Math $math = null)
    {
        $size = $vch->getSize();
        if ($size > $maxNumSize) {
        ...
        $math = $math ?: \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Bitcoin::getMath();
     ...
     * @return string
     */
    private function parseBuffer(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface $buffer) : string
    {
        $size = $buffer->getSize();
        if ($size === 0) {
            return '0';
     ...
     */
    private function serialize() : \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface
    {
        ...
        return new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer(\pack("C*", ...$result));
    }
    /**
     * @return BufferInterface
     */
    ...
    public function getInt() : int
    {
        if ($this->math->cmp(\gmp_init($this->number, 10), \gmp_init(self::MAX)) > 0) {
            ...
            if ($this->math->cmp(\gmp_init($this->number, 10), \gmp_init(self::MIN)) < 0) {
                return self::MIN;
            }
        }
