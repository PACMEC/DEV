...
namespace Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Bloom;
...
use Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Serializer\Bloom\BloomFilterSerializer;
...
class BloomFilter extends \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Serializable
{
     ...
     * @param int $flags
     */
    public function __construct(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math\Math $math, array $vFilter, int $numHashFuncs, int $nTweak, int $flags)
    {
        $this->math = $math;
     ...
     * @return BloomFilter
     */
    public static function create(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math\Math $math, int $nElements, float $nFpRate, int $nTweak, int $flags) : \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Bloom\BloomFilter
    {
    ...
    {
        return (int) \floor(\bcdiv(\min(\bcmul(\bcmul(\bcdiv('-1', (string) self::LN2SQUARED), (string) $nElements), (string) \log($fpRate)), \bcmul((string) self::MAX_FILTER_SIZE, '8')), '8'));
    }
    /**
    ...
    {
        return (int) \floor(\min(\bcmul(\bcdiv(\bcmul((string) $filterSize, '8'), (string) $nElements), (string) self::LN2), \bcmul((string) self::MAX_FILTER_SIZE, '8')));
    }
    /**
     * @param int $nHashNum
     ...
     * @return string
     */
    public function hash(int $nHashNum, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface $data) : string
    {
        $hash = \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Crypto\Hash::murmur3($data, $nHashNum * self::TWEAK_START + $this->nTweak & 0xffffffff)->getInt();
     ...
     * @return $this
     */
    public function insertData(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface $data)
    {
        if ($this->isFull()) {
            return $this;
        }
     ...
     * @return BloomFilter
     */
    public function insertOutPoint(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Transaction\OutPointInterface $outPoint) : \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Bloom\BloomFilter
    {
     ...
     * @return bool
     */
    public function containsData(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface $data) : bool
    {
        if ($this->isFull()) {
            return \true;
        }
     ...
     * @return bool
     */
    public function containsOutPoint(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Transaction\OutPointInterface $outPoint) : bool
    {
     ...
     * @return bool
     */
    public function isRelevantAndUpdate(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Transaction\TransactionInterface $tx) : bool
    {
        $this->updateEmptyFull();
        $found = \false;
        if ($this->isFull()) {
    ...
    {
        return (new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Serializer\Bloom\BloomFilterSerializer())->serialize($this);
    }
}
