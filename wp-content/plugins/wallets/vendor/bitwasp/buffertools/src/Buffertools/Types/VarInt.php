...
class VarInt extends \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Types\AbstractType
{
    ...
    public function solveWriteSize(\GMP $integer)
    {
        if (\gmp_cmp($integer, \gmp_pow(\gmp_init(2), 16)) < 0) {
            return [new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Types\Uint16(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\ByteOrder::LE), 0xfd];
        } else {
            if (\gmp_cmp($integer, \gmp_pow(\gmp_init(2), 32)) < 0) {
                return [new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Types\Uint32(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\ByteOrder::LE), 0xfe];
            } else {
                if (\gmp_cmp($integer, \gmp_pow(\gmp_init(2), 64)) < 0) {
                    return [new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Types\Uint64(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\ByteOrder::LE), 0xff];
                } else {
    ...
    public function write($integer) : string
    {
        ...
        if (\gmp_cmp($gmpInt, \gmp_sub(\gmp_pow(2, 16), 1)) <= 0) {
            ...
            if (\gmp_cmp($gmpInt, \gmp_sub(\gmp_pow(2, 32), 1)) <= 0) {
                return \pack("{$this->formatUint8}{$this->formatUint32LE}", 0xfe, $integer);
            } else {
                if (\gmp_cmp($gmpInt, \gmp_sub(\gmp_pow(2, 63), 1)) <= 0) {
        ...
        if ($byte < 0xfd) {
            ...
            if ($byte === 0xfd) {
                ...
                if ($byte === 0xfe) {
                    ...
                    $uint64 = new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Types\Uint64(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\ByteOrder::LE);
