...
class Buffertools
{
     ...
     * @throws \Exception
     */
    public static function numToVarInt(int $decimal) : \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface
    {
        return new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer(static::numToVarIntBin($decimal));
    }
    /**
    ...
    {
        $isBuffer = $bytes instanceof \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface;
        if ($isBuffer) {
            $bytes = $bytes->getBinary();
        }
        $flipped = \implode('', \array_reverse(\str_split($bytes, 1)));
        if ($isBuffer) {
            $flipped = new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer($flipped);
        }
        return $flipped;
    }
    /**
     ...
     * @return BufferInterface
     */
    public static function concat(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface $buffer1, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface $buffer2, int $size = null)
    {
        return new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer($buffer1->getBinary() . $buffer2->getBinary(), $size);
    }
    /**
    ...
    public static function sort(array $items, callable $convertToBuffer = null) : array
    {
        if (null == $convertToBuffer) {
            $convertToBuffer = function ($value) {
                if ($value instanceof \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\BufferInterface) {
                    return $value;
                }
                if ($value instanceof \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\SerializableInterface) {
