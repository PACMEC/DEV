...
class Signature
{
    ...
    {
        if ($options instanceof \Ethereumico\EthereumWallet\Dependencies\Elliptic\EC\Signature) {
            $this->r = $options->r;
            $this->s = $options->s;
        ...
        if (isset($options['r'])) {
            \assert(isset($options["r"]) && isset($options["s"]));
            //, "Signature without r or s");
            $this->r = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($options["r"], 16);
            $this->s = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($options["s"], 16);
    ...
    {
        $data = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::toArray($data, $enc);
        $dataLen = \count($data);
        $place = 0;
        if ($data[$place++] !== 0x30) {
            return \false;
        ...
        $this->r = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($r);
        $this->s = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($s);
        $this->recoveryParam = null;
        return \true;
    }
    ...
    public function toDER($enc = \false)
    {
        ...
        $arr = \array_merge($arr, $r, array(0x2));
        ...
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::encode($res, $enc);
    }
}
