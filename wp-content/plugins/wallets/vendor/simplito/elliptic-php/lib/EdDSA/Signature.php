...
class Signature
{
    ...
    function __construct($eddsa, $sig)
    {
        $this->eddsa = $eddsa;
        if (\is_string($sig)) {
            $sig = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::parseBytes($sig);
        }
        if (\is_array($sig) && !isset($sig["R"])) {
            $sig = ["R" => \array_slice($sig, 0, $eddsa->encodingLength), "S" => \array_slice($sig, $eddsa->encodingLength)];
        }
        \assert($sig["R"] && $sig["S"]);
        ...
        if ($sig["S"] instanceof \Ethereumico\EthereumWallet\Dependencies\BN\BN) {
            $this->_S = $sig["S"];
        }
    ...
    {
        return \strtoupper(\Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::encode($this->toBytes(), 'hex'));
    }
}
