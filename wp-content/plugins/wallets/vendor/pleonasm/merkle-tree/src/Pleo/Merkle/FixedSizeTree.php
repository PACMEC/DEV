 ...
 */
class FixedSizeTree implements \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\ITreeNode
{
    /**
     * @string[]
     */
    ...
    private static function buildHtreeRow($width, callable $hasher)
    {
        $row = [];
        if ($width === 2) {
            $row[] = new \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\TwoChildrenNode($hasher);
            return $row;
        }
        if ($width === 3) {
            $row[] = new \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\TwoChildrenNode($hasher);
            $row[] = new \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\TwoChildrenNode($hasher);
            $row[] = new \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\OneChildDuplicateNode(new \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\TwoChildrenNode($hasher));
        ...
        if ($odd) {
            $row[$rowSize - 1] = new \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\OneChildDuplicateNode($row[$rowSize - 1]);
        }
