...
class TwoChildrenNode implements \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\ITreeNode
{
    ...
    public function hash()
    {
        ...
        $first = $this->first;
        $second = $this->second;
        if ($this->first instanceof \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\ITreeNode) {
    ...
    public function data($first, $second)
    {
        ...
        if (\is_string($first) && \is_string($second) || $first instanceof \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\ITreeNode && $second instanceof \Ethereumico\EthereumWallet\Dependencies\Pleo\Merkle\ITreeNode) {
            $this->first = $first;
            $this->second = $second;
            return;
        }
