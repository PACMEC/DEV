...
/**
 * Elements PACMEC controls stack.
 *
...
abstract class Controls_Stack extends Base_Object {
	 ...
	 * The configuration.
	 *
	 * Holds the configuration used to generate the Elements PACMEC editor. It includes
	...
	public function start_controls_section( $section_id, array $args = [] ) {
		 ...
		 *
		 * Fires before Elements PACMEC section starts in the editor panel.
		 *
		 * @since 1.4.0
		 *
		...
		if ( null !== $this->current_section ) {
			wp_die( sprintf( 'Elements PACMEC: You can\'t start a section before the end of the previous section "%s".', $this->current_section['section'] ) ); // XSS ok.
		}
		 ...
		 * After section start.
		 *
		 * Fires after Elements PACMEC section starts in the editor panel.
		 *
		 * @since 1.4.0
		 *
		 * @param Controls_Stack $this       The control.
	...
	public function end_controls_section() {
		 ...
		 * Before section end.
		 *
		 * Fires before Elements PACMEC section ends in the editor panel.
		 *
		 * @since 1.4.0
		 *
		 * @param Controls_Stack $this       The control.
		 ...
		 * After section end.
		 *
		 * Fires after Elements PACMEC section ends in the editor panel.
		 *
		 * @since 1.4.0
		 *
		 * @param Controls_Stack $this       The control.
	...
	public function start_controls_tabs( $tabs_id, array $args = [] ) {
		if ( null !== $this->current_tab ) {
			wp_die( sprintf( 'Elements PACMEC: You can\'t start tabs before the end of the previous tabs "%s".', $this->current_tab['tabs_wrapper'] ) ); // XSS ok.
		}
	...
	public function start_controls_tab( $tab_id, $args ) {
		if ( ! empty( $this->current_tab['inner_tab'] ) ) {
			wp_die( sprintf( 'Elements PACMEC: You can\'t start a tab before the end of the previous tab "%s".', $this->current_tab['inner_tab'] ) ); // XSS ok.
		}
