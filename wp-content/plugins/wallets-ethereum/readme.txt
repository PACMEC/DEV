...
* To show user's Ethereum account address insert the `[wallets-ethereum-account]` shortcode wherever you like. You can use `label="My label"` attribute to set your own label text. And `nolabel="yes"` attribute to display no label at all.
* To show user's Ethereum account address's Ether balance insert the `[wallets-ethereum-balance]` shortcode wherever you like. Add the `displayfiat="1"` attribute to display the calculated fiat balance too.
* Limit balance update rate with `updatetimeout` attribute, like `[wallets-ethereum-balance updatetimeout="60"]` for once per minute update.
* Use `tokendecimals` attribute to configure the desired digits after the decimal separator count for the `[wallets-ethereum-balance]` shortcode.
* Use `tokendecimalchar` attribute to configure the desired decimal separator character for the `[wallets-ethereum-balance]` shortcode.
...
* New account creation form shortcode: `[wallets-ethereum-account-management-create]`
* Accounts list, select default shortcode: `[wallets-ethereum-account-management-select]`
* Private key import shortcode: `[wallets-ethereum-account-management-import]`
* Private key export shortcode: `[wallets-ethereum-account-management-export]`
...
* NFT (ERC721) tokens display and sending support: `[wallets-ethereum-nft]` shortcode, and NFT transfers display in the history table
...
* To show user's Ethereum account address's TSX ERC20 token balance insert the `[wallets-ethereum-balance tokenname="TSX" tokenaddress="0x6Fe928d427b0E339DB6FF1c7a852dc31b651bD3a"]` shortcode wherever you like.
* The [Cryptocurrency Product for WooCommerce](https://ethereumico.io/product/cryptocurrency-wordpress-plugin/ "Cryptocurrency Product for WooCommerce") plugin integration for the `[wallets-ethereum-balance]` shortcode is available. Add the `tokenwooproduct` attribute with a product id of the corresponding WooCommerce Token product as a value to display the balance in a fiat currency as well. The token to fiat currency rate would be calculated from the WooCommerce product price. Example: `[wallets-ethereum-balance tokenname="TSX" tokenaddress="0x6Fe928d427b0E339DB6FF1c7a852dc31b651bD3a" tokenwooproduct="123"]`. Result: `12.345 TSX $12.34`.
* The `tokeniconpath` attribute added to the `[wallets-ethereum-balance]` shortcode turns it to a more sophisticated widget with token icon. For token: `[wallets-ethereum-balance tokensymbol="TSX" tokenname="Test Coin" tokenaddress="0x6Fe928d427b0E339DB6FF1c7a852dc31b651bD3a" tokenwooproduct="123" tokeniconpath="https://example.com/icons/BTC.png"]`. For Ether: `[wallets-ethereum-balance displayfiat="1" tokeniconpath="https://example.com/icons/BTC.png"]`.
...
* The `[wallets-ethereum-accounts-table]` shortcode can be used to display a table of all accounts with fiat balances. Avatars and logins are also shown for the admin user. Integration with [BuddyPress](https://buddypress.org/) is provided for avatars display.
* To show the send ERC20 token form insert the `[wallets-ethereum-sendform]` shortcode wherever you like.
...
* `wallets-ethereum-dividends` shortcode can be used to display dividends payment history. See the `ERC20 Dividend Payments Add-On` of the [Cryptocurrency Product for WooCommerce](https://ethereumico.io/product/cryptocurrency-wordpress-plugin/ "Cryptocurrency Product for WooCommerce") plugin for details
* `tokenaddress` attribute for the `wallets-ethereum-accounts-table` forces this table to display token balances instead of the Ether
* `mintokenamount` attribute for the `wallets-ethereum-accounts-table` shortcode forces this table to display users with token balances greater or equal to the `mintokenamount` value set
...
* `tokenslist` attribute for the `wallets-ethereum-sendform` shortcode can contain a single allowed token address or a comma separated list of allowed token addresses.

> See the official site for a live demo: [https://ethereumico.io/wallets-ethereum/](https://ethereumico.io/wallets-ethereum/ "The Ethereum Wallet WordPress plugin")
...
1. The `[wallets-ethereum-account]` display
2. The `[wallets-ethereum-sendform]` display
3. The `[wallets-ethereum-history]` display
4. The Blockchain settings
5. The `[wallets-ethereum-account-management-create]` display
6. The `[wallets-ethereum-account-management-import]` display
7. The `[wallets-ethereum-account-management-select]` display
8. The `[wallets-ethereum-account-management-export]` display
9. The `[wallets-ethereum-nft columns="3" rows="2"]` display
10. The `[wallets-ethereum-balance]` display with different settings
11. The `[wallets-ethereum-nft columns="3" rows="2"]` token info display
12. The API Keys settings
13. The Admin Fee settings
14. The Advanced Blockchain settings part 1
15. The Advanced Blockchain settings part 2
16. The IPFS settings
...
> Make sure that [System Requirements](https://ethereumico.io/knowledge-base/wallets-ethereum-plugin-system-requirements/) are met on your hosting provider. These providers are tested for compliance: [Cloudways](https://www.cloudways.com/en/?id=462243), [Bluehost](https://www.bluehost.com/track/olegabr/), [SiteGround](https://www.siteground.com/go/ethereumico)

https://youtu.be/jB_JBlLGA6Q

https://youtu.be/F9_Yp1-E7JE

* Enter your settings in admin pages and place the `[wallets-ethereum-sendform]`, `[wallets-ethereum-balance]` and other shortcodes wherever you need it.
* For the proper use of the `[wallets-ethereum-accounts-table]` shortcode, change the `Permalink Settings` to anything other than the default value.
...
`
[wallets-ethereum-nft columns="3" rows="2"]

[wallets-ethereum-nft columns="3" rows="2" account="0x6975be450864c02b4613023c2152ee0743572325"]

[wallets-ethereum-account label="Your wallet:"]

[wallets-ethereum-account nolabel="yes"]

[wallets-ethereum-balance]

[wallets-ethereum-balance tokenname="TSX" tokenaddress="0x6Fe928d427b0E339DB6FF1c7a852dc31b651bD3a"]

[wallets-ethereum-sendform]

[wallets-ethereum-history]

[wallets-ethereum-history direction="in"]

[wallets-ethereum-history direction="out"]

[wallets-ethereum-account-management-create]

[wallets-ethereum-account-management-select]

[wallets-ethereum-account-management-import]

[wallets-ethereum-account-management-export]
`
...
* Send some Ropsten Ether to the account this plugin generated for you. Use `[wallets-ethereum-account]` shortcode to display it
...
* Now test the plugin by sending some Ropsten Ether and/or TSX tokens from the generated account address to your other address. Use the `[wallets-ethereum-sendform]` shortcode to render the send form on a page.
...
* Send some Rinkeby Ether to the account this plugin generated for you. Use `[wallets-ethereum-account]` shortcode to display it
...
* Use your favorite wallet to send TSX tokens to the account this plugin generated for you
* Now test the plugin by sending some Rinkeby Ether and/or TSX tokens from the generated account address to your other address. Use the `[wallets-ethereum-sendform]` shortcode to render the send form on a page.
...
* [ERC2212](https://github.com/ethereum/EIPs/issues/2212) support discontinued - standard proposal is abandoned.
...
* NFT (ERC721) tokens display and sending support: `[wallets-ethereum-nft]` shortcode, and history table support is added.
