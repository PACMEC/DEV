 ...
 * @license MIT
 */
namespace Ethereumico\EthereumWallet\Dependencies\Web3;

use InvalidArgumentException;
use Ethereumico\EthereumWallet\Dependencies\Web3\Providers\Provider;
use Ethereumico\EthereumWallet\Dependencies\Web3\Providers\HttpProvider;
use Ethereumico\EthereumWallet\Dependencies\Web3\RequestManagers\RequestManager;
...
use Ethereumico\EthereumWallet\Dependencies\Web3\Eth;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Ethabi;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Address;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Boolean;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Bytes;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\DynamicBytes;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Integer;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Str;
use Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Uinteger;
...
class Contract
{
    ...
    public function __construct($provider, $abi, $defaultBlock = 'latest')
    {
        if (\is_string($provider) && \filter_var($provider, \FILTER_VALIDATE_URL) !== \false) {
            ...
            if (\preg_match('/^https?:\\/\\//', $provider) === 1) {
                $requestManager = new \Ethereumico\EthereumWallet\Dependencies\Web3\RequestManagers\HttpRequestManager($provider);
                $this->provider = new \Ethereumico\EthereumWallet\Dependencies\Web3\Providers\HttpProvider($requestManager);
            }
        } else {
            if ($provider instanceof \Ethereumico\EthereumWallet\Dependencies\Web3\Providers\Provider) {
        ...
        if (\is_string($abi)) {
            ...
            $abiArray = \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::jsonToArray($abi);
        }
        ...
        if (\Ethereumico\EthereumWallet\Dependencies\Web3\Validators\TagValidator::validate($defaultBlock) || \Ethereumico\EthereumWallet\Dependencies\Web3\Validators\QuantityValidator::validate($defaultBlock)) {
            $this->defaultBlock = $defaultBlock;
        } else {
            $this->{$defaultBlock} = 'latest';
        }
        $this->abi = $abiArray;
        $this->eth = new \Ethereumico\EthereumWallet\Dependencies\Web3\Eth($this->provider);
        $this->ethabi = new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Ethabi(['address' => new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Address(), 'bool' => new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Boolean(), 'bytes' => new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Bytes(), 'dynamicBytes' => new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\DynamicBytes(), 'int' => new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Integer(), 'string' => new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Str(), 'uint' => new \Ethereumico\EthereumWallet\Dependencies\Web3\Contracts\Types\Uinteger()]);
    }
    /**
     * call
     *
    ...
    {
        if (\Ethereumico\EthereumWallet\Dependencies\Web3\Validators\AddressValidator::validate($address) === \false) {
            throw new \InvalidArgumentException('Please make sure address is valid.');
        }
        $this->toAddress = \Ethereumico\EthereumWallet\Dependencies\Web3\Formatters\AddressFormatter::format($address);
        return $this;
    }
    /**
     * bytecode
    ...
    {
        if (\Ethereumico\EthereumWallet\Dependencies\Web3\Validators\HexValidator::validate($bytecode) === \false) {
            throw new \InvalidArgumentException('Please make sure bytecode is valid.');
        }
        $this->bytecode = \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::stripZero($bytecode);
        return $this;
    }
    /**
     * abi
    ...
    {
        if (\Ethereumico\EthereumWallet\Dependencies\Web3\Validators\StringValidator::validate($abi) === \false) {
    ...
    public function deploy()
    {
        if (isset($this->constructor)) {
            ...
            $transaction['data'] = '0x' . $this->bytecode . \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::stripZero($data);
    ...
    public function send()
    {
        if (isset($this->functions)) {
            ...
            if (isset($transaction["from"]) || isset($transaction["to"]) || isset($transaction["gas"]) || isset($transaction["gasPrice"]) || isset($transaction["maxPriorityFeePerGas"]) || isset($transaction["maxFeePerGas"]) || isset($transaction["value"]) || isset($transaction["data"]) || isset($transaction["nonce"])) {
                $hasTransaction = \true;
            } else {
                $transaction = [];
            }
            ...
            foreach ($functions as $function) {
                ...
                try {
                    ...
                    $functionName = \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::jsonMethodToString($function);
            ...
            $transaction['data'] = $functionSignature . \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::stripZero($data);
            $this->eth->sendTransaction($transaction, function ($err, $transaction) use($callback) {
                if ($err !== null) {
                    return \call_user_func($callback, $err, null);
                }
    ...
    public function call()
    {
        if (isset($this->functions)) {
            ...
            if (!\is_array($transaction) && !isset($transaction["from"]) && !isset($transaction["to"]) && !isset($transaction["gas"]) && !isset($transaction["gasPrice"]) && !isset($transaction["maxPriorityFeePerGas"]) && !isset($transaction["maxFeePerGas"]) && !isset($transaction["value"]) && !isset($transaction["data"]) && !isset($transaction["nonce"])) {
                $transaction = [];
            }
            $functionSignature = $this->ethabi->encodeFunctionSignature($functionName);
            $transaction['to'] = $this->toAddress;
    ...
    public function getData()
    {
        if (isset($this->functions) || isset($this->constructor)) {
            ...
            if (empty($this->toAddress) && !empty($this->bytecode)) {
                ...
                $params = \array_splice($arguments, 0, \count($constructor['inputs']));
                $data = $this->ethabi->encodeParameters($constructor, $params);
                $functionData = $this->bytecode . \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::stripZero($data);
            } else {
                $method = \array_splice($arguments, 0, 1)[0];
                if (!\is_string($method)) {
                    throw new \InvalidArgumentException('Please make sure the method is string.');
                ...
                $functionSignature = $this->ethabi->encodeFunctionSignature($functionName);
                $functionData = \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::stripZero($functionSignature) . \Ethereumico\EthereumWallet\Dependencies\Web3\Utils::stripZero($data);
            }
            return $functionData;
        }
    }
