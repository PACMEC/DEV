 ...
 * @license MIT
 */
namespace Ethereumico\EthereumWallet\Dependencies\Web3\Methods;

use InvalidArgumentException;
use RuntimeException;
use Ethereumico\EthereumWallet\Dependencies\Web3\Methods\IMethod;
use Ethereumico\EthereumWallet\Dependencies\Web3\Methods\JSONRPC;
class EthMethod extends \Ethereumico\EthereumWallet\Dependencies\Web3\Methods\JSONRPC implements \Ethereumico\EthereumWallet\Dependencies\Web3\Methods\IMethod
{
    /**
     * validators
     *
        ...
        foreach ($rules as $key => $rule) {
            if (isset($params[$key])) {
                if (\is_array($rule)) {
                    ...
                    if ($isError) {
                        throw new \RuntimeException('Wrong type of ' . $this->method . ' method argument ' . $key . ' of type ' . \gettype($rule) . '.');
                    }
                } else {
