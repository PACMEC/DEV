 ...
 * @license MIT
 */
namespace Ethereumico\EthereumWallet\Dependencies\Web3p\EthereumUtil;
...
class Util
{
    ...
    public function recoverPublicKey($hash, $r, $s, $v)
    {
        ...
        $publicKey = $this->secp256k1->recoverPubKey($hash, ['r' => $r, 's' => $s], $v);
    ...
    public function ecsign(string $privateKey, string $message)
    {
        ...
        $secp256k1 = new \Ethereumico\EthereumWallet\Dependencies\Elliptic\EC('secp256k1');
        $privateKey = $secp256k1->keyFromPrivate($privateKey, 'hex');
        $signature = $privateKey->sign($message, ['canonical' => \true]);
    ...
    {
        $prefix = \sprintf("\31Ethereum Signed Message:\n%d", \mb_strlen($message));
