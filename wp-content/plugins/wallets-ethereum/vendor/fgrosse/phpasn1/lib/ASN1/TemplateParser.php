...
class TemplateParser
{
    ...
    {
        $parsedObject = \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\ASNObject::fromBinary($binary);
        foreach ($template as $key => $value) {
            $this->validate($parsedObject, $key, $value);
        }
        return $parsedObject;
    }
    private function validate(\Ethereumico\EthereumWallet\Dependencies\FG\ASN1\ASNObject $object, $key, $value)
    {
        if (\is_array($value)) {
    ...
    private function assertTypeId($expectedTypeId, \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\ASNObject $object)
    {
