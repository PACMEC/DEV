...
use Exception;
use Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Base128;
...
class ObjectIdentifier extends \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\ASNObject implements \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Parsable
{
    ...
    {
        return \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::OBJECT_IDENTIFIER;
    }
    ...
    protected function getEncodedValue()
    {
        $encodedValue = '';
        foreach ($this->subIdentifiers as $subIdentifier) {
            $encodedValue .= \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Base128::encode($subIdentifier);
        }
        return $encodedValue;
    }
    public function __toString()
    {
        return \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\OID::getName($this->value);
    }
    public static function fromBinary(&$binaryData, &$offsetIndex = 0)
    {
        self::parseIdentifier($binaryData[$offsetIndex], \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::OBJECT_IDENTIFIER, $offsetIndex++);
    ...
    protected static function parseOid(&$binaryData, &$offsetIndex, $octetsToRead)
    {
        ...
        while ($octetsToRead > 0) {
            ...
            do {
                if (0 === $octetsToRead) {
                    throw new \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Exception\ParserException('Malformed ASN.1 Object Identifier', $offsetIndex - 1);
                }
                $octetsToRead--;
                $octet = $binaryData[$offsetIndex++];
                $octets .= $octet;
            } while (\ord($octet) & 0x80);
            $oid .= \sprintf('%d.', \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Base128::decode($octets));
        }
        // Remove trailing '.'
