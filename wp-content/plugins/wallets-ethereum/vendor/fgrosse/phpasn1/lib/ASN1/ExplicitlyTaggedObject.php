 ...
 */
class ExplicitlyTaggedObject extends \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\ASNObject
{
    ...
    public function __toString()
    {
        switch ($length = \count($this->decoratedObjects)) {
            ...
            case 1:
                $decoratedType = \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::getShortName($this->decoratedObjects[0]->getType());
    ...
    {
        $identifier = \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::create(\Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::CLASS_CONTEXT_SPECIFIC, \true, $this->tag);
    ...
    public static function fromBinary(&$binaryData, &$offsetIndex = 0)
    {
        ...
        \assert(\Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::isContextSpecificClass($firstIdentifierOctet), 'identifier octet should indicate context specific class');
        \assert(\Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::isConstructed($firstIdentifierOctet), 'identifier octet should indicate constructed object');
        $tag = \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::getTagNumber($identifier);
        ...
        while ($remainingContentLength > 0) {
            $nextObject = \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\ASNObject::fromBinary($binaryData, $offsetIndex);
            $remainingContentLength -= $nextObject->getObjectLength();
            $decoratedObjects[] = $nextObject;
        }
        if ($remainingContentLength != 0) {
            throw new \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Exception\ParserException("Context-Specific explicitly tagged object [{$tag}] starting at offset {$offsetIndexOfDecoratedObject} specifies a length of {$totalContentLength} octets but {$remainingContentLength} remain after parsing the content", $offsetIndexOfDecoratedObject);
        }
