...
class Integer extends \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\ASNObject implements \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Parsable
{
    /** @var int */
    private $value;
    /**
    ...
    {
        return \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Identifier::INTEGER;
    }
    ...
    {
        $value = \Ethereumico\EthereumWallet\Dependencies\FG\Utility\BigInteger::create($this->value, 10);
    ...
    private static function ensureMinimalEncoding($binaryData, $offsetIndex)
    {
        ...
        if (\ord($binaryData[$offsetIndex]) == 0x0 && (\ord($binaryData[$offsetIndex + 1]) & 0x80) == 0 || \ord($binaryData[$offsetIndex]) == 0xff && (\ord($binaryData[$offsetIndex + 1]) & 0x80) == 0x80) {
            throw new \Ethereumico\EthereumWallet\Dependencies\FG\ASN1\Exception\ParserException("Integer not minimally encoded", $offsetIndex);
        }
    }
    public static function fromBinary(&$binaryData, &$offsetIndex = 0)
    {
        ...
        $number = \Ethereumico\EthereumWallet\Dependencies\FG\Utility\BigInteger::create(\ord($binaryData[$offsetIndex++]) & 0x7f);
        for ($i = 0; $i < $contentLength - 1; $i++) {
            $number = $number->multiply(0x100)->add(\ord($binaryData[$offsetIndex++]));
        }
        if ($isNegative) {
            $number = $number->subtract(\Ethereumico\EthereumWallet\Dependencies\FG\Utility\BigInteger::create(2)->toPower(8 * $contentLength - 1));
        }
