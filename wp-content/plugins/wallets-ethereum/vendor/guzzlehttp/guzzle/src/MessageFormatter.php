 ...
 * @final
 */
class MessageFormatter implements \Ethereumico\EthereumWallet\Dependencies\GuzzleHttp\MessageFormatterInterface
{
    /**
     ...
     */
    public function format(\Ethereumico\EthereumWallet\Dependencies\Psr\Http\Message\RequestInterface $request, ?\Ethereumico\EthereumWallet\Dependencies\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        $cache = [];
        /** @var string */
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \Ethereumico\EthereumWallet\Dependencies\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \Ethereumico\EthereumWallet\Dependencies\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
                ...
                case 'req_body':
                    ...
                    break;
                case 'res_body':
                    if (!$response instanceof \Ethereumico\EthereumWallet\Dependencies\Psr\Http\Message\ResponseInterface) {
                        $result = 'NULL';
                        break;
                    }
     ...
     */
    private function headers(\Ethereumico\EthereumWallet\Dependencies\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
