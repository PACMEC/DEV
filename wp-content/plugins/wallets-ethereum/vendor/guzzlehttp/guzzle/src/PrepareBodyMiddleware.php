    ...
    public function __invoke(\Ethereumico\EthereumWallet\Dependencies\Psr\Http\Message\RequestInterface $request, array $options) : \Ethereumico\EthereumWallet\Dependencies\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (\is_string($uri) && ($type = \Ethereumico\EthereumWallet\Dependencies\GuzzleHttp\Psr7\MimeType::fromFilename($uri))) {
        ...
        return $fn(\Ethereumico\EthereumWallet\Dependencies\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
    private function addExpectHeader(\Ethereumico\EthereumWallet\Dependencies\Psr\Http\Message\RequestInterface $request, array $options, array &$modify) : void
    {
