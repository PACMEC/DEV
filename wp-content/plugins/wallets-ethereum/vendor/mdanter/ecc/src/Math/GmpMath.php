...
class GmpMath implements \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\GmpMathInterface
{
    /**
     * {@inheritDoc}
    ...
    public function decHex(string $dec) : string
    {
        ...
        if (\Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Util\BinaryString::length($hex) % 2 != 0) {
            $hex = '0' . $hex;
        }
        return $hex;
    }
    ...
    public function intToFixedSizeString(\GMP $x, int $byteSize) : string
    {
        ...
        if (\Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Util\NumberSize::bnNumBits($this, $x) >= \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Util\NumberSize::bnNumBits($this, $range)) {
    ...
    public function stringToInt(string $s) : \GMP
    {
        ...
        $sLen = \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Util\BinaryString::length($s);
        for ($c = 0; $c < $sLen; $c++) {
    ...
    {
        return $this->stringToInt(\hash('sha1', $this->intToString($m), \true));
    }
    /**
     * {@inheritDoc}
     ...
     */
    public function getNumberTheory() : \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\NumberTheory
    {
        return new \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\NumberTheory($this);
    }
    /**
     * @param \GMP $modulus
     * @return ModularArithmetic
     */
    public function getModularArithmetic(\GMP $modulus) : \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\ModularArithmetic
    {
        return new \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\ModularArithmetic($this, $modulus);
    }
}
