...
use Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\InvalidNetworkParameter;
use Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingBase58Prefix;
use Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingBech32Prefix;
use Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingBip32Prefix;
use Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingNetworkParameter;
class Network implements \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Network\NetworkInterface
{
    ...
    {
        if (!\is_string($value)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\InvalidNetworkParameter("{$field} must be a string");
        }
        $length = \strlen($value);
        if ($length == 0) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\InvalidNetworkParameter("{$field} must be non-empty string");
        }
        if ($length % 2 !== 0) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\InvalidNetworkParameter("{$field} must have even number of characters (hex representing bytes)");
        }
        if (!\ctype_xdigit($value)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\InvalidNetworkParameter("{$field} prefix must be a valid hex string");
        }
    }
    /**
     * Network constructor.
    ...
    public function __construct()
    {
        ...
        if (\count($this->bip32ScriptTypeMap) !== \count($this->bip32PrefixMap)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\InvalidNetworkParameter("BIP32 prefixes not configured correctly. Number of items does not match.");
        }
    }
    /**
    ...
    protected function getBase58Prefix(string $prefixType) : string
    {
        if (!$this->hasBase58Prefix($prefixType)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingBase58Prefix();
        }
    ...
    protected function getBech32Prefix(string $prefixType) : string
    {
        if (!$this->hasBech32Prefix($prefixType)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingBech32Prefix();
        }
    ...
    protected function getBip32Prefix(string $prefixType) : string
    {
        if (!$this->hasBip32Prefix($prefixType)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingBip32Prefix();
        }
    ...
    public function getSignedMessageMagic() : string
    {
        if (null === $this->signedMessagePrefix) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingNetworkParameter("Missing magic string for signed message");
        }
    ...
    {
        if (null === $this->p2pMagic) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Exceptions\MissingNetworkParameter("Missing network magic bytes");
        }
