...
namespace Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math;

use Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\GmpMath;
use Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Util\NumberSize;
class Math extends \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Math\GmpMath
{
    /**
     * @param \GMP $integer
     * @return bool
    ...
    public function decodeCompact($compact, &$isNegative, &$isOverflow) : \GMP
    {
        ...
        $word = $this->bitwiseAnd($compact, \gmp_init(0x7fffff, 10));
        ...
        $zero = \gmp_init(0);
        $isNegative = $this->cmp($word, $zero) !== 0 && $this->cmp($this->bitwiseAnd($compact, \gmp_init(0x800000)), $zero) === 1;
        $isOverflow = $this->cmp($word, $zero) !== 0 && ($this->cmp($size, \gmp_init(34, 10)) > 0 || $this->cmp($word, \gmp_init(0xff, 10)) > 0 && $this->cmp($size, \gmp_init(33, 10)) > 0 || $this->cmp($word, \gmp_init(0xffff, 10)) > 0 && $this->cmp($size, \gmp_init(32, 10)) > 0);
        return $word;
    }
    /**
     * @param \GMP $integer
    ...
    public function encodeCompact(\GMP $integer, bool $fNegative) : \GMP
    {
        ...
        $size = (int) \Ethereumico\EthereumWallet\Dependencies\Mdanter\Ecc\Util\NumberSize::bnNumBytes($this, $integer);
        if ($size <= 3) {
        ...
        if ($this->cmp($this->bitwiseAnd($compact, \gmp_init(0x800000, 10)), \gmp_init(0, 10)) > 0) {
            $compact = $this->rightShift($compact, 8);
            $size = $size + 1;
        }
        $compact = $this->bitwiseOr($compact, $this->leftShift(\gmp_init($size, 10), 24));
        if ($fNegative && $this->cmp($this->bitwiseAnd($compact, \gmp_init(0x7fffff, 10)), \gmp_init(0, 10)) > 0) {
            /// ?
            $compact = $this->bitwiseOr($compact, \gmp_init(0x800000, 10));
        }
        return $compact;
    }
}
