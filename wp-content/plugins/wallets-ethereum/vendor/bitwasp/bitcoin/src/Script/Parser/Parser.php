...
class Parser implements \Iterator
{
     ...
     */
    public function __construct(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Math\Math $math, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptInterface $script)
    {
        $this->math = $math;
        ...
        $this->script = $script;
        $this->empty = new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer('', 0);
    }
    /**
     * @return int
     */
    ...
    private function doNext(int $ptr)
    {
        ...
        $pushData = $this->empty;
        $dataSize = 0;
        if ($opCode <= \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_PUSHDATA4) {
            if ($opCode < \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_PUSHDATA1) {
                $dataSize = $opCode;
            } else {
                if ($opCode === \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_PUSHDATA1) {
                    $dataSize = $this->unpackSize('C', 1);
                } else {
                    if ($opCode === \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_PUSHDATA2) {
            ...
            if ($dataSize > 0) {
                $pushData = new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer(\substr($this->data, $this->position, $dataSize), $dataSize);
            }
            $this->position += $dataSize;
        }
        $this->array[$ptr] = new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Parser\Operation($opCode, $pushData, $dataSize);
        $this->count++;
    ...
    public function slice(int $begin, int $length = null)
    {
        ...
        return new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Script(new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Buffertools\Buffer(\substr($this->data, $begin, $length)));
    }
    /**
     *
     */
    ...
    {
        return \implode(' ', \array_map(function (\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Parser\Operation $operation) {
            $op = $operation->getOp();
            if ($op === \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_0 || $op === \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_1NEGATE || $op >= \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_1 && $op <= \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Opcodes::OP_16) {
