...
class BitcoinConsensus implements \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Consensus\ConsensusInterface
{
    /**
    ...
    public function verify(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Transaction\TransactionInterface $tx, \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\ScriptInterface $scriptPubKey, int $flags, int $nInputToSign, int $amount) : bool
    {
        if ($flags !== ($flags & BITCOINCONSENSUS_SCRIPT_FLAGS_VERIFY_ALL)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Consensus\Exception\BitcoinConsensusException("Invalid flags for bitcoinconsensus");
        }
        $error = 0;
        if ($flags & \Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Interpreter\InterpreterInterface::VERIFY_WITNESS) {
