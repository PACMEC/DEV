 ...
 */
namespace Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC;

use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\Formats\Signature\ASN1 as ASN1Signature;
...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common;
 ...
 * @access  public
 */
class PrivateKey extends \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC implements \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\PrivateKey
{
    ...
    public function multiply($coordinates)
    {
        if ($this->curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            if ($this->curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\Curves\Curve25519 && self::$engines['libsodium']) {
                return \sodium_crypto_scalarmult($this->dA->toBytes(), $coordinates);
            }
            $point = [$this->curve->convertInteger(new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(\strrev($coordinates), 256))];
        ...
        $point = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\Formats\Keys\PKCS1::extractPoint($coordinates, $this->curve);
        $point = $this->curve->multiplyPoint($point, $this->dA);
        if ($this->curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
    ...
    public function sign($message)
    {
        ...
        if ($this->curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            if ($this->curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\Curves\Ed25519 && self::$engines['libsodium'] && !isset($this->context)) {
                $result = \sodium_crypto_sign_detached($message, $this->toString('libsodium'));
                return $shortFormat == 'SSH2' ? \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', 'ssh-' . \strtolower($this->getCurve()), $result) : $result;
            }
            ...
            $curve = $this->curve;
            $hash = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash($curve::HASH);
            $secret = \substr($hash->hash($this->dA->secret), $curve::SIZE);
            if ($curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\Curves\Ed25519) {
            ...
            $r = \strrev($r);
            $r = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($r, 256);
            list(, $r) = $r->divide($order);
            $R = $curve->multiplyPoint($curve->getBasePoint(), $r);
            $R = $curve->encodePoint($R);
            $k = $hash->hash($dom . $R . $A . $message);
            $k = \strrev($k);
            $k = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($k, 256);
            list(, $k) = $k->divide($order);
            $S = $k->multiply($dA)->add($r);
            list(, $S) = $S->divide($order);
            $S = \str_pad(\strrev($S->toBytes()), $curve::SIZE, "\0");
            return $shortFormat == 'SSH2' ? \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', 'ssh-' . \strtolower($this->getCurve()), $R . $S) : $R . $S;
        }
        if (self::$engines['OpenSSL'] && \in_array($this->hash->getHash(), \openssl_get_md_methods())) {
            ...
            if ($result) {
                ...
                \extract(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\Formats\Signature\ASN1::load($signature));
                return $shortFormat == 'SSH2' ? $format::save($r, $s, $this->getCurve()) : $format::save($r, $s);
            }
        }
        $e = $this->hash->hash($message);
        $e = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($e, 256);
        $Ln = $this->hash->getLength() - $order->getLength();
        $z = $Ln > 0 ? $e->bitwise_rightShift($Ln) : $e;
        while (\true) {
            $k = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger::randomRange(self::$one, $order->subtract(self::$one));
    ...
    public function getPublicKey()
    {
        ...
        $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC::loadFormat($format, $key);
        if ($this->curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            return $key;
        }
