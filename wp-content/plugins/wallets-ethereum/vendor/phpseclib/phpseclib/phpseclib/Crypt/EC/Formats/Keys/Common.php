...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Prime as PrimeCurve;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Binary as BinaryCurve;
...
trait Common
{
     ...
     */
    public static function setImplicitCurve(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
    ...
    protected static function loadCurveByParam(array $params)
    {
        ...
        if (isset($params['namedCurve'])) {
            ...
            if (!\class_exists($curve)) {
                throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $params['namedCurve'] . ' is not supported');
            }
            return new $curve();
        }
        ...
        if (isset($params['specifiedCurve'])) {
            ...
            switch ($data['fieldID']['fieldType']) {
                case 'prime-field':
                    $curve = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Prime();
                    $curve->setModulo($data['fieldID']['parameters']);
                    $curve->setCoefficients(new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($data['curve']['a'], 256), new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($data['curve']['b'], 256));
                    $point = self::extractPoint("\0" . $data['base'], $curve);
                    $curve->setBasePoint(...$point);
                    $curve->setOrder($data['order']);
                    return $curve;
                case 'characteristic-two-field':
                    $curve = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Binary();
                    $params = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($data['fieldID']['parameters']);
                    $params = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($params[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Characteristic_two::MAP);
                    ...
                    switch ($params['basis']) {
                        case 'tpBasis':
                            ...
                            break;
                        case 'ppBasis':
                            $temp = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::decodeBER($params['parameters']);
                            $temp = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::asn1map($temp[0], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Pentanomial::MAP);
                    ...
                    $len = \ceil($modulo[0] / 8);
                    $curve->setCoefficients(\Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Hex::encode($data['curve']['a']), \Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Hex::encode($data['curve']['b']));
                    $point = self::extractPoint("\0" . $data['base'], $curve);
                    $curve->setBasePoint(...$point);
                    $curve->setOrder($data['order']);
                    return $curve;
                default:
                    throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $data['fieldID']['fieldType'] . ' is not supported');
            }
        }
     ...
     * @return object[]
     */
    public static function extractPoint($str, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
        if ($curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            ...
            $y[0] = $y[0] & \chr(0x7f);
            $y = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($y, 256);
        ...
        if (($val = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($str)) != "\0") {
            throw new \UnexpectedValueException('extractPoint expects the first byte to be null - not ' . \Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Hex::encode($val));
        }
        if ($str == "\0") {
            return [];
        }
        ...
        if ($keylen == 2 * $order + 1) {
            ...
            list(, $w, $x, $y) = $matches;
            if ($w != "\4") {
                throw new \UnexpectedValueException('The first byte of an uncompressed point should be 04 - not ' . \Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Hex::encode($val));
            }
            $point = [$curve->convertInteger(new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($x, 256)), $curve->convertInteger(new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($y, 256))];
            if (!$curve->verifyPoint($point)) {
                throw new \RuntimeException('Unable to verify that point exists on curve');
            }
            return $point;
     ...
     * @return string|false
     */
    private static function encodeParameters(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Base $curve, $returnArray = \false, array $options = [])
    {
        ...
        if ($useNamedCurves) {
            if (isset(self::$curveOIDs[$name])) {
                ...
                return $returnArray ? ['namedCurve' => $name] : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER(['namedCurve' => $name], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
            }
            foreach (new \DirectoryIterator(__DIR__ . '/../../Curves/') as $file) {
                ...
                switch ($name) {
                    case 'Prime':
                        if (!$candidate instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Prime) {
                            break;
                        }
                        ...
                        return $returnArray ? ['namedCurve' => $testName] : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER(['namedCurve' => $testName], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
                    case 'Binary':
                        if (!$candidate instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Binary) {
                            break;
                        }
        ...
        $x = $point[0]->toBytes();
        $y = $point[1]->toBytes();
        if ($curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Prime) {
            /*
             * valid versions are:
             *
             * ecdpVer1:
            ...
            return $returnArray ? ['specifiedCurve' => $data] : \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER(['specifiedCurve' => $data], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        }
        if ($curve instanceof \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Binary) {
            ...
            switch ($basis) {
                case 3:
                    $basis = 'tpBasis';
                    $modulo = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($modulo[0]);
                    break;
                case 5:
                    $basis = 'ppBasis';
                    // these should be in strictly ascending order (hence the commented out rsort above)
                    $modulo = ['k1' => new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($modulo[2]), 'k2' => new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($modulo[1]), 'k3' => new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($modulo[0])];
                    $modulo = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER($modulo, \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Pentanomial::MAP);
                    $modulo = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Element($modulo);
            }
            $params = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1::encodeDER(['m' => new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($m), 'basis' => $basis, 'parameters' => $modulo], \Ethereumico\EthereumWallet\Dependencies\phpseclib3\File\ASN1\Maps\Characteristic_two::MAP);
        ...
        throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Curve cannot be serialized');
    }
    /**
     * Use Specified Curve
     *
