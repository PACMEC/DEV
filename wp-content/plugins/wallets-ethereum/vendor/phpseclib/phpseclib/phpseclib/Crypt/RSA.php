...
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\PrivateKey;
use Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\PublicKey;
 ...
 * @access  public
 */
abstract class RSA extends \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Common\AsymmetricKey
{
    /**
     * Algorithm Name
     *
    ...
    public static function createKey($bits = 2048)
    {
        ...
        if ($num_primes == 2 && $bits >= 384 && self::$defaultExponent == 65537) {
            ...
            if (self::$engines['OpenSSL']) {
                ...
                return \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA::load($privatekeystr);
            }
        }
        static $e;
        if (!isset($e)) {
            $e = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger(self::$defaultExponent);
        }
        $n = clone self::$one;
        ...
        do {
            for ($i = 1; $i <= $num_primes; $i++) {
                if ($i != $num_primes) {
                    $primes[$i] = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger::randomPrime($regSize);
                } else {
                    \extract(\Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger::minMaxBits($bits));
                    /** @var BigInteger $min
                     *  @var BigInteger $max
                     */
                    ...
                    $primes[$i] = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger::randomRangePrime($min, $max);
                }
        ...
        // }
        $privatekey = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\PrivateKey();
    ...
    {
        $key = $components['isPublicKey'] ? new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\PublicKey() : new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\PrivateKey();
        ...
        if ($components['format'] == \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
    ...
    protected function __construct()
    {
        ...
        $this->mgfHash = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash('sha256');
    ...
    {
        return new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Math\BigInteger($x, 256);
    }
    /**
    ...
    protected function emsa_pkcs1_v1_5_encode_without_null($m, $emLen)
    {
        ...
        switch ($this->hash->getHash()) {
            ...
            case 'sha512/256':
                ...
                break;
            default:
                throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('md2 and md5 require NULLs');
        }
        $t .= $h;
        $tLen = \strlen($t);
        if ($emLen < $tLen + 11) {
    ...
    public function withHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/224':
            case 'sha512/256':
                $new->hash = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash($hash);
                break;
            default:
                throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256');
        }
    ...
    public function withMGFHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/224':
            case 'sha512/256':
                $new->mgfHash = new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Hash($hash);
                break;
            default:
                throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256');
        }
    ...
    public function withPadding($padding)
    {
        ...
        if ($numSelected > 1) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\InconsistentSetupException('Multiple encryption padding modes have been selected; at most only one should be selected');
        }
        ...
        if ($numSelected > 1) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\InconsistentSetupException('Multiple signature padding modes have been selected; at most only one should be selected');
        }
