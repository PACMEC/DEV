...
abstract class OpenSSH
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (\strpos($key, 'BEGIN OPENSSH PRIVATE KEY') !== \false) {
            ...
            $magic = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::shift($key, 15);
            if ($magic != "openssh-key-v1\0") {
                throw new \RuntimeException('Expected openssh-key-v1');
            }
            list($ciphername, $kdfname, $kdfoptions, $numKeys) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('sssN', $key);
            if ($numKeys != 1) {
            ...
            list($publicKey, $paddedKey) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $key);
            list($type) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $publicKey);
            list($checkint1, $checkint2) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $paddedKey);
        ...
        list($type) = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $key);
        self::checkType($type);
    ...
    protected static function wrapPrivateKey($publicKey, $privateKey, $password, $options)
    {
        if (!empty($password) && \is_string($password)) {
            throw new \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Exception\UnsupportedFormatException('Encrypted OpenSSH private keys are not supported');
        }
        list(, $checkint) = \unpack('N', \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Crypt\Random::string(4));
        $comment = isset($options['comment']) ? $options['comment'] : self::$comment;
        $paddedKey = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('NN', $checkint, $checkint) . $privateKey . \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $comment);
        /*
        ...
        */
        ...
        $key = \Ethereumico\EthereumWallet\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('sssNss', 'none', 'none', '', 1, $publicKey, $paddedKey);
        $key = "openssh-key-v1\0{$key}";
        return "-----BEGIN OPENSSH PRIVATE KEY-----\n" . \chunk_split(\Ethereumico\EthereumWallet\Dependencies\ParagonIE\ConstantTime\Base64::encode($key), 70, "\n") . "-----END OPENSSH PRIVATE KEY-----\n";
    }
}
