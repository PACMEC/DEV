...
namespace Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\ShortCurve;

use JsonSerializable;
use Ethereumico\EthereumWallet\Dependencies\BN\BN;
class Point extends \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\BaseCurve\Point implements \JsonSerializable
{
    public $x;
    public $y;
    public $inf;
    function __construct($curve, $x, $y, $isRed)
    {
        ...
        if ($x == null && $y == null) {
            ...
            $this->x = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($x, 16);
            $this->y = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($y, 16);
    ...
    public function _getBeta()
    {
        ...
        if (isset($this->precomputed)) {
            ...
            $beta->precomputed = array("beta" => null, "naf" => null, "doubles" => null);
            if (isset($this->precomputed["naf"])) {
                $beta->precomputed["naf"] = array("wnd" => $this->precomputed["naf"]["wnd"], "points" => \array_map($endoMul, $this->precomputed["naf"]["points"]));
            }
            if (isset($this->precomputed["doubles"])) {
                $beta->precomputed["doubles"] = array("step" => $this->precomputed["doubles"]["step"], "points" => \array_map($endoMul, $this->precomputed["doubles"]["points"]));
            }
    ...
    public function jsonSerialize()
    {
        ...
        $addPre = \false;
        if (isset($this->precomputed["doubles"])) {
            $pre["doubles"] = array("step" => $this->precomputed["doubles"]["step"], "points" => \array_slice($this->precomputed["doubles"]["points"], 1));
            $addPre = \true;
        }
        if (isset($this->precomputed["naf"])) {
            $pre["naf"] = array("naf" => $this->precomputed["naf"]["wnd"], "points" => \array_slice($this->precomputed["naf"]["points"], 1));
            $addPre = \true;
        }
        if ($addPre) {
            \array_push($res, $pre);
    ...
    public static function fromJSON($curve, $obj, $red)
    {
        ...
        if (isset($pre["doubles"])) {
            ...
            $point->precomputed["doubles"] = array("step" => $pre["doubles"]["step"], "points" => $tmp);
        }
        if (isset($pre["naf"])) {
            $tmp = \array_map($obj2point, $pre["naf"]["points"]);
            \array_unshift($tmp, $point);
            $point->precomputed["naf"] = array("wnd" => $pre["naf"]["wnd"], "points" => $tmp);
        }
        return $point;
    }
    ...
    {
        $k = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($k, 16);
    ...
    {
        return $this->mulAdd($k1, $p2, $k2, \true);
    }
    public function eq($point)
    {
        return $this === $point || $this->inf === $point->inf && ($this->inf || $this->x->cmp($point->x) === 0 && $this->y->cmp($point->y) === 0);
    }
    public function neg($precompute = \false)
    {
        ...
        if ($precompute && isset($this->precomputed)) {
            ...
            if (isset($pre["naf"])) {
                $res->precomputed["naf"] = array("wnd" => $pre["naf"]["wnd"], "points" => \array_map($negate, $pre["naf"]["points"]));
            }
            if (isset($pre["doubles"])) {
                $res->precomputed["doubles"] = array("step" => $pre["doubles"]["step"], "points" => \array_map($negate, $pre["doubles"]["points"]));
            }
        }
        return $res;
    }
