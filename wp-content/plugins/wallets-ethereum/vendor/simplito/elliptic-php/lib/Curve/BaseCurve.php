...
abstract class BaseCurve
{
    ...
    {
        $this->type = $type;
        $this->p = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($conf["p"], 16);
        //Use Montgomery, when there is no fast reduction for the prime
        $this->red = isset($conf["prime"]) ? \Ethereumico\EthereumWallet\Dependencies\BN\BN::red($conf["prime"]) : \Ethereumico\EthereumWallet\Dependencies\BN\BN::mont($this->p);
        //Useful for many curves
        $this->zero = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN(0))->toRed($this->red);
        $this->one = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN(1))->toRed($this->red);
        $this->two = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN(2))->toRed($this->red);
        //Curve configuration, optional
        $this->n = isset($conf["n"]) ? new \Ethereumico\EthereumWallet\Dependencies\BN\BN($conf["n"], 16) : null;
        ...
        if ($adjustCount == null || $adjustCount->cmpn(100) > 0) {
            ...
            $this->_maxwellTrick = \true;
        }
    }
    public abstract function point($x, $z);
    public abstract function validate($point);
    public function _fixedNafMul($p, $k)
    {
        \assert(isset($p->precomputed));
        $doubles = $p->_getDoubles();
        $naf = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::getNAF($k, 1);
    ...
    public function _wnafMul($p, $k)
    {
        ...
        //Get NAF form
        $naf = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::getNAF($k, $w);
    ...
    public function _wnafMulAdd($defW, $points, $coeffs, $len, $jacobianResult = \false)
    {
        ...
        for ($i = $len - 1; $i >= 1; $i -= 2) {
            ...
            if ($wndWidth[$a] != 1 || $wndWidth[$b] != 1) {
                $naf[$a] = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::getNAF($coeffs[$a], $wndWidth[$a]);
                $naf[$b] = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::getNAF($coeffs[$b], $wndWidth[$b]);
            ...
            $jsf = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::getJSF($coeffs[$a], $coeffs[$b]);
    ...
    {
        $bytes = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::toArray($bytes, $enc);
        $len = $this->p->byteLength();
        $count = \count($bytes);
        //uncompressed, hybrid-odd, hybrid-even
        if (($bytes[0] == 0x4 || $bytes[0] == 0x6 || $bytes[0] == 0x7) && $count - 1 == 2 * $len) {
            if ($bytes[0] == 0x6) {
        ...
        if (($bytes[0] == 0x2 || $bytes[0] == 0x3) && $count - 1 == $len) {
            return $this->pointFromX(\array_slice($bytes, 1, $len), $bytes[0] == 0x3);
        }
