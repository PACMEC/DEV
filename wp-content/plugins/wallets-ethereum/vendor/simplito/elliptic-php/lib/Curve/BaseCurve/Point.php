...
namespace Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\BaseCurve;
...
abstract class Point
{
    ...
    public abstract function eq($other);
    ...
    {
        return $this->encode($enc, \true);
    }
    public function encode($enc, $compact = \false)
    {
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::encode($this->_encode($compact), $enc);
    }
    protected function _encode($compact)
    {
        ...
        if ($compact) {
            \array_unshift($x, $this->getY()->isEven() ? 0x2 : 0x3);
            return $x;
        }
        return \array_merge(array(0x4), $x, $this->getY()->toArray("be", $len));
    }
    public function precompute($power = null)
    {
        ...
        $this->precomputed = array("naf" => $this->_getNAFPoints(8), "doubles" => $this->_getDoubles(4, $power), "beta" => $this->_getBeta());
        return $this;
    }
    ...
    public function _getDoubles($step = null, $power = null)
    {
        ...
        return array("step" => $step, "points" => $doubles);
    }
    public function _getNAFPoints($wnd)
    {
        ...
        return array("wnd" => $wnd, "points" => $res);
    }
