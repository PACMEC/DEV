...
use Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\ShortCurve\Point;
use Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\ShortCurve\JPoint;
use Ethereumico\EthereumWallet\Dependencies\BN\BN;
use Exception;
class ShortCurve extends \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\BaseCurve
{
    public $a;
    public $b;
    public $tinv;
    ...
    private function _getEndomorphism($conf)
    {
        ...
        $lambda = null;
        if (isset($conf["beta"])) {
            $beta = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN($conf["beta"], 16))->toRed($this->red);
        } else {
        ...
        if (isset($conf["lambda"])) {
            $lambda = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($conf["lambda"], 16);
        } else {
        ...
        if (!isset($conf["basis"])) {
            ...
            $callback = function ($vector) {
                return array("a" => new \Ethereumico\EthereumWallet\Dependencies\BN\BN($vector["a"], 16), "b" => new \Ethereumico\EthereumWallet\Dependencies\BN\BN($vector["b"], 16));
            };
            $basis = \array_map($callback, $conf["basis"]);
        }
        return array("beta" => $beta, "lambda" => $lambda, "basis" => $basis);
    }
    private function _getEndoRoots($num)
    {
        ...
        // Root = (-1 +- Sqrt(-3)) / 2
        //
        $red = $num === $this->p ? $this->red : \Ethereumico\EthereumWallet\Dependencies\BN\BN::mont($num);
        $tinv = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN(2))->toRed($red)->redInvm();
        $ntinv = $tinv->redNeg();
        $s = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN(3))->toRed($red)->redNeg()->redSqrt()->redMul($tinv);
        return array($ntinv->redAdd($s)->fromRed(), $ntinv->redSub($s)->fromRed());
    }
    private function _getEndoBasis($lambda)
    {
        ...
        $u = $lambda;
        $v = $this->n->_clone();
        $x1 = new \Ethereumico\EthereumWallet\Dependencies\BN\BN(1);
        $y1 = new \Ethereumico\EthereumWallet\Dependencies\BN\BN(0);
        $x2 = new \Ethereumico\EthereumWallet\Dependencies\BN\BN(0);
        $y2 = new \Ethereumico\EthereumWallet\Dependencies\BN\BN(1);
        ...
        return array(array("a" => $a1, "b" => $b1), array("a" => $a2, "b" => $b2));
    }
    ...
    {
        $x = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($x, 16);
        if (!$x->red) {
            $x = $x->toRed($this->red);
        }
    ...
    {
        return new \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\ShortCurve\Point($this, $x, $y, $isRed);
    }
    public function pointFromJSON($obj, $red)
    {
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\ShortCurve\Point::fromJSON($this, $obj, $red);
    }
    public function jpoint($x, $y, $z)
    {
        return new \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\ShortCurve\JPoint($this, $x, $y, $z);
    }
}
