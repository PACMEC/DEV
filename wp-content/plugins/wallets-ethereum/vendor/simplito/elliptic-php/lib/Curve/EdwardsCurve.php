...
use Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\EdwardsCurve\Point;
use Ethereumico\EthereumWallet\Dependencies\BN\BN;
class EdwardsCurve extends \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\BaseCurve
{
    public $twisted;
    public $mOneA;
    public $extended;
    ...
    function __construct($conf)
    {
        ...
        $this->a = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN($conf["a"], 16))->umod($this->red->m);
        $this->a = $this->a->toRed($this->red);
        $this->c = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN($conf["c"], 16))->toRed($this->red);
        $this->c2 = $this->c->redSqr();
        $this->d = (new \Ethereumico\EthereumWallet\Dependencies\BN\BN($conf["d"], 16))->toRed($this->red);
    ...
    {
        $y = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($y, 16);
        if (!$y->red) {
            $y = $y->toRed($this->red);
        }
        // x^2 = (y^2 - 1) / (d y^2 + 1)
    ...
    {
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\EdwardsCurve\Point::fromJSON($this, $obj);
    }
    public function point($x = null, $y = null, $z = null, $t = null)
    {
        return new \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curve\EdwardsCurve\Point($this, $x, $y, $z, $t);
    }
}
