...
namespace Ethereumico\EthereumWallet\Dependencies\Elliptic\EC;
...
class KeyPair
{
    ...
    {
        if ($pub instanceof \Ethereumico\EthereumWallet\Dependencies\Elliptic\EC\KeyPair) {
            return $pub;
        }
        return new \Ethereumico\EthereumWallet\Dependencies\Elliptic\EC\KeyPair($ec, array("pub" => $pub, "pubEnc" => $enc));
    }
    public static function fromPrivate($ec, $priv, $enc)
    {
        if ($priv instanceof \Ethereumico\EthereumWallet\Dependencies\Elliptic\EC\KeyPair) {
            return $priv;
        }
        return new \Ethereumico\EthereumWallet\Dependencies\Elliptic\EC\KeyPair($ec, array("priv" => $priv, "privEnc" => $enc));
    }
    public function validate()
    {
        ...
        return array("result" => \true, "reason" => null);
    }
    ...
    {
        $this->priv = new \Ethereumico\EthereumWallet\Dependencies\BN\BN($key, isset($enc) && $enc ? $enc : 16);
    ...
    {
        return "<Key priv: " . (isset($this->priv) ? $this->priv->toString(16, 2) : "") . " pub: " . (isset($this->pub) ? $this->pub->inspect() : "") . ">";
    }
