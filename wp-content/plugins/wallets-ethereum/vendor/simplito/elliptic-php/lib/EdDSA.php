...
use Ethereumico\EthereumWallet\Dependencies\Elliptic\EdDSA\KeyPair;
use Ethereumico\EthereumWallet\Dependencies\Elliptic\EdDSA\Signature;
class EdDSA
{
    public $curve;
    public $g;
    ...
    function __construct($curve)
    {
        ...
        $curve = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Curves::getCurve($curve)->curve;
        $this->curve = $curve;
        $this->g = $curve->g;
    ...
    {
        $message = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::parseBytes($message);
    ...
    public function hashInt()
    {
        ...
        for ($i = 0; $i < \count($arguments); $i++) {
            \hash_update($hash, \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::toBin($arguments[$i]));
        }
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::intFromLE(\hash_final($hash))->umod($this->curve->n);
    }
    public function keyFromPublic($pub)
    {
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\EdDSA\KeyPair::fromPublic($this, $pub);
    }
    public function keyFromSecret($secret)
    {
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\EdDSA\KeyPair::fromSecret($this, $secret);
    }
    public function makeSignature($sig)
    {
        if ($sig instanceof \Ethereumico\EthereumWallet\Dependencies\Elliptic\EdDSA\Signature) {
            return $sig;
        }
        return new \Ethereumico\EthereumWallet\Dependencies\Elliptic\EdDSA\Signature($this, $sig);
    }
    /**
    ...
    {
        $bytes = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::parseBytes($bytes);
        $lastIx = \count($bytes) - 1;
        $normed = $bytes;
        $normed[$lastIx] = $bytes[$lastIx] & ~0x80;
        $xIsOdd = ($bytes[$lastIx] & 0x80) !== 0;
        $y = \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::intFromLE($normed);
    ...
    {
        return \Ethereumico\EthereumWallet\Dependencies\Elliptic\Utils::intFromLE($bytes);
    }
