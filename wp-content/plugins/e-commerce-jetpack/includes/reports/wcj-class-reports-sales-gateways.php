...
class WCJ_Reports_Product_Sales_Gateways {
		...
		while( true ) {
			...
			foreach ( $loop_orders->posts as $order_id ) {
				if ( '' == ( $payment_gateway = get_post_meta( $order_id, '_payment_method_title', true ) ) ) {
					$payment_gateway = __( 'N/A', 'e-commerce-jetpack' );
				}
	...
	function output_report_results() {
		if ( ! empty( $this->gateways ) ) {
			$table_data = array();
			$table_data[] = array(
				'<strong><em>' . __( 'Gateway', 'e-commerce-jetpack' ) . '</em></strong>', '<strong><em>' . __( 'Orders', 'e-commerce-jetpack' ) . '</em></strong>' );
			...
			$result = wcj_get_table_html( $table_data, array( 'table_class' => 'widefat striped', 'table_heading_type' => 'none' ) ) .
				'<p><em>' . sprintf( __( 'Total orders: %d', 'e-commerce-jetpack' ), array_sum( $this->gateways ) ) . '</em></p>';
		} else {
			$result = '<p><em>' . __( 'No sales data for current period.', 'e-commerce-jetpack' ) . '</em></p>';
		}
		return $result;
	}
}
