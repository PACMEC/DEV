...
foreach ( $products as $product_id => $desc ) {
	$product_options = array(
		array(
			...
			'default'    => 0,
			'type'       => 'price',
			'title'      => __( 'Product cost (purchase) price', 'e-commerce-jetpack' ) . ' (' . get_woocommerce_currency_symbol() . ')',
			'desc'       => $desc,
		...
		array(
			...
			'default'    => 0,
			'type'       => 'price',
			'title'      => __( 'Extra expenses (shipping etc.)', 'e-commerce-jetpack' ) . ' (' . get_woocommerce_currency_symbol() . ')',
			'desc'       => $desc,
			'product_id' => $product_id,
		...
		array(
			...
			'default'    => 0,
			'type'       => 'price',
			'title'      => __( 'Affiliate commission', 'e-commerce-jetpack' ) . ' (' . get_woocommerce_currency_symbol() . ')',
			'desc'       => $desc,
			'product_id' => $product_id,
	...
	$product_options = array_merge( $product_options, array(
		array(
			...
			'default'    => '',
			'type'       => 'date',
			'title'      => '<em>' . __( '(Last) Purchase date', 'e-commerce-jetpack' ) . '</em>',
			'desc'       => $desc,
			'product_id' => $product_id,
		...
		array(
			...
			'default'    => '',
			'type'       => 'text',
			'title'      => '<em>' . __( 'Seller', 'e-commerce-jetpack' ) . '</em>',
			'desc'       => $desc,
			'product_id' => $product_id,
		...
		array(
			...
			'default'    => '',
			'type'       => 'textarea',
			'title'      => '<em>' . __( 'Purchase info', 'e-commerce-jetpack' ) . '</em>',
			'desc'       => $desc,
			'product_id' => $product_id,
