...
class EC
{
    ...
    public function genKeyPair($options = null)
    {
        ...
        $drbg = new HmacDRBG(array("hash" => $this->hash, "pers" => isset($options["pers"]) ? $options["pers"] : "", "entropy" => isset($options["entropy"]) ? $options["entropy"] : Utils::randBytes($this->hash["hmacStrength"]), "nonce" => $this->n->toArray()));
    ...
    public function sign($msg, $key, $enc = null, $options = null)
    {
        ...
        if (isset($options["k"])) {
            ...
            // Instatiate HmacDRBG
            $drbg = new HmacDRBG(array("hash" => $this->hash, "entropy" => $bkey, "nonce" => $nonce, "pers" => isset($options["pers"]) ? $options["pers"] : "", "persEnc" => isset($options["persEnc"]) ? $options["persEnc"] : \false));
        ...
        for ($iter = 0; \true; $iter++) {
            ...
            return new Signature(array("r" => $r, "s" => $s, "recoveryParam" => $recoveryParam));
        }
    }
