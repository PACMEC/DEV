...
class OutputClassifier
{
    ...
    private function decodeP2PKH(array $decoded)
    {
        ...
        if ($dup->getOp() === Opcodes::OP_DUP && $hash->getOp() === Opcodes::OP_HASH160 && $buf->isPush() && $buf->getDataSize() === 20 && $eq->getOp() === Opcodes::OP_EQUALVERIFY && $checksig->getOp() === Opcodes::OP_CHECKSIG) {
    ...
    private function decodeMultisig(array $decoded)
    {
        ...
        if ($mOp->getOp() >= Opcodes::OP_0 && $nOp->getOp() <= Opcodes::OP_16 && $checksig->getOp() === Opcodes::OP_CHECKMULTISIG) {
            return $solutions;
        }
        return \false;
    }
    ...
    {
        if (\count($decoded) === 2 && $decoded[0]->getOp() === Opcodes::OP_0 && $decoded[1]->isPush() && $decoded[1]->getDataSize() === 20) {
            return $decoded[1]->getData();
        }
        return \false;
    }
    ...
    {
        if (\count($decoded) === 2 && $decoded[0]->getOp() === Opcodes::OP_0 && $decoded[1]->isPush() && $decoded[1]->getDataSize() === 32) {
            return $decoded[1]->getData();
        }
        return \false;
    }
    ...
    private function decodeWitnessCoinbaseCommitment(array $decoded)
    {
        ...
        if ($decoded[1]->isPush()) {
            ...
            if ($decoded[1]->getDataSize() === 0x24 && \substr($data, 0, 4) === "\xaa!\xa9\xed") {
