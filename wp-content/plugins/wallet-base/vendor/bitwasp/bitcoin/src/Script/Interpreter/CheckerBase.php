...
abstract class CheckerBase
{
    ...
    {
        $this->sigSerializer = $sigSerializer ?: new TransactionSignatureSerializer(EcSerializer::getSerializer(DerSignatureSerializerInterface::class, \true, $ecAdapter));
     ...
     */
    public abstract function getSigHash(ScriptInterface $script, int $hashType, int $sigVersion) : BufferInterface;
    /**
    ...
    public function checkLockTime(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Interpreter\Number $scriptLockTime) : bool
    {
        ...
        if (!($txLockTime < Locktime::BLOCK_MAX && $nLockTime < Locktime::BLOCK_MAX || $txLockTime >= Locktime::BLOCK_MAX && $nLockTime >= Locktime::BLOCK_MAX)) {
            return \false;
        }
    ...
    public function checkSequence(\Ethereumico\EthereumWallet\Dependencies\BitWasp\Bitcoin\Script\Interpreter\Number $sequence) : bool
    {
        ...
        if (!($txToSequenceMasked < TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG && $nSequenceMasked < TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG || $txToSequenceMasked >= TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG && $nSequenceMasked >= TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG)) {
            return \false;
        }
