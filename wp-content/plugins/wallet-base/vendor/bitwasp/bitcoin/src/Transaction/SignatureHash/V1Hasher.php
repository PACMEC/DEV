...
class V1Hasher extends SigHash
{
     ...
     */
    public function __construct(TransactionInterface $transaction, int $amount, OutPointSerializerInterface $outpointSerializer = null, TransactionOutputSerializer $outputSerializer = null)
    {
        $this->amount = $amount;
    ...
    public function calculate(ScriptInterface $txOutScript, int $inputToSign, int $sighashType = SigHash::ALL) : BufferInterface
    {
        ...
        $preimage = new Buffer(\pack("V", $this->tx->getVersion()) . $hashPrevOuts->getBinary() . $hashSequence->getBinary() . $this->outpointSerializer->serialize($input->getOutPoint())->getBinary() . Buffertools::numToVarInt($scriptBuf->getSize())->getBinary() . $scriptBuf->getBinary() . \pack("P", $this->amount) . \pack("V", $input->getSequence()) . $hashOutputs->getBinary() . \pack("V", $this->tx->getLockTime()) . \pack("V", $sighashType));
