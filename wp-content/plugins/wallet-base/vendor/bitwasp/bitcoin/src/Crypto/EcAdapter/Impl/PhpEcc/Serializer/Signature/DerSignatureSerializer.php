...
class DerSignatureSerializer implements DerSignatureSerializerInterface
{
    ...
    public function serialize(SignatureInterface $signature) : BufferInterface
    {
        ...
        $inner = \sprintf("\x02%s%s\x02%s%s", Buffertools::numToVarIntBin(\strlen($rBin)), $rBin, Buffertools::numToVarIntBin(\strlen($sBin)), $sBin);
        return new Buffer(\sprintf("0%s%s", Buffertools::numToVarIntBin(\strlen($inner)), $inner));
    }
    /**
     * @param Parser $parser
