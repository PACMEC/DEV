...
class RawExtendedKeySerializer
{
    ...
    {
        return new Buffer(\pack("H*", $keyParams->getPrefix()) . $this->uint8->write($keyParams->getDepth()) . $this->uint32->write($keyParams->getParentFingerprint()) . $this->uint32->write($keyParams->getSequence()) . $this->bytestring32->write($keyParams->getChainCode()) . $this->bytestring33->write($keyParams->getKeyData()));
    }
    /**
     * @param Parser $parser
     * @return RawKeyParams
    ...
    {
        try {
            return new RawKeyParams($this->bytestring4->read($parser)->getHex(), (int) $this->uint8->read($parser), (int) $this->uint32->read($parser), (int) $this->uint32->read($parser), $this->bytestring32->read($parser), $this->bytestring33->read($parser));
