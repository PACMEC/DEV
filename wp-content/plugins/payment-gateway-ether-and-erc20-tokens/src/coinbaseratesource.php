...
class CoinbaseRateSource {
	...
	/**
	 * The https://coinbase.com API Key
	 *
	 * @var string
	 */
	private $apiKey;
	...
	public function get_rate($eth_value) {
		...
		set_transient( $transient_key, $rate
            , apply_filters( 'epg_token_exchange_rate_cache_duration_CoinbaseRateSource', 1800 ) );
	...
	private function get_rate_from_api() {
        ...
        $url = 'https://api.coinbase.com/v2/exchange-rates?currency=' . strtoupper($this->destination);
        $args = array(
            'timeout'     => 5,
            'redirection' => 5,
            'httpversion' => '1.1',
//            'user-agent'  => 'WordPress/' . $wp_version . '; ' . home_url(),
        ...
        if ( ! isset( $body->{'data'}->{'rates'} ) ) {
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('Could not fetch ETH token pricing - missing rates after decoding.');
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('URL: ' . $url);
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('args: ' . print_r($args, true));
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('response: ' . print_r($response, true));
			throw new \Exception( 'Could not fetch ETH token pricing - missing id after decoding.' );
		}
        if ( ! isset( $body->{'data'}->{'rates'}->{$source} ) ) {
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('Could not fetch ETH token pricing - missing token symbol after decoding.');
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('URL: ' . $url);
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('args: ' . print_r($args, true));
            $GLOBALS['ether-and-erc20-tokens-woocommerce-payment-gateway']->log('response: ' . print_r($response, true));
			throw new \Exception( 'Could not fetch ETH token pricing - missing quote after decoding.' );
		}
        return 1.0 / ((float)$body->{'data'}->{'rates'}->{$source});
	}
}
