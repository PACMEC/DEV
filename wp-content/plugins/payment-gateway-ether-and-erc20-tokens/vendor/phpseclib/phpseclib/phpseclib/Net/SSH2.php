 ...
 * No compression
 */
\define('Ethereumico\\Epg\\Dependencies\\NET_SSH2_COMPRESSION_NONE', 1);
/**
 * zlib compression
 */
\define('Ethereumico\\Epg\\Dependencies\\NET_SSH2_COMPRESSION_ZLIB', 2);
/**
 * zlib@openssh.com
 */
\define('Ethereumico\\Epg\\Dependencies\\NET_SSH2_COMPRESSION_ZLIB_AT_OPENSSH', 3);
/**#@-*/
/**
 * Pure-PHP implementation of SSHv2.
 *
...
class SSH2
{
                    ...
                    switch ($method) {
                        case 'publickey':
                            foreach ($args as $key => $arg) {
                                if ($arg instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Common\PrivateKey || $arg instanceof \Ethereumico\Epg\Dependencies\phpseclib3\System\SSH\Agent) {
                                    $newargs[] = $arg;
                                    unset($args[$key]);
                                    break;
                                }
    ...
    private function get_binary_packet($skip_channel_filter = \false)
    {
        ...
        if (!\is_resource($this->fsock) || \feof($this->fsock)) {
            $this->bitmap = 0;
            throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\ConnectionClosedException('Connection closed (by server) prematurely ' . $elapsed . 's');
        }
        $start = \microtime(\true);
        $raw = \stream_get_contents($this->fsock, $this->decrypt_block_size);
        if (!\strlen($raw)) {
            $this->bitmap = 0;
            throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\ConnectionClosedException('No data received from server');
        }
        if ($this->decrypt) {
