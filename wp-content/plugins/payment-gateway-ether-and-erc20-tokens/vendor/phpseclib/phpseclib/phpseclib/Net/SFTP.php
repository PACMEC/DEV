...
class SFTP extends \Ethereumico\Epg\Dependencies\phpseclib3\Net\SSH2
{
    ...
    private function partial_init_sftp_connection()
    {
        ...
        list($this->defaultVersion) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        ...
        if (isset($this->extensions['versions']) && (!$this->preferredVersion || $this->preferredVersion != $this->version)) {
            ...
            foreach ($supported as $ver) {
                if (\in_array($ver, $versions)) {
                    ...
                    $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', 'version-select', "{$ver}");
        ...
        while (\true) {
            ...
            switch ($this->packet_type) {
                case NET_SFTP_NAME:
                    ...
                    for ($i = 0; $i < $count; $i++) {
                        list($shortname) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                        // SFTPv4 "removed the long filename from the names structure-- it can now be
                        //         built from information available in the attrs structure."
                        if ($this->version < 4) {
                            list($longname) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                        }
    ...
    {
        $attr = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('NQ', NET_SFTP_ATTR_SIZE, $new_size);
    ...
    public function touch($filename, $time = null, $atime = null)
    {
        ...
        $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_ACCESSTIME, $atime, $time) : \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('NQ2', NET_SFTP_ATTR_ACCESSTIME | NET_SFTP_ATTR_MODIFYTIME, $atime, $time);
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $filename);
    ...
    public function chown($filename, $uid, $recursive = \false)
    {
        ...
        */
        $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_UIDGID, $uid, -1) : \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Nss', NET_SFTP_ATTR_OWNERGROUP, $uid, '');
    ...
    {
        $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_UIDGID, $gid, -1) : \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Nss', NET_SFTP_ATTR_OWNERGROUP, '', $gid);
        return $this->setstat($filename, $attr, $recursive);
    }
    /**
    ...
    private function setstat_recursive($path, $attr, &$i)
    {
        ...
        foreach ($entries as $filename => $props) {
            ...
            if ($props['type'] == NET_SFTP_TYPE_DIRECTORY) {
                ...
                $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $temp);
        ...
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $path);
    ...
    public function symlink($target, $link)
    {
        ...
        if ($this->version == 6) {
            $type = NET_SFTP_LINK;
            $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ssC', $link, $target, 1);
        } else {
            ...
            /* quoting http://bxr.su/OpenBSD/usr.bin/ssh/PROTOCOL#347 :
                           ...
                           SSH_FXP_SYMLINK as follows:
                               ...
                               string      targetpath
                               string      linkpath */
            $packet = \substr($this->server_identifier, 0, 15) == 'SSH-2.0-OpenSSH' ? \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $target, $link) : \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $link, $target);
        }
        $this->send_sftp_packet($type, $packet);
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
    ...
    public function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null)
    {
        ...
        $this->remove_from_stat_cache($remote_file);
        $packet = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $remote_file);
        ...
        if ($mode & \Ethereumico\Epg\Dependencies\phpseclib3\Net\SFTP::SOURCE_LOCAL_FILE) {
            if (isset($fp) && \is_resource($fp)) {
                \fclose($fp);
            }
            if ($this->preserveTime) {
                $stat = \stat($data);
                $attr = $this->version < 4 ? \pack('N3', NET_SFTP_ATTR_ACCESSTIME, $stat['atime'], $stat['time']) : \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('NQ2', NET_SFTP_ATTR_ACCESSTIME | NET_SFTP_ATTR_MODIFYTIME, $stat['atime'], $stat['time']);
    ...
    {
        $attr = [];
        list($attr[$key]) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Q', $response);
        if ($flags & NET_SFTP_ATTR_SUBSECOND_TIMES) {
            list($attr[key . '-nseconds']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
        }
        return $attr;
    }
    /**
    ...
    {
        if ($this->version >= 4) {
            list($flags, $attr['type']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NC', $response);
        } else {
        ...
        foreach ($this->attributes as $key => $value) {
            ...
            switch ($flags & $key) {
                case NET_SFTP_ATTR_SIZE:
                    ...
                    list($attr['size']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Q', $response);
                    break;
                case NET_SFTP_ATTR_UIDGID:
                ...
                case NET_SFTP_ATTR_ACL:
                    ...
                    list($count) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
                    for ($i = 0; $i < $count; $i++) {
                        list($type, $flag, $mask, $who) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N3s', $result);
                    }
                    break;
                case NET_SFTP_ATTR_OWNERGROUP:
                    // 0x00000080
                    list($attr['owner'], $attr['$group']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $response);
                    break;
                case NET_SFTP_ATTR_SUBSECOND_TIMES:
                    // 0x00000100
                    break;
                case NET_SFTP_ATTR_BITS:
                    ...
                    list($attrib_bits, $attrib_bits_valid) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N2', $response);
                ...
                case NET_SFTP_ATTR_ALLOCATION_SIZE:
                    ...
                    list($attr['allocation-size']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpack('Q', $response);
                    break;
                case NET_SFTP_ATTR_TEXT_HINT:
                    // 0x00000800
                    ...
                    list($text_hint) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('C', $response);
                    // the above should be $attr['text-hint']
                    break;
                case NET_SFTP_ATTR_MIME_TYPE:
                    // 0x00001000
                    // see https://datatracker.ietf.org/doc/html/draft-ietf-secsh-filexfer-13#section-7.11
                    list($attr['mime-type']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                    break;
                case NET_SFTP_ATTR_LINK_COUNT:
                    // 0x00002000
                    // see https://datatracker.ietf.org/doc/html/draft-ietf-secsh-filexfer-13#section-7.12
                    list($attr['link-count']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSS2('N', $response);
                    break;
                case NET_SFTP_ATTR_UNTRANSLATED_NAME:
                    // 0x00004000
                    // see https://datatracker.ietf.org/doc/html/draft-ietf-secsh-filexfer-13#section-7.13
                    list($attr['untranslated-name']) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                    break;
                case NET_SFTP_ATTR_CTIME:
                    // 0x00008000
