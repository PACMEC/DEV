 ...
 */
namespace Ethereumico\Epg\Dependencies\phpseclib3\Net\SFTP;

use Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Common\PrivateKey;
use Ethereumico\Epg\Dependencies\phpseclib3\Net\SFTP;
use Ethereumico\Epg\Dependencies\phpseclib3\Net\SSH2;
/**
 * SFTP Stream Wrapper
 *
 * @package SFTP
...
class Stream
{
    ...
    protected function parse_path($path)
    {
        ...
        if (\preg_match('/^{[a-z0-9]+}$/i', $host)) {
            $host = \Ethereumico\Epg\Dependencies\phpseclib3\Net\SSH2::getConnectionByResourceId($host);
            if ($host === \false) {
                return \false;
            }
            $this->sftp = $host;
            ...
            if (isset($sftp) && $sftp instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Net\SFTP) {
                $this->sftp = $sftp;
                return $path;
            }
            ...
            if (isset($context[$scheme]['privkey']) && $context[$scheme]['privkey'] instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Common\PrivateKey) {
            ...
            if (isset(self::$instances[$host][$port][$user][(string) $pass])) {
                ...
                $this->sftp = new \Ethereumico\Epg\Dependencies\phpseclib3\Net\SFTP($host, $port);
    ...
    private function _stream_write($data)
    {
        ...
        $result = $this->sftp->put($this->path, $data, \Ethereumico\Epg\Dependencies\phpseclib3\Net\SFTP::SOURCE_STRING, $this->pos);
        if (isset($this->notification) && \is_callable($this->notification)) {
            if (!$result) {
                \call_user_func($this->notification, \STREAM_NOTIFY_FAILURE, \STREAM_NOTIFY_SEVERITY_ERR, $this->sftp->getLastSFTPError(), NET_SFTP_OPEN, 0, 0);
                return 0;
