...
use Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\PrivateKey;
use Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\PublicKey;
use Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\Parameters;
 ...
 * @access  public
 */
abstract class DH extends \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Common\AsymmetricKey
{
    /**
     * Algorithm Name
     *
    ...
    {
        $params = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\Parameters();
        if (\count($args) == 2 && $args[0] instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger && $args[1] instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger) {
            //if (!$args[0]->isPrime()) {
            ...
            $params->prime = \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger::randomPrime($args[0]);
            $params->base = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger(2);
            return $params;
        ...
        $params->prime = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($prime, 16);
        $params->base = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger(2);
        return $params;
    }
    /**
     ...
     */
    public static function createKey(\Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\Parameters $params, $length = 0)
    {
        ...
        $key = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\PrivateKey();
        $key->prime = $params->prime;
        $key->base = $params->base;
        $key->privateKey = \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger::randomRange($one, $max);
    ...
    {
        if ($private instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\PrivateKey) {
            // DH\PrivateKey
            switch (\true) {
                case $public instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\PublicKey:
                ...
                case \is_string($public):
                    $public = new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($public, -256);
                case $public instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger:
        ...
        if ($private instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\PrivateKey) {
            switch (\true) {
                case $public instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\PublicKey:
    ...
    protected static function onLoad($components)
    {
        if (!isset($components['privateKey']) && !isset($components['publicKey'])) {
            $new = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\Parameters();
        } else {
            $new = isset($components['privateKey']) ? new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\PrivateKey() : new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DH\PublicKey();
        }
    ...
    {
        throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedOperationException('DH does not use a hash algorithm');
    }
    /**
