 ...
 */
namespace Ethereumico\Epg\Dependencies\phpseclib3\Crypt\RSA;
...
use Ethereumico\Epg\Dependencies\phpseclib3\File\ASN1\Maps\DigestInfo;
 ...
 * @access  public
 */
class PublicKey extends \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\RSA implements \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Common\PublicKey
{
     ...
     */
    private function exponentiate(\Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger $x)
    {
    ...
    private function rsassa_pkcs1_v1_5_verify($m, $s)
    {
        ...
        try {
            ...
            $exception = \true;
        } catch (\Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException $e) {
            $r2 = \false;
        }
        if ($exception) {
    ...
    private function rsassa_pkcs1_v1_5_relaxed_verify($m, $s)
    {
        ...
        if (\Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::shift($em, 2) != "\0\1") {
            return \false;
        }
        $em = \ltrim($em, "ÿ");
        if (\Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::shift($em) != "\0") {
            return \false;
        }
        $decoded = \Ethereumico\Epg\Dependencies\phpseclib3\File\ASN1::decodeBER($em);
        ...
        if (!isset($oids)) {
            ...
            \Ethereumico\Epg\Dependencies\phpseclib3\File\ASN1::loadOIDs($oids);
        }
        $decoded = \Ethereumico\Epg\Dependencies\phpseclib3\File\ASN1::asn1map($decoded[0], \Ethereumico\Epg\Dependencies\phpseclib3\File\ASN1\Maps\DigestInfo::MAP);
        ...
        $hash = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Hash($hash);
        $em = $hash->hash($m);
        $em2 = $decoded['digest'];
    ...
    private function rsaes_pkcs1_v1_5_encrypt($m, $pkcs15_compat = \false)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Random::string($psLen - \strlen($ps));
    ...
    private function rsaes_oaep_encrypt($m)
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Random::string($this->hLen);
    ...
    public function toString($type, array $options = [])
    {
        ...
        if ($type == \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
            if ($this->signaturePadding == self::SIGNATURE_PSS) {
                ...
                throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedFormatException('The PSS format can only be used when the signature method has been explicitly set to PSS');
            }
        }
    ...
    {
        $new = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\RSA\PrivateKey();
