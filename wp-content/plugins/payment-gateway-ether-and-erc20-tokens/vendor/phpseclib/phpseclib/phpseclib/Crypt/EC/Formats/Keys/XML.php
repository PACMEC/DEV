...
abstract class XML
{
    ...
    {
        return \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Base64::decode(\str_replace(["\r", "\n", ' ', "\t"], '', $value));
    }
    /**
     ...
     * @return object[]
     */
    private static function extractPointRFC4050(\DOMXPath $xpath, \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
        ...
        $point = [$curve->convertInteger(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($x->item(0)->getAttribute('Value'))), $curve->convertInteger(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($y->item(0)->getAttribute('Value')))];
    ...
    private static function loadCurveByParam(\DOMXPath $xpath)
    {
        ...
        if ($namedCurve->length == 1) {
            ...
            if ($name === \false) {
                throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Curve with OID of ' . $oid . ' is not supported');
            }
            $curve = '\\phpseclib3\\Crypt\\EC\\Curves\\' . $name;
            if (!\class_exists($curve)) {
                throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $name . ' is not supported');
            }
            return new $curve();
        }
        ...
        switch ($type) {
            case 'prime-field':
                ...
                $curve->setModulo(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($p, 256));
                $curve->setCoefficients(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($a, 256), new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($b, 256));
                $point = self::extractPoint("\0" . $base, $curve);
                $curve->setBasePoint(...$point);
                $curve->setOrder(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($order, 256));
                return $curve;
            case 'gnb':
            case 'tnb':
            case 'pnb':
            default:
                throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $type . ' is not supported');
        }
    }
    /**
     * Returns an instance of \phpseclib3\Crypt\EC\BaseCurves\Base based
    ...
    private static function loadCurveByParamRFC4050(\DOMXPath $xpath)
    {
        ...
        switch ($type) {
            case 'prime-field':
                $curve = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Prime();
                $p = \str_replace(["\r", "\n", ' ', "\t"], '', $p);
                $curve->setModulo(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($p));
                $a = \str_replace(["\r", "\n", ' ', "\t"], '', $a);
                $b = \str_replace(["\r", "\n", ' ', "\t"], '', $b);
                $curve->setCoefficients(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($a), new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($b));
                $x = \str_replace(["\r", "\n", ' ', "\t"], '', $x);
                $y = \str_replace(["\r", "\n", ' ', "\t"], '', $y);
                $curve->setBasePoint(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($x), new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($y));
                $order = \str_replace(["\r", "\n", ' ', "\t"], '', $order);
                $curve->setOrder(new \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger($order));
                return $curve;
            default:
                throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $type . ' is not supported');
        }
    ...
    public static function savePublicKey(\Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, array $options = [])
    {
        ...
        return '<' . $pre . 'ECDSAKeyValue xmlns' . $post . '="http://www.w3.org/2009/xmldsig11#">' . "\r\n" . self::encodeXMLParameters($curve, $pre, $options) . "\r\n" . '<' . $pre . 'PublicKey>' . \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Base64::encode($publicKey) . '</' . $pre . 'PublicKey>' . "\r\n" . '</' . $pre . 'ECDSAKeyValue>';
    }
    /**
     * Encode Parameters
     *
     ...
     * @return string|false
     */
    private static function encodeXMLParameters(\Ethereumico\Epg\Dependencies\phpseclib3\Crypt\EC\BaseCurves\Base $curve, $pre, array $options = [])
    {
        ...
        if (self::$rfc4050) {
            ...
            switch ($temp['fieldID']['fieldType']) {
                case 'prime-field':
                    ...
                    break;
                default:
                    throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $temp['fieldID']['fieldType'] . ' is not supported');
            }
        ...
        if (isset($result['specifiedCurve'])) {
            ...
            switch ($temp['fieldID']['fieldType']) {
                case 'prime-field':
                    $xml .= '<' . $pre . 'Prime>' . "\r\n" . '<' . $pre . 'P>' . \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Base64::encode($temp['fieldID']['parameters']->toBytes()) . '</' . $pre . 'P>' . "\r\n" . '</' . $pre . 'Prime>' . "\r\n";
                    break;
                default:
                    throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $temp['fieldID']['fieldType'] . ' is not supported');
            }
            $xml .= '</' . $pre . 'FieldID>' . "\r\n" . '<' . $pre . 'Curve>' . "\r\n" . '<' . $pre . 'A>' . \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Base64::encode($temp['curve']['a']) . '</' . $pre . 'A>' . "\r\n" . '<' . $pre . 'B>' . \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Base64::encode($temp['curve']['b']) . '</' . $pre . 'B>' . "\r\n" . '</' . $pre . 'Curve>' . "\r\n" . '<' . $pre . 'Base>' . \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Base64::encode($temp['base']) . '</' . $pre . 'Base>' . "\r\n" . '<' . $pre . 'Order>' . \Ethereumico\Epg\Dependencies\ParagonIE\ConstantTime\Base64::encode($temp['order']) . '</' . $pre . 'Order>' . "\r\n" . '</' . $pre . 'ECParameters>';
            return $xml;
        }
    }
}
