...
use Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\PrivateKey;
use Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\PublicKey;
use Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\Parameters;
 ...
 * @access  public
 */
abstract class DSA extends \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\Common\AsymmetricKey
{
    /**
     * Algorithm Name
     *
    ...
    public static function createParameters($L = 2048, $N = 224)
    {
        ...
        $q = \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger::randomPrime($N);
        $divisor = $q->multiply($two);
        do {
            $x = \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger::random($L);
        ...
        $dsa = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\Parameters();
        $dsa->p = $p;
        $dsa->q = $q;
        $dsa->g = $g;
        return $dsa;
    ...
    public static function createKey(...$args)
    {
        ...
        if (\count($args) == 2 && \is_int($args[0]) && \is_int($args[1])) {
            ...
            if (\count($args) == 1 && $args[0] instanceof \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\Parameters) {
                $params = $args[0];
            } else {
                if (!\count($args)) {
                    $params = self::createParameters();
                } else {
                    throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\InsufficientSetupException('Valid parameters are either two integers (L and N), a single DSA object or no parameters at all.');
                }
            }
        }
        $private = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\PrivateKey();
        $private->p = $params->p;
        $private->q = $params->q;
        $private->g = $params->g;
        $private->x = \Ethereumico\Epg\Dependencies\phpseclib3\Math\BigInteger::randomRange(self::$one, $private->q->subtract(self::$one));
    ...
    protected static function onLoad($components)
    {
        ...
        if (!isset($components['x']) && !isset($components['y'])) {
            $new = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\Parameters();
        } else {
            if (isset($components['x'])) {
                $new = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\PrivateKey();
                $new->x = $components['x'];
            } else {
                $new = new \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA\PublicKey();
            }
        }
        $new->p = $components['p'];
        $new->q = $components['q'];
    ...
    public function getParameters()
    {
        ...
        return \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\DSA::load($key, 'PKCS1')->withHash($this->hash->getHash())->withSignatureFormat($this->shortFormat);
    }
    /**
