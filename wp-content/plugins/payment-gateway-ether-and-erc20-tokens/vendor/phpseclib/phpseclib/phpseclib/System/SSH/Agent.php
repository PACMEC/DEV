 ...
 */
namespace Ethereumico\Epg\Dependencies\phpseclib3\System\SSH;
...
use Ethereumico\Epg\Dependencies\phpseclib3\System\SSH\Agent\Identity;
...
class Agent
{
        ...
        if (!$address) {
            switch (\true) {
                ...
                case isset($_ENV['SSH_AUTH_SOCK']):
                    ...
                    break;
                default:
                    throw new \Ethereumico\Epg\Dependencies\phpseclib3\Exception\BadConfigurationException('SSH_AUTH_SOCK not found');
            }
        }
    ...
    public function requestIdentities()
    {
        ...
        list($type, $keyCount) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('CN', $packet);
        if ($type != self::SSH_AGENT_IDENTITIES_ANSWER) {
            throw new \RuntimeException('Unable to request identities');
        }
        $identities = [];
        for ($i = 0; $i < $keyCount; $i++) {
            list($key_blob, $comment) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $packet);
            $temp = $key_blob;
            list($key_type) = \Ethereumico\Epg\Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $temp);
            switch ($key_type) {
                case 'ssh-rsa':
                case 'ssh-dss':
                case 'ssh-ed25519':
                ...
                case 'ecdsa-sha2-nistp521':
                    $key = \Ethereumico\Epg\Dependencies\phpseclib3\Crypt\PublicKeyLoader::load($key_type . ' ' . \base64_encode($key_blob));
            }
            // resources are passed by reference by default
            if (isset($key)) {
                $identity = (new \Ethereumico\Epg\Dependencies\phpseclib3\System\SSH\Agent\Identity($this->fsock))->withPublicKey($key)->withPublicKeyBlob($key_blob);
                $identities[] = $identity;
                unset($key);
            }
        }
