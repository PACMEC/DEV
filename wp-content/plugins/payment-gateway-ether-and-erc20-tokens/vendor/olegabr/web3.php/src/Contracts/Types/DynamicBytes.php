...
class DynamicBytes extends \Ethereumico\Epg\Dependencies\Web3\Contracts\SolidityType implements \Ethereumico\Epg\Dependencies\Web3\Contracts\Types\IType
{
    /**
     * construct
     * 
    ...
    {
        if (!\Ethereumico\Epg\Dependencies\Web3\Utils::isHex($value)) {
        ...
        $bn = \Ethereumico\Epg\Dependencies\Web3\Utils::toBn(\floor(\mb_strlen($value) / 2));
        $bnHex = $bn->toHex(\true);
    ...
    public function outputFormat($value, $name)
    {
        ...
        $size = \intval(\Ethereumico\Epg\Dependencies\Web3\Utils::toBn('0x' . \mb_substr($value, 0, 64))->toString());
        $length = 2 * $size;
