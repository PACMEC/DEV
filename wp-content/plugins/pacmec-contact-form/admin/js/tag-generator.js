( function( $ ) {
	...
	$( 'form.tag-generator-panel .control-box :input' ).change( function() {
		...
		pcf.taggen.normalize( $( this ) );
		pcf.taggen.update( $form );
	} );

	$( 'input.insert-tag' ).click( function() {
		var $form = $( this ).closest( 'form.tag-generator-panel' );
		var tag = $form.find( 'input.tag' ).val();
		pcf.taggen.insert( tag );
		tb_remove(); // close thickbox
		return false;
	} );

	pcf.taggen.update = function( $form ) {
		...
		if ( $.isFunction( pcf.taggen.update[ id ] ) ) {
			return pcf.taggen.update[ id ].call( this, $form );
		}
		...
		$form.find( 'input.tag' ).each( function() {
			...
			var components = pcf.taggen.compose( tag_type, $form );
			$( this ).val( components );
		} );
	...
	pcf.taggen.update.captcha = function( $form ) {
		var captchac = pcf.taggen.compose( 'captchac', $form );
		var captchar = pcf.taggen.compose( 'captchar', $form );

		$form.find( 'input.tag' ).val( captchac + ' ' + captchar );
	};

	pcf.taggen.compose = function( tagType, $form ) {
	...
	pcf.taggen.normalize = function( $input ) {
		var val = $input.val();
		...
		if ( $input.is( ':checkbox.exclusive' ) ) {
			pcf.taggen.exclusiveCheckbox( $input );
		}
	};

	pcf.taggen.exclusiveCheckbox = function( $cb ) {
		if ( $cb.is( ':checked' ) ) {
			$cb.siblings( ':checkbox.exclusive' ).prop( 'checked', false );
		}
	};

	pcf.taggen.insert = function( content ) {
		$( 'textarea#pcf-form' ).each( function() {
			this.focus();
