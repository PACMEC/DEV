...
class PCF_Contact_Form_List_Table extends WP_List_Table {
	...
	function prepare_items() {
		...
		$this->items = PCF_ContactForm::find( $args );

		$total_items = PCF_ContactForm::count();
	...
	function column_title( $item ) {
		$url = admin_url( 'admin.php?page=pcf&post=' . absint( $item->id() ) );
		...
		&& current_user_can( 'pcf_edit_contact_form', $item->id() ) ) {
		...
		if ( current_user_can( 'pcf_edit_contact_form', $item->id() ) ) {
			$copy_link = wp_nonce_url(
				add_query_arg( array( 'action' => 'copy' ), $url ),
				'pcf-copy-contact-form_' . absint( $item->id() ) );
