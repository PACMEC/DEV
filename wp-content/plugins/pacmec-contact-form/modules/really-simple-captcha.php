...
add_action( 'pcf_init', 'pcf_add_form_tag_captcha' );

function pcf_add_form_tag_captcha() {
	// CAPTCHA-Challenge (image)
	pcf_add_form_tag( 'captchac',
		'pcf_captchac_form_tag_handler',
		array( 'name-attr' => true )
	);

	// CAPTCHA-Response (input)
	pcf_add_form_tag( 'captchar',
		'pcf_captchar_form_tag_handler',
		array(
			'name-attr' => true,
			'do-not-store' => true,
		)
...
function pcf_captchac_form_tag_handler( $tag ) {
	if ( ! class_exists( 'ReallySimpleCaptcha' ) ) {
		$error = sprintf(
			esc_html( __( "To use CAPTCHA, you need %s plugin installed.", 'contact-form-7' ) ),
			pcf_link( 'https://wordpress.org/plugins/really-simple-captcha/', 'Really Simple CAPTCHA' ) );
	...
	$op = array_merge( $op, pcf_captchac_options( $tag->options ) );

	if ( ! $filename = pcf_generate_captcha( $op ) ) {
		return '';
	}
	...
	$atts['alt'] = 'captcha';
	$atts['src'] = pcf_captcha_url( $filename );
	...
	$html = sprintf(
		'<input type="hidden" name="_pcf_captcha_challenge_%1$s" value="%2$s" /><img %3$s />',
		$tag->name, $prefix, $atts );

	return $html;
}

function pcf_captchar_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'pcf_validate_captchar', 'pcf_captcha_validation_filter', 10, 2 );

function pcf_captcha_validation_filter( $result, $tag ) {
	$type = $tag->type;
	$name = $tag->name;

	$captchac = '_pcf_captcha_challenge_' . $name;

	$prefix = isset( $_POST[$captchac] ) ? (string) $_POST[$captchac] : '';
	$response = isset( $_POST[$name] ) ? (string) $_POST[$name] : '';
	$response = pcf_canonicalize( $response );

	if ( 0 == strlen( $prefix ) || ! pcf_check_captcha( $prefix, $response ) ) {
		$result->invalidate( $tag, pcf_get_message( 'captcha_not_match' ) );
	}

	if ( 0 != strlen( $prefix ) ) {
		pcf_remove_captcha( $prefix );
	}
...
add_filter( 'pcf_ajax_onload', 'pcf_captcha_ajax_refill' );
add_filter( 'pcf_ajax_json_echo', 'pcf_captcha_ajax_refill' );

function pcf_captcha_ajax_refill( $items ) {
	if ( ! is_array( $items ) ) {
		return $items;
	}

	$fes = pcf_scan_form_tags( array( 'type' => 'captchac' ) );
	...
	foreach ( $fes as $fe ) {
		...
		$op = pcf_captchac_options( $options );
		if ( $filename = pcf_generate_captcha( $op ) ) {
			$captcha_url = pcf_captcha_url( $filename );
...
add_filter( 'pcf_messages', 'pcf_captcha_messages' );

function pcf_captcha_messages( $messages ) {
...
add_action( 'pcf_admin_init', 'pcf_add_tag_generator_captcha', 46 );

function pcf_add_tag_generator_captcha() {
	if ( ! pcf_use_really_simple_captcha() ) {
		return;
	}
	...
	$tag_generator->add( 'captcha',
		...
		'pcf_tag_generator_captcha' );
}

function pcf_tag_generator_captcha( $contact_form, $args = '' ) {
...
<fieldset>
<legend><?php echo sprintf( esc_html( __( "To use CAPTCHA, you first need to install and activate %s plugin.", 'contact-form-7' ) ), pcf_link( 'https://wordpress.org/plugins/really-simple-captcha/', 'Really Simple CAPTCHA' ) ); ?></legend>
</fieldset>
</div>
<?php
	...
	$desc_link = pcf_link( __( 'https://contactform7.com/captcha/', 'contact-form-7' ), __( 'CAPTCHA', 'contact-form-7' ) );
...
add_action( 'pcf_admin_warnings', 'pcf_captcha_display_warning_message' );

function pcf_captcha_display_warning_message() {
	...
	$uploads_dir = pcf_captcha_tmp_dir();
	pcf_init_captcha();
...
function pcf_init_captcha() {
	...
	$dir = trailingslashit( pcf_captcha_tmp_dir() );
...
function pcf_captcha_tmp_dir() {
	if ( defined( 'PCF_CAPTCHA_TMP_DIR' ) ) {
		return PCF_CAPTCHA_TMP_DIR;
	} else {
		return pcf_upload_dir( 'dir' ) . '/pcf_captcha';
	}
}

function pcf_captcha_tmp_url() {
	if ( defined( 'PCF_CAPTCHA_TMP_URL' ) ) {
		return PCF_CAPTCHA_TMP_URL;
	} else {
		return pcf_upload_dir( 'url' ) . '/pcf_captcha';
	}
}

function pcf_captcha_url( $filename ) {
	$url = trailingslashit( pcf_captcha_tmp_url() ) . $filename;

	if ( is_ssl() && 'http:' == substr( $url, 0, 5 ) ) {
		$url = 'https:' . substr( $url, 5 );
	}

	return apply_filters( 'pcf_captcha_url', esc_url_raw( $url ) );
}

function pcf_generate_captcha( $options = null ) {
	if ( ! $captcha = pcf_init_captcha() ) {
		return false;
	}
...
function pcf_check_captcha( $prefix, $response ) {
	if ( ! $captcha = pcf_init_captcha() ) {
		return false;
	}
...
function pcf_remove_captcha( $prefix ) {
	if ( ! $captcha = pcf_init_captcha() ) {
		return false;
	}
...
add_action( 'template_redirect', 'pcf_cleanup_captcha_files', 20 );

function pcf_cleanup_captcha_files() {
	if ( ! $captcha = pcf_init_captcha() ) {
		return false;
	}
...
function pcf_captchac_options( $options ) {
	if ( ! is_array( $options ) ) {
		return array();
	}
