...
add_filter( 'pcf_spam', 'pcf_akismet' );

function pcf_akismet( $spam ) {
	if ( $spam ) {
		return $spam;
	}

	if ( ! pcf_akismet_is_available() ) {
		return false;
	}

	if ( ! $params = pcf_akismet_submitted_params() ) {
		return false;
	}
	...
	return pcf_akismet_comment_check( $c );
}

function pcf_akismet_is_available() {
...
function pcf_akismet_submitted_params() {
	$params = array(
		'author' => '',
		'author_email' => '',
		'author_url' => '',
	...
	foreach ( (array) $_POST as $key => $val ) {
		if ( '_pcf' == substr( $key, 0, 6 ) || '_wpnonce' == $key ) {
			continue;
		}

		if ( is_array( $val ) ) {
			$val = implode( ', ', pcf_array_flatten( $val ) );
		}
		...
		if ( $tags = pcf_scan_form_tags( array( 'name' => $key ) ) ) {
			$tag = $tags[0];
...
function pcf_akismet_comment_check( $comment ) {
	global $akismet_api_host, $akismet_api_port;

	$spam = false;
	$query_string = pcf_build_query( $comment );
	...
	return apply_filters( 'pcf_akismet_comment_check', $spam, $comment );
}
