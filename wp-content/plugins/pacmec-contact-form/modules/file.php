...
add_action( 'pcf_init', 'pcf_add_form_tag_file' );

function pcf_add_form_tag_file() {
	pcf_add_form_tag( array( 'file', 'file*' ),
		'pcf_file_form_tag_handler', array( 'name-attr' => true ) );
}

function pcf_file_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'pcf_form_enctype', 'pcf_file_form_enctype_filter' );

function pcf_file_form_enctype_filter( $enctype ) {
	$multipart = (bool) pcf_scan_form_tags( array( 'type' => array( 'file', 'file*' ) ) );
...
add_filter( 'pcf_validate_file', 'pcf_file_validation_filter', 10, 2 );
add_filter( 'pcf_validate_file*', 'pcf_file_validation_filter', 10, 2 );

function pcf_file_validation_filter( $result, $tag ) {
	$name = $tag->name;
	...
	if ( $file['error'] && UPLOAD_ERR_NO_FILE != $file['error'] ) {
		$result->invalidate( $tag, pcf_get_message( 'upload_failed_php_error' ) );
		return $result;
	}
	...
	if ( ! preg_match( $file_type_pattern, $file['name'] ) ) {
		$result->invalidate( $tag, pcf_get_message( 'upload_file_type_invalid' ) );
		return $result;
	}
	...
	if ( $file['size'] > $allowed_size ) {
		$result->invalidate( $tag, pcf_get_message( 'upload_file_too_large' ) );
		return $result;
	}

	pcf_init_uploads(); // Confirm upload dir
	$uploads_dir = pcf_upload_tmp_dir();
	$uploads_dir = pcf_maybe_add_random_dir( $uploads_dir );

	$filename = $file['name'];
	$filename = pcf_canonicalize( $filename, 'as-is' );
	$filename = sanitize_file_name( $filename );
	$filename = pcf_antiscript_file_name( $filename );
	$filename = wp_unique_filename( $uploads_dir, $filename );

	$new_file = trailingslashit( $uploads_dir ) . $filename;

	if ( false === @move_uploaded_file( $file['tmp_name'], $new_file ) ) {
		$result->invalidate( $tag, pcf_get_message( 'upload_failed' ) );
		return $result;
	}
	...
	if ( $submission = PCF_Submission::get_instance() ) {
...
add_filter( 'pcf_messages', 'pcf_file_messages' );

function pcf_file_messages( $messages ) {
...
add_action( 'pcf_admin_init', 'pcf_add_tag_generator_file', 50 );

function pcf_add_tag_generator_file() {
	$tag_generator = PCF_TagGenerator::get_instance();
	$tag_generator->add( 'file', __( 'file', 'contact-form-7' ),
		'pcf_tag_generator_file' );
}

function pcf_tag_generator_file( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );
	$type = 'file';

	$description = __( "Generate a form-tag for a file uploading field. For more details, see %s.", 'contact-form-7' );

	$desc_link = pcf_link( __( 'https://contactform7.com/file-uploading-and-attachment/', 'contact-form-7' ), __( 'File Uploading and Attachment', 'contact-form-7' ) );
...
add_action( 'pcf_admin_warnings', 'pcf_file_display_warning_message' );

function pcf_file_display_warning_message() {
...
function pcf_init_uploads() {
	$dir = pcf_upload_tmp_dir();
	wp_mkdir_p( $dir );
...
function pcf_maybe_add_random_dir( $dir ) {
	do {
...
function pcf_upload_tmp_dir() {
	if ( defined( 'PCF_UPLOADS_TMP_DIR' ) )
		return PCF_UPLOADS_TMP_DIR;
	else
		return pcf_upload_dir( 'dir' ) . '/pcf_uploads';
}

add_action( 'template_redirect', 'pcf_cleanup_upload_files', 20 );

function pcf_cleanup_upload_files( $seconds = 60, $max = 100 ) {
	if ( is_admin() || 'GET' != $_SERVER['REQUEST_METHOD']
	|| is_robots() || is_feed() || is_trackback() ) {
		return;
	}

	$dir = trailingslashit( pcf_upload_tmp_dir() );
