...
add_action( 'pcf_init', 'pcf_add_form_tag_date' );

function pcf_add_form_tag_date() {
	pcf_add_form_tag( array( 'date', 'date*' ),
		'pcf_date_form_tag_handler', array( 'name-attr' => true ) );
}

function pcf_date_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'pcf_validate_date', 'pcf_date_validation_filter', 10, 2 );
add_filter( 'pcf_validate_date*', 'pcf_date_validation_filter', 10, 2 );

function pcf_date_validation_filter( $result, $tag ) {
	$name = $tag->name;
	...
	} elseif ( '' != $value && ! pcf_is_date( $value ) ) {
		$result->invalidate( $tag, pcf_get_message( 'invalid_date' ) );
	} elseif ( '' != $value && ! empty( $min ) && $value < $min ) {
		$result->invalidate( $tag, pcf_get_message( 'date_too_early' ) );
	} elseif ( '' != $value && ! empty( $max ) && $max < $value ) {
		$result->invalidate( $tag, pcf_get_message( 'date_too_late' ) );
	}
...
add_filter( 'pcf_messages', 'pcf_date_messages' );

function pcf_date_messages( $messages ) {
...
add_action( 'pcf_admin_init', 'pcf_add_tag_generator_date', 19 );

function pcf_add_tag_generator_date() {
	$tag_generator = PCF_TagGenerator::get_instance();
	$tag_generator->add( 'date', __( 'date', 'contact-form-7' ),
		'pcf_tag_generator_date' );
}

function pcf_tag_generator_date( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );
	$type = 'date';

	$description = __( "Generate a form-tag for a date input field. For more details, see %s.", 'contact-form-7' );

	$desc_link = pcf_link( __( 'https://contactform7.com/date-field/', 'contact-form-7' ), __( 'Date Field', 'contact-form-7' ) );
