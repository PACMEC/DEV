...
add_action( 'pcf_submit', 'pcf_flamingo_submit', 10, 2 );

function pcf_flamingo_submit( $contactform, $result ) {
	...
	$cases = (array) apply_filters( 'pcf_flamingo_submit_if',
	...
	$email = pcf_flamingo_get_value( 'email', $contactform );
	$name = pcf_flamingo_get_value( 'name', $contactform );
	$subject = pcf_flamingo_get_value( 'subject', $contactform );
	...
	foreach ( $special_mail_tags as $smt ) {
		$meta[$smt] = apply_filters( 'pcf_special_mail_tags',
			'', '_' . $smt, false );
	}
	...
	$channel_id = pcf_flamingo_add_channel(
	...
	do_action( 'pcf_after_flamingo', $result );
}

function pcf_flamingo_get_value( $field, $contactform ) {
	...
	if ( in_array( $field, array( 'email', 'name', 'subject' ) ) ) {
		...
		if ( empty( $templates[0] ) ) {
			...
			$template = trim( pcf_strip_quote( $templates[0] ) );
		}

		$value = pcf_mail_replace_tags( $template );
	}

	$value = apply_filters( 'pcf_flamingo_get_value', $value,
		$field, $contactform );

	return $value;
}

function pcf_flamingo_add_channel( $slug, $name = '' ) {
...
add_filter( 'pcf_special_mail_tags', 'pcf_flamingo_serial_number', 10, 3 );

function pcf_flamingo_serial_number( $output, $name, $html ) {
	...
	if ( ! $contact_form = PCF_ContactForm::get_current() ) {
		return $output;
	}
