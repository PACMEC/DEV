...
class PCF_RECAPTCHA extends PCF_Service {
	...
	private function menu_page_url( $args = '' ) {
		...
		$url = menu_page_url( 'pcf-integration', false );
	...
	public function load( $action = '' ) {
		if ( 'setup' == $action ) {
			if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
				check_admin_referer( 'pcf-recaptcha-setup' );
...
<tr>
	...
	<td class="code"><?php echo esc_html( pcf_mask_password( $secret ) ); ?></td>
</tr>
</tbody>
</table>
...
<p><?php echo sprintf( esc_html( __( "For more details, see %s.", 'contact-form-7' ) ), pcf_link( __( 'https://contactform7.com/recaptcha/', 'contact-form-7' ), __( 'reCAPTCHA', 'contact-form-7' ) ) ); ?></p>
<?php
		}
	}
...
<?php wp_nonce_field( 'pcf-recaptcha-setup' ); ?>
<table class="form-table">
<tbody>
<tr>
...
add_action( 'pcf_init', 'pcf_recaptcha_register_service' );

function pcf_recaptcha_register_service() {
	...
	$services = array(
		'recaptcha' => PCF_RECAPTCHA::get_instance(),
	);
...
add_action( 'pcf_enqueue_scripts', 'pcf_recaptcha_enqueue_scripts' );

function pcf_recaptcha_enqueue_scripts() {
...
add_action( 'wp_footer', 'pcf_recaptcha_callback_script' );

function pcf_recaptcha_callback_script() {
...
document.addEventListener( 'pcfsubmit', function( event ) {
...
add_action( 'pcf_init', 'pcf_recaptcha_add_form_tag_recaptcha' );

function pcf_recaptcha_add_form_tag_recaptcha() {
	$recaptcha = PCF_RECAPTCHA::get_instance();

	if ( $recaptcha->is_active() ) {
		pcf_add_form_tag( 'recaptcha', 'pcf_recaptcha_form_tag_handler',
			array( 'display-block' => true ) );
	}
}

function pcf_recaptcha_form_tag_handler( $tag ) {
	if ( ! wp_script_is( 'google-recaptcha', 'registered' ) ) {
		pcf_recaptcha_enqueue_scripts();
	}
	...
	$atts['class'] = $tag->get_class_option(
		pcf_form_controls_class( $tag->type, 'g-recaptcha' ) );
	$atts['id'] = $tag->get_id_option();

	$html = sprintf( '<div %1$s></div>', pcf_format_atts( $atts ) );
	$html .= pcf_recaptcha_noscript(
		array( 'sitekey' => $atts['data-sitekey'] ) );
	$html = sprintf( '<div class="pcf-form-control-wrap">%s</div>', $html );

	return $html;
}

function pcf_recaptcha_noscript( $args = '' ) {
	$args = wp_parse_args( $args, array(
		'sitekey' => '',
	) );
...
add_filter( 'pcf_spam', 'pcf_recaptcha_check_with_google', 9 );

function pcf_recaptcha_check_with_google( $spam ) {
	if ( $spam ) {
		return $spam;
	}
	...
	$response_token = pcf_recaptcha_response();
	$spam = ! $recaptcha->verify( $response_token );

	return $spam;
}

add_action( 'pcf_admin_init', 'pcf_add_tag_generator_recaptcha', 45 );

function pcf_add_tag_generator_recaptcha() {
	$tag_generator = PCF_TagGenerator::get_instance();
	$tag_generator->add( 'recaptcha', __( 'reCAPTCHA', 'contact-form-7' ),
		'pcf_tag_generator_recaptcha', array( 'nameless' => 1 ) );
}

function pcf_tag_generator_recaptcha( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );
...
<fieldset>
<legend><?php echo sprintf( esc_html( __( "To use reCAPTCHA, first you need to install an API key pair. For more details, see %s.", 'contact-form-7' ) ), pcf_link( __( 'https://contactform7.com/recaptcha/', 'contact-form-7' ), __( 'reCAPTCHA', 'contact-form-7' ) ) ); ?></legend>
</fieldset>
</div>
<?php
	...
	$desc_link = pcf_link( __( 'https://contactform7.com/recaptcha/', 'contact-form-7' ), __( 'reCAPTCHA', 'contact-form-7' ) );
...
function pcf_recaptcha_response() {
