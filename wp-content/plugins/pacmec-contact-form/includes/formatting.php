...
function pcf_autop( $pee, $br = 1 ) {
	...
	/* pcf: remove select and input */
	$allblocks = '(?:table|thead|tfoot|caption|col|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|form|map|area|blockquote|address|math|style|p|h[1-6]|hr|fieldset|legend|section|article|aside|hgroup|header|footer|nav|figure|figcaption|details|menu|summary)';
	$pee = preg_replace( '!(<' . $allblocks . '[^>]*>)!', "\n$1", $pee );
	$pee = preg_replace( '!(</' . $allblocks . '>)!', "$1\n\n", $pee );

	/* pcf: take care of [response], [recaptcha], and [hidden] tags */
	$form_tags_manager = PCF_FormTagsManager::get_instance();
	...
	/* pcf: take care of [response], [recaptcha], and [hidden] tag */
	...
	if ( $br ) {
		...
		$pee = preg_replace_callback(
			...
			'pcf_autop_preserve_newline_callback', $pee );
		$pee = preg_replace( '|(?<!<br />)\s*\n|', "<br />\n", $pee ); // optionally make line breaks
		$pee = str_replace( '<WPPreserveNewline />', "\n", $pee );

		/* pcf: remove extra <br /> just added before [response], [recaptcha], and [hidden] tags */
...
function pcf_autop_preserve_newline_callback( $matches ) {
	return str_replace( "\n", '<WPPreserveNewline />', $matches[0] );
}

function pcf_sanitize_query_var( $text ) {
...
function pcf_strip_quote_deep( $arr ) {
	if ( is_string( $arr ) ) {
		return pcf_strip_quote( $arr );
	}
	...
	if ( is_array( $arr ) ) {
		...
		foreach ( $arr as $key => $text ) {
			$result[$key] = pcf_strip_quote_deep( $text );
		}
...
function pcf_normalize_newline( $text, $to = "\n" ) {
	if ( ! is_string( $text ) ) {
		return $text;
	}
...
function pcf_normalize_newline_deep( $arr, $to = "\n" ) {
	if ( is_array( $arr ) ) {
		$result = array();

		foreach ( $arr as $key => $text ) {
			$result[$key] = pcf_normalize_newline_deep( $text, $to );
		}

		return $result;
	}

	return pcf_normalize_newline( $arr, $to );
}

function pcf_strip_newline( $str ) {
	$str = (string) $str;
	$str = str_replace( array( "\r", "\n" ), '', $str );
	return trim( $str );
}

function pcf_canonicalize( $text, $strto = 'lower' ) {
 ...
 */
function pcf_is_name( $string ) {
	return preg_match( '/^[A-Za-z][-A-Za-z0-9_:.]*$/', $string );
}

function pcf_sanitize_unit_tag( $tag ) {
	$tag = preg_replace( '/[^A-Za-z0-9_-]/', '', $tag );
	return $tag;
}

function pcf_is_email( $email ) {
	$result = is_email( $email );
	return apply_filters( 'pcf_is_email', $result, $email );
}

function pcf_is_url( $url ) {
	$result = ( false !== filter_var( $url, FILTER_VALIDATE_URL ) );
	return apply_filters( 'pcf_is_url', $result, $url );
}

function pcf_is_tel( $tel ) {
	$result = preg_match( '%^[+]?[0-9()/ -]*$%', $tel );
	return apply_filters( 'pcf_is_tel', $result, $tel );
}

function pcf_is_number( $number ) {
	$result = is_numeric( $number );
	return apply_filters( 'pcf_is_number', $result, $number );
}

function pcf_is_date( $date ) {
	...
	return apply_filters( 'pcf_is_date', $result, $date );
}

function pcf_is_mailbox_list( $mailbox_list ) {
...
function pcf_is_email_in_domain( $email, $domain ) {
	$email_list = pcf_is_mailbox_list( $email );
...
function pcf_is_email_in_site_domain( $email ) {
	if ( pcf_is_localhost() ) {
		return true;
	}
	...
	if ( pcf_is_email_in_domain( $email, $site_domain ) ) {
		return true;
	}
	...
	if ( preg_match( '%^https?://([^/]+)%', $home_url, $matches ) ) {
		...
		&& pcf_is_email_in_domain( $email, $site_domain ) ) {
			return true;
		}
	}
...
function pcf_antiscript_file_name( $filename ) {
...
function pcf_mask_password( $text, $length_unmasked = 0 ) {
	$length = strlen( $text );
