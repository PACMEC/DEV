...
add_action( 'pcf_upgrade', 'pcf_convert_to_cpt', 10, 2 );

function pcf_convert_to_cpt( $new_ver, $old_ver ) {
	global $wpdb;
	...
	} elseif ( ( $opt = get_option( 'pcf' ) ) && ! empty( $opt['contact_forms'] ) ) {
		...
		if ( $post_id ) {
			...
			foreach ( $metas as $meta ) {
				update_post_meta( $post_id, '_' . $meta,
					pcf_normalize_newline_deep( maybe_unserialize( $row->{$meta} ) ) );
			}
		}
	}
}

add_action( 'pcf_upgrade', 'pcf_prepend_underscore', 10, 2 );

function pcf_prepend_underscore( $new_ver, $old_ver ) {
	if ( version_compare( $old_ver, '3.0-dev', '<' ) ) {
		return;
	}
	...
	$posts = PCF_ContactForm::find( array(
		'post_status' => 'any',
		'posts_per_page' => -1,
	) );
