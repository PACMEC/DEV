...
class PCF_Mail {
	...
	private function htmlize( $body ) {
		$header = apply_filters( 'pcf_mail_html_header',
			'<!doctype html>
...
', $this );
		...
		$footer = apply_filters( 'pcf_mail_html_footer',
			'</body>
</html>', $this );
	...
	private function compose( $send = true ) {
		...
		$components = apply_filters( 'pcf_mail_components',
			$components, pcf_get_current_contact_form(), $this );

		if ( ! $send ) {
			return $components;
		}

		$subject = pcf_strip_newline( $components['subject'] );
		$sender = pcf_strip_newline( $components['sender'] );
		$recipient = pcf_strip_newline( $components['recipient'] );
	...
	public function replace_tags( $content, $args = '' ) {
		...
		return pcf_mail_replace_tags( $content, $args );
	}
...
function pcf_mail_replace_tags( $content, $args = '' ) {
	$args = wp_parse_args( $args, array(
		'html' => false,
		'exclude_blank' => false,
	) );
	...
	if ( is_array( $content ) ) {
		foreach ( $content as $key => $value ) {
			$content[$key] = pcf_mail_replace_tags( $value, $args );
		}
	...
	foreach ( $content as $num => $line ) {
		$line = new PCF_MailTaggedText( $line, $args );
...
add_action( 'phpmailer_init', 'pcf_phpmailer_init' );

function pcf_phpmailer_init( $phpmailer ) {
	$custom_headers = $phpmailer->getCustomHeaders();
	$phpmailer->clearCustomHeaders();
	$pcf_content_type = false;
	...
	foreach ( (array) $custom_headers as $custom_header ) {
		...
		if ( 'X-WPCF7-Content-Type' === $name ) {
			$pcf_content_type = trim( $value );
		} else {
			$phpmailer->addCustomHeader( $name, $value );
		}
	}

	if ( 'text/html' === $pcf_content_type ) {
		$phpmailer->msgHTML( $phpmailer->Body );
	} elseif ( 'text/plain' === $pcf_content_type ) {
		$phpmailer->AltBody = '';
	}
}

class PCF_MailTaggedText {
	...
	private function replace_tags_callback( $matches, $html = false ) {
		...
		if ( ! empty( $values ) ) {
			...
			$values = pcf_strip_quote_deep( $matches[0] );
		}
		...
		if ( null !== $submitted ) {
			...
			$replaced = pcf_flat_join( $replaced );
			...
			$replaced = apply_filters( 'pcf_mail_tag_replaced',
		...
		$special = apply_filters( 'pcf_special_mail_tags', '', $tagname, $html );
...
add_filter( 'pcf_special_mail_tags', 'pcf_special_mail_tag', 10, 3 );

function pcf_special_mail_tag( $output, $name, $html ) {
	$name = preg_replace( '/^pcf\./', '_', $name ); // for back-compat
