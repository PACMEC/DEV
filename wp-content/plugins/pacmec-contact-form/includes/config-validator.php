...
class PCF_ConfigValidator {
	...
	public function validate() {
		...
		do_action( 'pcf_config_validator_validate', $this );
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		if ( $form_tags ) {
			$form_tag = new PCF_FormTag( $form_tags[0] );
			...
			if ( pcf_form_tag_supports( $form_tag->type, 'selectable-values' ) ) {
				if ( $form_tag->pipes instanceof PCF_Pipes ) {
					if ( $do_not_heat ) {
				...
				if ( $last_item && pcf_is_mailbox_list( $last_item ) ) {
			...
			$tagname = preg_replace( '/^pcf\./', '_', $tagname ); // for back-compat
	...
	public function detect_multiple_controls_in_label( $section, $content ) {
		...
		if ( preg_match_all( $pattern, $content, $matches ) ) {
			...
			foreach ( $matches[1] as $insidelabel ) {
				...
				foreach ( $tags as $tag ) {
					$is_multiple_controls_container = pcf_form_tag_supports(
	...
	public function validate_mail( $template = 'mail' ) {
		...
		$body = new PCF_MailTaggedText( $body,
			array( 'callback' => $callback ) );
	...
	public function detect_invalid_mailbox_syntax( $section, $content, $args = '' ) {
		...
		if ( ! pcf_is_mailbox_list( $content ) ) {
			return $this->add_error( $section,
