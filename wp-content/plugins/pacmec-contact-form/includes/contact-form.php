...
class PCF_ContactForm {

	const post_type = 'pcf_contact_form';
	...
	public static function get_template( $args = '' ) {
		...
		foreach ( $properties as $key => $value ) {
			$properties[$key] = PCF_ContactFormTemplate::get_default( $key );
		}

		$contact_form->properties = $properties;

		$contact_form = apply_filters( 'pcf_contact_form_default_pack',
			$contact_form, $args );
	...
	private static function get_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'pcf-f%1$d-p%2$d-o%3$d',
				absint( $id ), get_the_ID(), $global_count );
		} else {
			$unit_tag = sprintf( 'pcf-f%1$d-o%2$d',
	...
	private function __construct( $post = null ) {
		...
		do_action( 'pcf_contact_form', $this );
	}

	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>PCF_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'contact-form-7' );
	...
	public function get_properties() {
		...
		$properties = (array) apply_filters( 'pcf_contact_form_properties',
			$properties, $this );
	...
	public function locale() {
		if ( pcf_is_valid_locale( $this->locale ) ) {
			return $this->locale;
		} else {
			return '';
		}
	...
	public function is_posted() {
		if ( ! PCF_Submission::get_instance() ) {
			return false;
		}

		if ( empty( $_POST['_pcf_unit_tag'] ) ) {
			return false;
		}

		return $this->unit_tag == $_POST['_pcf_unit_tag'];
	}
	...
	public function form_html( $args = '' ) {
		...
		if ( 'raw_form' == $args['output'] ) {
			return '<pre class="pcf-raw-form"><code>'
				. esc_html( $this->prop( 'form' ) ) . '</code></pre>';
		}

		if ( $this->is_true( 'subscribers_only' )
		&& ! current_user_can( 'pcf_submit', $this->id() ) ) {
			$notice = __(
				"This contact form is available only for logged in users.",
				'contact-form-7' );
			$notice = sprintf(
				'<p class="pcf-subscribers-only">%s</p>',
				esc_html( $notice ) );

			return apply_filters( 'pcf_subscribers_only_notice', $notice, $this );
		}
		...
		$html = sprintf( '<div %s>',
			pcf_format_atts( array(
				'role' => 'form',
				'class' => 'pcf',
				'id' => $this->unit_tag,
				( get_option( 'html_type' ) == 'text/html' ) ? 'lang' : 'xml:lang'
					=> $lang_tag,
				'dir' => pcf_is_rtl( $this->locale ) ? 'rtl' : 'ltr',
			) )
		);

		$html .= "\n" . $this->screen_reader_response() . "\n";

		$url = pcf_get_request_uri();
		...
		$url = apply_filters( 'pcf_form_action_url', $url );

		$id_attr = apply_filters( 'pcf_form_id_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_id'] ) );

		$name_attr = apply_filters( 'pcf_form_name_attr',
		...
		$class = implode( ' ', $class );
		$class = apply_filters( 'pcf_form_class_attr', $class );

		$enctype = apply_filters( 'pcf_form_enctype', '' );
		$autocomplete = apply_filters( 'pcf_form_autocomplete', '' );

		$novalidate = apply_filters( 'pcf_form_novalidate',
			pcf_support_html5() );
		...
		$atts = array(
			...
			'method' => 'post',
			'class' => $class,
			'enctype' => pcf_enctype_value( $enctype ),
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_pcf' => $this->id(),
			'_pcf_version' => PCF_VERSION,
			'_pcf_locale' => $this->locale(),
			'_pcf_unit_tag' => $this->unit_tag,
			'_pcf_container_post' => 0,
		);

		if ( in_the_loop() ) {
			$hidden_fields['_pcf_container_post'] = (int) get_the_ID();
		}

		if ( $this->nonce_is_active() ) {
			$hidden_fields['_wpnonce'] = pcf_create_nonce();
		}

		$hidden_fields += (array) apply_filters(
			'pcf_form_hidden_fields', array() );
	...
	public function form_response_output() {
		...
		if ( $this->is_posted() ) { // Post response output for non-AJAX
			...
			if ( $submission->is( 'validation_failed' ) ) {
				$class .= ' pcf-validation-errors';
			} elseif ( $submission->is( 'spam' ) ) {
				$class .= ' pcf-spam-blocked';
			} elseif ( $submission->is( 'mail_sent' ) ) {
				$class .= ' pcf-mail-sent-ok';
			} elseif ( $submission->is( 'mail_failed' ) ) {
				$class .= ' pcf-mail-sent-ng';
			}
		} else {
			$class .= ' pcf-display-none';
		}
		...
		$output = apply_filters( 'pcf_form_response_output',
	...
	public function validation_error( $name ) {
		...
		$error = sprintf(
			'<span role="alert" class="pcf-not-valid-tip">%s</span>',
			esc_html( $error ) );

		return apply_filters( 'pcf_validation_error', $error, $name, $this );
	}

	/* Form Elements */

	public function replace_all_form_tags() {
		$manager = PCF_FormTagsManager::get_instance();
		$form = $this->prop( 'form' );
	...
	public function form_do_shortcode() {
		pcf_deprecated_function( __METHOD__, '4.6',
			'PCF_ContactForm::replace_all_form_tags' );
	...
	public function form_scan_shortcode( $cond = null ) {
		pcf_deprecated_function( __METHOD__, '4.6',
			'PCF_ContactForm::scan_form_tags' );

		return $this->scan_form_tags( $cond );
	}

	public function form_elements() {
		return apply_filters( 'pcf_form_elements',
	...
	public function collect_mail_tags( $args = '' ) {
		...
		return apply_filters( 'pcf_collect_mail_tags', $mailtags, $args, $this );
	}
	...
	public function submit( $args = '' ) {
		...
		$submission = PCF_Submission::get_instance( $this, array(
		...
		if ( ! empty( $on_submit ) ) {
			$result['scripts_on_submit'] = array_map(
				'pcf_strip_quote', $on_submit );
		}

		do_action( 'pcf_submit', $this, $result );
	...
	public function message( $status, $filter = true ) {
		...
		if ( $filter ) {
			...
			$message = pcf_mail_replace_tags( $message, array( 'html' => true ) );
			$message = apply_filters( 'pcf_display_message', $message, $status );
		}
	...
	public function nonce_is_active() {
		$is_active = PCF_VERIFY_NONCE;

		if ( $this->is_true( 'subscribers_only' ) ) {
			$is_active = true;
		}

		return (bool) apply_filters( 'pcf_verify_nonce', $is_active, $this );
	}
	...
	private function upgrade() {
		...
		if ( is_array( $messages ) ) {
			foreach ( pcf_messages() as $key => $arr ) {
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value ) {
				update_post_meta( $post_id, '_' . $prop,
					pcf_normalize_newline_deep( $value ) );
			}
			...
			if ( $this->initial() ) {
				$this->id = $post_id;
				do_action( 'pcf_after_create', $this );
			} else {
				do_action( 'pcf_after_update', $this );
			}

			do_action( 'pcf_after_save', $this );
		}
	...
	public function copy() {
		...
		return apply_filters( 'pcf_copy', $new, $this );
	}
	...
	public function shortcode( $args = '' ) {
		...
		return apply_filters( 'pcf_contact_form_shortcode', $shortcode, $args, $this );
	}
}
