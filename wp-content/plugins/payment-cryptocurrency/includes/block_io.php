	...
	class BlockIo {
		...
		private function _request( $path, $args = array(), $method = 'POST' ) {
			...
			if ( $result instanceof WP_Error ) {
				...
				$error = '';
				// Use the response body as the result.
				$json_result = json_decode( $result['body'] );
			}

			if ( ! isset( $json_result->status ) || 'success' !== $json_result->status ) {
				$error .= isset( $json_result->data->error_message ) ? $json_result->data->error_message : '';
