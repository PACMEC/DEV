...
if ( ! class_exists( 'Redux_Output', false ) ) {
	...
	class Redux_Output extends Redux_Class {
		...
		public function enqueue() {
			...
			if ( ! empty( $core->typography ) && filter_var( $core->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
				...
				if ( ! class_exists( 'Redux_Typography' ) ) {
					include_once Redux_Core::$dir . '/inc/fields/typography/class-redux-typography.php';
				}
		 ...
		 * @access public
		 */
		public function google_fonts_preconnect( $urls, $relationship_type ): array {
		 ...
		 * @access public
		 */
		private function output_variables( $core, $section = array(), $field = array(), $value = array(), ?string $style_data = '' ) {
		 ...
		 * @return bool
		 */
		private function can_output_css( $core, $field ): ?bool {
			$return = true;
