...
if ( ! class_exists( 'Redux_Functions_Ex', false ) ) {
	...
	class Redux_Functions_Ex {
		 ...
		 * @return string
		 */
		public static function output_alpha_data( $data ) {
		 ...
		 * @access private
		 */
		public static function array_merge_recursive_distinct( $array1, $array2 ): array {
			$merged = $array1;
		 ...
		 */
		public static function nested_wp_parse_args( &$a, $b ): array {
			$a      = $a;
			$b      = $b;
			$result = $b;
		...
		public static function register_class_path( $prefix = '', $path = '' ) {
			if ( ! class_exists( 'Redux_Autoloader' ) ) {
				include_once Redux_Path::get_path( '/inc/classes/class-redux-autoloader.php' );
			}
