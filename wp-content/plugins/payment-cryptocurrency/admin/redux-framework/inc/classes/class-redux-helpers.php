...
if (! class_exists('Redux_Helpers', false) ) {
	...
	class Redux_Helpers {
		 ...
		 * @return array
		 */
		public static function get_extensions( $opt_name = '' ): array {
			if (empty($opt_name) ) {
     ...
     // phpcs:ignore Squiz.PHP.CommentedOutCode
		 ...
		 * @return array
		 */
		public static function process_redux_callers( $simple = false ): array {
			$data = array();
		 ...
		 * @return array
		 */
		public static function localize( $localize ): array {
		 ...
		 * @return array
		 */
		public static function get_request_headers( $args = array() ): array {
			$instances = Redux_Instances::get_all_instances();
		 ...
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function compileSystemStatus( $json_output, $remote_checks ): array {  // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
		 ...
		 * @return array
		 */
		public static function compile_system_status( $json_output = false, $remote_checks = false ): array {
			global $wpdb;
			...
			if (! class_exists('Browser') ) {
				include_once Redux_Core::$dir . 'inc/lib/browser.php';
			}
		 ...
		 * @deprecated No longer using camelCase naming convention.
		 */
		private static function getReduxTemplates( $custom_template_path ): array {  // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
		 ...
		 * @return array
		 */
		private static function get_redux_templates( $custom_template_path ): array {
		 ...
		 * @return array
		 */
		private static function scan_template_files( $template_path ): array {
		 ...
		 *                                   conditions to determine if a user has permission. Invalid conditions
		 *                                   are skipped (conditions which aren't a string/array/bool/number(cast
		 *                                   to bool)). Example array where the user needs to have either the
		 *                                   'edit_posts' capability OR doesn't have the 'delete_pages' cap OR
		 *                                   has the 'update_plugins' AND 'add_users' capabilities. array(
		 *                                   'relation'     => 'OR',      // Optional, defaults to AND.
		 *                                   'edit_posts',                // Equivalent to 'edit_posts' => true,
		 *                                   'delete_pages' => false,     // Tests that the user DOESN'T have
		 *                                   this capability array(                       // Nested conditions
		 *                                   array (up to 30 nestings) 'update_plugins', 'add_users', ), ).
		 *
		 ...
		 * @return array
		 */
		private static function array_map_r( $func, $arr ): array {
			$new_arr = array();
		 ...
		 * @return array
		 */
		public static function redux_stats_additions( $options ): array {
			$options['redux'] = array(
