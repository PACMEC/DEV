...
if ( ! class_exists( 'Redux_Colors', false ) ) {
	...
	class Redux_Colors extends Redux_Class {
		 ...
		 */
		public static function hex_to_hsv( $hex ): array {
		 ...
		 * @return array       returns array( 'h', 's', 'v' ).
		 */
		public static function rgb_to_hsv( $color = array() ): array {
			$var_r = ( $color[0] / 255 );
			$var_g = ( $color[1] / 255 );
			$var_b = ( $color[2] / 255 );
		 ...
		 * @return float
		 */
		public static function lumosity_difference( $color_1 = '#ffffff', $color_2 = '#000000' ): float {
			$color_1 = self::sanitize_hex( $color_1, false );
			$color_2 = self::sanitize_hex( $color_2, false );
