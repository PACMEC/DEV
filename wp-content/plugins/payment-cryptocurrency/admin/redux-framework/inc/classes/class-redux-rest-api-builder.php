...
class Redux_Rest_Api_Builder {
	 ...
	 * @return array
	 */
	public function get_field( $request = array() ): array {
		$type = $request['type'];
		...
		if ( isset( $field_classes[ mb_strtolower( $type ) ] ) ) {
			...
			if ( ! class_exists( $class ) ) {
				include_once $field_classes[ mb_strtolower( $type ) ];
			}
	 ...
	 * @return array
	 */
	public function sort_by_order( $a, $b ): array {
	 ...
	 * @return array
	 */
	private function prepare_fields_output( $fields = array() ): array {
		if ( empty( $fields ) ) {
			return $fields;
		}
	 ...
	 * @return array
	 */
	public function render_field( $request = array() ): array {
		$type          = $request['type'];
		$field_classes = $this->get_field_paths();
		if ( isset( $field_classes[ mb_strtolower( $type ) ] ) ) {
			$class = 'Redux_' . ucwords( str_replace( '-', '_', $type ) );
